...
typedef struct ghl_m1_closure {
   ...
   *
   * When it is false, @c chi is the Eulerian Minerbo Eddington factor built
   * from the Eulerian flux factor sqrt(gamma^ij F_i F_j)/E, while @c xi
   * remains the comoving reduced-flux magnitude of the published tensor. The
   * two are independent reported quantities on this path and are not related
   ...
   *   fluid velocity with fluid Eulerian speed above roughly 0.5c. Flux
...
typedef struct ghl_m1_implicit_solve_diagnostics {
   ...
   * residual_scaled_norm <= 1. A branched source path that performs no Newton
   * iteration (thin, thick-equilibrium, scattering-dominated) reports zero
   * residuals and zero newton_iterations; ghl_m1_neutrino_source_diagnostics
   * path identifies the branch that produced the endpoint. */
 ...
 * callers must still inspect `Jthick_is_valid`.  The candidate is usable by
 * the optional diffusion correction only when that flag is true, which
 * requires a finite, strictly positive value.  A completed calculation may
 * therefore return `ghl_success` with `*Jthick_is_valid == false`.
 */
 ...
 *     (ghl_m1_neutrino_rates). The provider is responsible for EOS lookup,
 * weak-equilibrium targets, unit conversion, and channel-specific microphysics.
 * ========================================================================== */
...
static inline ghl_error_codes_t ghl_m1_compute_neutrino_comoving_moments(
  ...
  const ghl_m1_rad_state rad_state = ghl_m1_neutrino_project_rad_state(state);
  return ghl_m1_compute_comoving_moments(
 ...
 * source_failures exactly once per species from those caller values; an
 * exhausted Newton schedule increments source_terminal_fallbacks exactly once
 * per species instead. No other caller-owned neutrino diagnostic field is
 * changed by an aborted pair transaction, and internal-stage source
 * diagnostics are not published on abort. On successful active pair
 * publication, independent-stage diagnostics are retained and
 * source_converged is incremented once per species for the pair E/F stage.
 * With no active pair fields, the call remains equivalent to the independent
 * single-species path. Exhausted Newton substep schedules return
 ...
 *        Initialized once the required pointers are validated, so a hard
 *        failure leaves a defined no-implicit-solve record rather than the
