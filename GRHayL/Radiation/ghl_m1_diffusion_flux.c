...
ghl_error_codes_t ghl_m1_compute_diffusion_flux(
  ...
  if(!isfinite(hll_flux_tildeE) || !isfinite(E_star) || !isfinite(chi_tr_face)
     || chi_tr_face < 0.0 || !isfinite(D_face) || D_face < 0.0 || !isfinite(delta_l)
     || delta_l <= 0.0) {
  ...
  const double F_diff = W_face * D_face * (gamma_grad + V_face[d] * VdotGrad);
...
ghl_error_codes_t ghl_m1_compute_neutrino_diffusion_flux(
  ...
  return ghl_m1_compute_diffusion_flux(
        m1_params, metric_face, direction, hll_flux_tildeE, E_star, Jthick_L,
        Jthick_L_is_valid, Jthick_R, Jthick_R_is_valid, gradJthick_face, W_face, V_face,
        rates_face->kappa_tr, D_face, delta_l, corrected_flux_tildeE, a_face);
}
