...
static void ghl_m1_reset_optional_diagnostics(ghl_m1_diagnostics *restrict diagnostics) {
...
static ghl_error_codes_t ghl_m1_validate_inputs(
  ...
  if(!isfinite(closure->xi) || closure->xi < 0.0 || closure->xi > 1.0
     || !isfinite(closure->chi) || closure->chi < 1.0 / 3.0 || closure->chi > 1.0
     || !isfinite(closure->root_residual) || closure->root_residual < 0.0
...
ghl_error_codes_t ghl_m1_compute_diagnostics(
  ...
  if(m1_params == NULL || metric == NULL || rad_state == NULL || closure == NULL
     || diagnostics == NULL) {
  ...
  // These fields are workflow-owned and may be filled by callers after closure
  // diagnostics.
