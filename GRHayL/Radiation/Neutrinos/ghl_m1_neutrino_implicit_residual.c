...
static ghl_error_codes_t ghl_m1_neutrino_compute_implicit_residual_core(
  ...
  ghl_error_codes_t error
        = ghl_m1_neutrino_build_trial_state_core(metric, U, &rad_state);
  ...
  error = ghl_m1_neutrino_compute_EF_interaction_sources_validated(
        m1_params, metric, prims_frozen, &rad_state, rates, closure_fallback_observed,
        &EF_sources);
  if(error != ghl_success) {
    return error;
  }
  ...
  residual_local[0] = U[0] - U_base[0] - dt_alpha_sqrt_detgamma * EF_sources.S_E;
  for(int i = 0; i < 3; i++) {
    residual_local[i + 1]
          = U[i + 1] - U_base[i + 1] - dt_alpha_sqrt_detgamma * EF_sources.S[i];
  }
