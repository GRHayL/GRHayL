...
static ghl_error_codes_t ghl_m1_pair_make_effective_rates(
  ...
  for(int process = 0; process < ghl_m1_neutrino_pair_process_count; ++process) {
    ...
    if(pair_rates->eta_E_pair[process] != 0.0) {
      /* Zero partner occupancy contributes no absorption.  Short-circuit it so
       * an overflowing eta_E_pair / J_eq cannot turn inf * 0 into a NaN and
       * reject the required zero-timestep no-op.  The direct expression is
       * retained whenever it is nonzero and representable so normal-range
       * opacities keep their established evaluation order; a finite zero with
       * nonzero factors may have lost a positive quotient to underflow. */
      if(partner_n_com != 0.0) {
        const double direct = pair_rates->eta_E_pair[process] / pair_rates->J_eq
                              * partner_n_com / partner_n_eq;
...
static ghl_error_codes_t ghl_m1_pair_publish_abort(
...
static ghl_error_codes_t ghl_m1_pair_publish_failure(
      ...
      ghl_m1_neutrino_source_diagnostics diagnostics[ghl_m1_pair_species_count],
      ghl_m1_neutrino_diagnostics neutrino_diagnostics[ghl_m1_pair_species_count]) {
  return ghl_m1_pair_publish_abort(
        error, state_transport, state_out, exchange, diagnostics, neutrino_diagnostics,
        false);
}
...
ghl_error_codes_t ghl_m1_solve_neutrino_pair_source_update(
  ...
  ghl_m1_neutrino_source_diagnostics stage_source_diagnostics[ghl_m1_pair_species_count];
  for(int species = 0; species < ghl_m1_pair_species_count; ++species) {
    ghl_m1_pair_zero_source_diagnostics(&stage_source_diagnostics[species]);
  }
  ...
  for(int species = 0; species < ghl_m1_pair_species_count; ++species) {
    ...
    error = ghl_m1_solve_neutrino_source_update(
          ...
          &stage_source_diagnostics[species], &stage_diagnostics[species]);
    if(error != ghl_success) {
  ...
  if(!pair_rates_active) {
    ...
    for(int species = 0; species < ghl_m1_pair_species_count; ++species) {
      ...
      diagnostics[species] = stage_source_diagnostics[species];
  ...
  for(int species = 0; species < ghl_m1_pair_species_count; ++species) {
    ...
    pair_diagnostics[species].source_converged++;
