...
ghl_error_codes_t ghl_m1_solve_neutrino_implicit_homogeneous_update_with_number_policy(
  ...
  /* Once all output pointers are known, state, exchange, and the solve
   * diagnostics are transactional.  Initializing solve_diagnostics here keeps
   * every hard-failure return below from leaving a caller-supplied struct
   * untouched, matching the output-initialization contract used by the thin
   * wrapper above and by ghl_m1_solve_neutrino_source_update.
   * Neutrino diagnostics are observability state: every later hard failure
   * increments source_failures, while exhausted retry schedules increment
  ...
  *exchange = (ghl_m1_neutrino_exchange){ 0 };
  ...
  if(successful_substeps > 0) {
    ...
    double dL_rad_cc = 0.0;
    const ghl_error_codes_t cc_error = ghl_m1_neutrino_charged_current_lepton_delta(
          rates, dt_alpha, state_in->N, number_projected, physical_number_endpoint,
          number_endpoint_gamma, &dL_rad_cc);
    if(cc_error != ghl_success) {
      return ghl_m1_neutrino_publish_hard_failure(cc_error, neutrino_diagnostics);
    }
...
ghl_error_codes_t ghl_m1_solve_neutrino_implicit_homogeneous_update(
  ...
  return ghl_m1_solve_neutrino_implicit_homogeneous_update_with_number_policy(
        m1_params, nu_params, metric, prims_frozen, rates, dt, n_b_cons, -1.0, state_in,
        state_out, exchange, solve_diagnostics, neutrino_diagnostics);
}
