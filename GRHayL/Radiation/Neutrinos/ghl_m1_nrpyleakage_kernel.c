...
static double ghl_m1_nrpyleakage_finite_rate(
      const double x,
...
static bool raw_value_is_valid(const double x) { return isfinite(x) && x >= 0.0; }
...
validate_raw_species(const ghl_m1_nrpyleakage_species_raw_rates *restrict r) {
  if(!isfinite(r->neutrino_degeneracy) || !raw_value_is_valid(r->F2)
     || !raw_value_is_valid(r->F3) || !raw_value_is_valid(r->F4)
     || !raw_value_is_valid(r->F5) || !raw_value_is_valid(r->n_eq_cgs)
     || !raw_value_is_valid(r->J_eq_mev_cgs) || !raw_value_is_valid(r->mean_energy_mev)
     || !raw_value_is_valid(r->eta_N_beta_cgs) || !raw_value_is_valid(r->eta_N_pair_cgs)
...
static ghl_error_codes_t
validate_kernel_result(const ghl_m1_nrpyleakage_legacy_kernel_result *restrict result) {
...
ghl_error_codes_t ghl_m1_nrpyleakage_build_thermo_state_from_eos_quantities(
  ...
  if(!isfinite(rho) || rho <= 0.0 || !isfinite(T) || T <= 0.0 || !isfinite(Ye)
     || Ye < 0.0 || Ye > 1.0) {
    return ghl_error_m1_microphysics_failure;
  }
  ...
  ghl_m1_nrpyleakage_thermo_state candidate
        = { .rho = rho,
            .rho_cgs = rho * NRPyLeakage_units_geom_to_cgs_D,
            .T = T,
            .Ye = Ye,
            .muhat = muhat,
            .mu_e = mu_e,
  ...
  if(Ye < 0.5) {
    ...
    candidate.Y_np = Y_n;
    candidate.Y_pn = metahat_over_T > 0.0 ? (2.0 * Ye - 1.0) / (-expm1(-metahat_over_T))
                                          : (2.0 * Ye - 1.0) * exp(metahat_over_T)
                                                  / expm1(metahat_over_T);
  }
  else {
    candidate.Y_np = Y_n;
    candidate.Y_pn = Y_p;
  }
  if(candidate.Y_np <= 0.0) {
    candidate.Y_np = Y_n;
  }
  if(candidate.Y_pn <= 0.0) {
    candidate.Y_pn = Y_p;
  }
  ...
  if(strict_validation) {
    ...
    if(candidate.rho_cgs <= 0.0 || candidate.X_n < 0.0 || candidate.X_n > 1.0
       || candidate.X_p < 0.0 || candidate.X_p > 1.0 || candidate.Y_np < 0.0
       || candidate.Y_pn < 0.0) {
      return ghl_error_m1_microphysics_failure;
    }
  }
...
static ghl_error_codes_t compute_thermo_state_impl(
  ...
  if(use_legacy_eos_callback) {
    ...
    if(eos->eos_type != ghl_eos_tabulated
       || eos->table_type != ghl_eos_table_stellarcollapse || !isfinite(rho)
       || rho <= 0.0 || !isfinite(T) || T <= 0.0 || !isfinite(Ye) || Ye < 0.0
       || Ye > 1.0) {
      return ghl_error_m1_microphysics_failure;
    }
  ...
  return ghl_m1_nrpyleakage_build_thermo_state_from_eos_quantities(
        rho, Ye, T, muhat, mu_e, mu_p, mu_n, X_n, X_p, strict_validation, thermo);
#endif
}
...
#define EvaluateFermiDiracOrReturn(out, k, z)                                      \
  do {                                                                             \
    const ghl_error_codes_t err = evaluate_fermi_dirac_integral((k), (z), &(out)); \
    if(err != ghl_success)                                                         \
      return err;                                                                  \
  } while(0)
...
static ghl_error_codes_t compute_kernel(
  ...
  const double tmp_11 = NRPyLeakage_enable_beta_nue
                        * EnsureFinite(Y_pn * tmp_10 * tmp_2 * tmp_11_factor);
  const double tmp_12
        = NRPyLeakage_enable_brems_nui_anui
          * EnsureFinite(
                NRPyLeakage_Brems_C1 * NRPyLeakage_Brems_zeta * pow(T, 4.5) * rho_cgs
  ...
  const double tmp_41 = (1 - Y_e) * ((5.0 / 24.0) * tmp_7 + 1.0 / 24.0)
                        / ((2.0 / 3.0) * fmax(mu_n * tmp_0, 0) + 1);
  const double tmp_42 = NRPyLeakage_N_A * NRPyLeakage_sigma_0 * ((T) * (T)) * rho_cgs
                        / ((NRPyLeakage_m_e_c2) * (NRPyLeakage_m_e_c2));
  double tmp_43_fd, tmp_44_fd, tmp_51_fd, tmp_57_fd, tmp_60_fd, tmp_61_fd, tmp_63_fd,
        tmp_70_fd, tmp_71_fd, tmp_78_fd, tmp_83_fd, tmp_87_fd4, tmp_87_fd2;
  ...
  const double tmp_48 = Y_e * ((1.0 / 6.0) * tmp_47 + (5.0 / 24.0) * tmp_7)
                        / ((2.0 / 3.0) * fmax(mu_p * tmp_0, 0) + 1);
  ...
  const double tmp_58 = NRPyLeakage_enable_beta_anue
                        * EnsureFinite(Y_np * tmp_10 * tmp_18 * tmp_58_factor);
  ...
  const double tmp_66
        = EnsureFinite(NRPyLeakage_Brems_C2 * T * tmp_12 / NRPyLeakage_Brems_C1);
...
ghl_error_codes_t ghl_m1_nrpyleakage_compute_raw_rates_from_thermo(
  ...
  const double state_values[]
        = { thermo->rho,   thermo->rho_cgs, thermo->T,    thermo->Ye,
            thermo->muhat, thermo->mu_e,    thermo->mu_p, thermo->mu_n,
            thermo->X_n,   thermo->X_p,     thermo->Y_np, thermo->Y_pn };
  for(size_t i = 0; i < sizeof(state_values) / sizeof(state_values[0]); ++i) {
    if(!isfinite(state_values[i])) {
      return ghl_error_m1_microphysics_failure;
    }
  }
  if(thermo->rho <= 0.0 || thermo->rho_cgs <= 0.0 || thermo->T <= 0.0 || thermo->Ye < 0.0
     || thermo->Ye > 1.0 || thermo->X_n < 0.0 || thermo->X_p < 0.0 || thermo->Y_np < 0.0
     || thermo->Y_pn < 0.0) {
    return ghl_error_m1_microphysics_failure;
  }
  ghl_m1_nrpyleakage_legacy_kernel_result result;
