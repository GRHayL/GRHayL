...
static double ghl_m1_nrpyleakage_record_nonfinite_rate(
      const double x,
...
evaluate_fermi_dirac_integral(const int k, const double z, double *const integral) {
  ...
  const bool invalid_integral = helper_failure | !isfinite(*integral) | (*integral <= 0.0);
  if(invalid_integral) {
    const ghl_error_codes_t error_codes[] = {
        ghl_error_m1_microphysics_failure, err};
...
validate_raw_species(const ghl_m1_nrpyleakage_species_raw_rates *restrict r) {
  const bool invalid = !isfinite(r->neutrino_degeneracy)
                       | !raw_value_is_valid(r->F2)
                       | !raw_value_is_valid(r->F3)
                       | !raw_value_is_valid(r->F4)
                       | !raw_value_is_valid(r->F5)
...
static ghl_error_codes_t validate_kernel_result(
      const ghl_m1_nrpyleakage_kernel_result *restrict result) {
...
ghl_error_codes_t ghl_m1_nrpyleakage_build_thermo_state_from_eos_quantities(
  ...
  const bool invalid_input = !isfinite(rho) | (rho <= 0.0) | !isfinite(T)
                             | (T <= 0.0) | !isfinite(Ye) | (Ye < 0.0)
                             | (Ye > 1.0);
  if(invalid_input) {
    return ghl_error_m1_microphysics_failure;
  }
  ...
  const double values[]
        = { candidate.rho,  candidate.T,    candidate.Ye,   candidate.muhat,
            candidate.mu_e, candidate.mu_p, candidate.mu_n, candidate.X_n,
            candidate.X_p };
...
static ghl_error_codes_t compute_kernel(
  ...
  double X_n, X_p;
  if(NRPyLeakage_normalize_nucleon_fractions(
         thermo->X_n, thermo->X_p, &X_n, &X_p) != ghl_success) {
    return ghl_error_m1_microphysics_failure;
  }
  ...
  double tmp_43_fd, tmp_44_fd, tmp_51_fd, tmp_60_fd, tmp_61_fd, tmp_63_fd,
  ...
  const double nue_N_cc = EnsureFinite(tmp_42 * Y_np * tmp_50 * beta_nue_absorption.number);
  ...
  const double anue_N_cc = EnsureFinite(tmp_42 * Y_pn * tmp_50 * beta_anue_absorption.number);
  ...
  const double nue_E_cc = EnsureFinite(tmp_42 * Y_np * tmp_50 * beta_nue_absorption.energy);
  const double tmp_81 = nue_E_p + nue_E_n + nue_E_cc;
  const double beta_E_anue = NRPyLeakage_enable_beta_anue
  ...
  const double anue_E_cc = EnsureFinite(tmp_42 * Y_pn * tmp_50 * beta_anue_absorption.energy);
...
ghl_error_codes_t ghl_m1_nrpyleakage_compute_raw_rates_from_thermo(
  ...
  const double state_values[]
        = { thermo->rho,  thermo->T,    thermo->Ye,   thermo->muhat,
            thermo->mu_e, thermo->mu_p, thermo->mu_n, thermo->X_n,
            thermo->X_p };
