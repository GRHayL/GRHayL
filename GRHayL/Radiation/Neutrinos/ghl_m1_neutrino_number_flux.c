...
ghl_error_codes_t ghl_m1_neutrino_build_current_from_moments(
  ...
  if(metric == NULL || nu_params == NULL || state == NULL || comoving == NULL
     || V_con == NULL || current == NULL) {
  ...
  if(!isfinite(W) || W < 1.0 || !isfinite(comoving->J)
     || !(comoving->J > nu_params->J_floor) || !isfinite(comoving->Hn)) {
    return ghl_error_m1_invalid_state;
  }
...
ghl_error_codes_t ghl_m1_neutrino_derive_current(
      ...
      const ghl_m1_neutrino_state *restrict state,
      ghl_m1_neutrino_current *restrict current) {
  return derive_current(m1_params, nu_params, metric, prims, state, NULL, current);
}
...
ghl_error_codes_t ghl_m1_neutrino_derive_current_from_closure(
  ...
  return derive_current(m1_params, nu_params, metric, prims, state, closure, current);
}
...
ghl_error_codes_t ghl_m1_neutrino_physical_number_flux_from_current(
  ...
  const ghl_error_codes_t velocity_error
        = ghl_m1_validate_transport_velocity(metric, current->number_transport_velocity);
...
ghl_error_codes_t ghl_m1_compute_neutrino_number_flux_from_closure(
      ...
      double number_flux[3],
      double number_transport_velocity[3]) {
  if(closure == NULL || number_flux == NULL || number_transport_velocity == NULL) {
    return ghl_error_m1_null_pointer;
  }
  ghl_m1_neutrino_current current;
