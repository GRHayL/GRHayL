...
ghl_error_codes_t ghl_m1_compute_neutrino_rusanov_flux(
  ...
  if(m1_params == NULL || nu_params == NULL || metric_face == NULL || state_L == NULL
     || state_R == NULL || closure_L == NULL || closure_R == NULL
     || number_flux_L == NULL || number_flux_R == NULL
     || number_transport_velocity_L == NULL || number_transport_velocity_R == NULL
     || flux_tildeN == NULL || flux_tildeE == NULL || flux_tildeF == NULL) {
    return ghl_error_m1_null_pointer;
  }

  if(direction < ghl_m1_dirn0 || direction > ghl_m1_dirn2 || !isfinite(speed)
     || speed < 0.0) {
  ...
  if(!isfinite(state_L->N) || !isfinite(state_R->N) || state_L->N < nu_params->N_floor
     || state_R->N < nu_params->N_floor) {
    return ghl_error_m1_invalid_state;
  }
  ...
  error = ghl_m1_validate_transport_velocity(metric_face, number_transport_velocity_L);
  if(error != ghl_success) {
    return error;
  }
  error = ghl_m1_validate_transport_velocity(metric_face, number_transport_velocity_R);
  if(error != ghl_success) {
    return error;
  }
  for(int i = 0; i < 3; ++i) {
    ...
    if(!isfinite(expected_L) || !isfinite(expected_R)
       || (state_L->N == 0.0
                 ? number_flux_L[i] != 0.0
                 : fabs(number_flux_L[i] - expected_L) > 128.0 * DBL_EPSILON * scale_L)
  ...
  /* Both closure tensors were validated against this face metric above. */
  error = ghl_m1_compute_physical_flux_validated(
        metric_face, direction, &rad_state_L, closure_L, &physical_E_L, physical_F_L);
  if(error != ghl_success) {
    return error;
  }
  error = ghl_m1_compute_physical_flux_validated(
        metric_face, direction, &rad_state_R, closure_R, &physical_E_R, physical_F_R);
  if(error != ghl_success) {
    return error;
  }
