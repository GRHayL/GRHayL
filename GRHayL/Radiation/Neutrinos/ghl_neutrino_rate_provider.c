...
static ghl_error_codes_t publish_recovered_rates(
  ...
  for(int s = 0; s < ghl_m1_neutrino_species_count; ++s) {
    const ghl_error_codes_t error = ghl_m1_validate_neutrino_rates(&candidate[s], NULL);
...
ghl_error_codes_t ghl_neutrino_rate_provider_initialize_nrpyleakage(
  ...
  provider->emissivity_cgs_to_code
        = pow(NRPyLeakage_units_geom_to_cgs_L, 3) * NRPyLeakage_units_geom_to_cgs_T;
  provider->baryon_mass_code = NRPyLeakage_amu / NRPyLeakage_units_geom_to_cgs_M;
  ...
  /* The production raw kernel already supplies the physical FD mean.  A
   * production provider therefore has no physical mean-energy floor. */
...
static ghl_error_codes_t validate_provider_context(
  ...
  if(provider->backend < ghl_neutrino_rate_backend_reference
     || provider->backend > ghl_neutrino_rate_backend_nrpyleakage
     || !isfinite(provider->nu_x_multiplicity) || provider->nu_x_multiplicity != 4.0
  ...
  if(provider->backend == ghl_neutrino_rate_backend_reference) {
    ...
    if(!isfinite(provider->min_mean_energy) || provider->min_mean_energy != 0.0
       ...
       || provider->baryon_mass_code != NRPyLeakage_amu / NRPyLeakage_units_geom_to_cgs_M
       || provider->charged_current_scale != 1.0 || provider->scattering_scale != 1.0
       || provider->pair_scale != 1.0 || provider->bremsstrahlung_scale != 1.0
       || provider->plasmon_scale != 1.0) {
      return ghl_error_m1_microphysics_failure;
    }
  }
...
static bool same_provider_configuration(
  ...
  return lhs->use_tabulated_eos == rhs->use_tabulated_eos
         ...
         && lhs->min_mean_energy == rhs->min_mean_energy && lhs->backend == rhs->backend
...
static ghl_error_codes_t assemble_nrpyleakage_rates(
  ...
  for(int s = 0; s < ghl_m1_neutrino_species_count; ++s) {
    ...
    const double multiplicity
          = s == ghl_m1_neutrino_nux ? provider->nu_x_multiplicity : 1.0;
...
static void compute_staged_rates(
  ...
  if((mask
    ...
    if((mask & ghl_neutrino_rate_channel_pair) != 0) {
      ...
      const double kappa_E_nue = kappa_N * rates[ghl_m1_neutrino_nue].mean_energy;
      const double kappa_E_anue = kappa_N * rates[ghl_m1_neutrino_anue].mean_energy;
      rates[ghl_m1_neutrino_nue].eta_N_pair[ghl_m1_neutrino_pair_process_pair]
            = kappa_N * electron_n_eq_geometric_mean;
      rates[ghl_m1_neutrino_anue].eta_N_pair[ghl_m1_neutrino_pair_process_pair]
            = kappa_N * electron_n_eq_geometric_mean;
      rates[ghl_m1_neutrino_nue].eta_E_pair[ghl_m1_neutrino_pair_process_pair]
            = kappa_E_nue * rates[ghl_m1_neutrino_nue].J_eq;
      rates[ghl_m1_neutrino_anue].eta_E_pair[ghl_m1_neutrino_pair_process_pair]
            = kappa_E_anue * rates[ghl_m1_neutrino_anue].J_eq;
    }
    if((mask & ghl_neutrino_rate_channel_plasmon) != 0) {
      const double kappa_N = provider->plasmon_scale * theta * theta * theta;
      const double kappa_E_nue = kappa_N * rates[ghl_m1_neutrino_nue].mean_energy;
      const double kappa_E_anue = kappa_N * rates[ghl_m1_neutrino_anue].mean_energy;
      rates[ghl_m1_neutrino_nue].eta_N_pair[ghl_m1_neutrino_pair_process_plasmon]
            = kappa_N * electron_n_eq_geometric_mean;
      rates[ghl_m1_neutrino_anue].eta_N_pair[ghl_m1_neutrino_pair_process_plasmon]
            = kappa_N * electron_n_eq_geometric_mean;
      rates[ghl_m1_neutrino_nue].eta_E_pair[ghl_m1_neutrino_pair_process_plasmon]
            = kappa_E_nue * rates[ghl_m1_neutrino_nue].J_eq;
      rates[ghl_m1_neutrino_anue].eta_E_pair[ghl_m1_neutrino_pair_process_plasmon]
            = kappa_E_anue * rates[ghl_m1_neutrino_anue].J_eq;
    }
    if((mask & ghl_neutrino_rate_channel_bremsstrahlung) != 0) {
      ...
      const double kappa_E_nue = kappa_N * rates[ghl_m1_neutrino_nue].mean_energy;
      const double kappa_E_anue = kappa_N * rates[ghl_m1_neutrino_anue].mean_energy;
      rates[ghl_m1_neutrino_nue].eta_N_pair[ghl_m1_neutrino_pair_process_bremsstrahlung]
            = kappa_N * electron_n_eq_geometric_mean;
      rates[ghl_m1_neutrino_anue].eta_N_pair[ghl_m1_neutrino_pair_process_bremsstrahlung]
            = kappa_N * electron_n_eq_geometric_mean;
      rates[ghl_m1_neutrino_nue].eta_E_pair[ghl_m1_neutrino_pair_process_bremsstrahlung]
            = kappa_E_nue * rates[ghl_m1_neutrino_nue].J_eq;
      rates[ghl_m1_neutrino_anue].eta_E_pair[ghl_m1_neutrino_pair_process_bremsstrahlung]
            = kappa_E_anue * rates[ghl_m1_neutrino_anue].J_eq;
    }
  }
}
...
static ghl_error_codes_t recover_failure(
  ...
  switch(provider->failure_policy) {
    ...
    case ghl_neutrino_rate_failure_hold_last:
      if(!recovered_key_valid || !same_rates_key(cache, provider, eos, rho, T, Ye)) {
        return error;
      }
      ...
      if(diagnostics != NULL) {
        ...
        memcpy(
              ...
              sizeof(diagnostics->beta_kirchhoff_relative_mismatch));
        memcpy(
              diagnostics->beta_kirchhoff_mismatch_valid,
              cache->beta_kirchhoff_mismatch_valid,
              sizeof(diagnostics->beta_kirchhoff_mismatch_valid));
      }
      return ghl_success;
...
ghl_error_codes_t ghl_neutrino_rate_provider_compute_cell(
  ...
  ghl_error_codes_t err = compute_thermo(
        ...
        &mu_p, &mu_n, &X_n, &X_p, &recovered_key_valid);
  if(err != ghl_success) {
    return recover_failure(
          provider, cache, diagnostics, eos, rho, T, Ye, recovered_key_valid, err,
          rates);
  }
  ...
  if(provider->backend == ghl_neutrino_rate_backend_nrpyleakage) {
    err = assemble_nrpyleakage_rates(
          ...
          candidate_mismatch, candidate_mismatch_valid);
    if(err != ghl_success) {
      return recover_failure(
            provider, cache, diagnostics, eos, rho, T, Ye, true, err, rates);
    }
  }
  else {
