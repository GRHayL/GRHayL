...
ghl_error_codes_t ghl_m1_apply_neutrino_number_floor(
  ...
  if(!isfinite(nu_params->N_floor) || nu_params->N_floor < 0.0 || !isfinite(N_in)) {
...
static void ghl_m1_accumulate_neutrino_repair_magnitudes(
  ...
  for(int i = 0; i < 3; i++) {
    diagnostics->repair_dF[i] += fabs(state_after->F[i] - state_before->F[i]);
  }

  const double dL_e
        = diagnostics->repair_lepton_weight * (state_after->N - state_before->N);
...
ghl_error_codes_t ghl_m1_neutrino_check_EN_bounds(
  ...
  if(!isfinite(nu_params->mean_energy_min) || !isfinite(nu_params->mean_energy_max)) {
    return ghl_error_m1_invalid_state;
  }
  if(nu_params->mean_energy_min > 0.0 && nu_params->mean_energy_max > 0.0
  ...
  if(nu_params->mean_energy_min > 0.0 && mean_energy < nu_params->mean_energy_min) {
    return ghl_error_m1_invalid_state;
  }
  if(nu_params->mean_energy_max > 0.0 && mean_energy > nu_params->mean_energy_max) {
    return ghl_error_m1_invalid_state;
  }
  return ghl_success;
}
