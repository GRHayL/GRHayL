...
static bool rate_expected_sum(const double supplied, const double a, const double b) {
  volatile double expected_storage = a + b;
  const double expected = expected_storage;
  if(!isfinite(expected)) {
    return false;
...
ghl_error_codes_t ghl_m1_validate_neutrino_rates(
  ...
  /* Species range */
  if(rates->species != ghl_m1_neutrino_nue && rates->species != ghl_m1_neutrino_anue
  ...
  if(!isfinite(rates->eta_N) || !isfinite(rates->eta_E) || !isfinite(rates->kappa_a_N)
     || !isfinite(rates->kappa_a_E) || !isfinite(rates->kappa_s)
     || !isfinite(rates->kappa_tr) || !isfinite(rates->n_eq) || !isfinite(rates->J_eq)
  ...
  if(rates->eta_N < 0.0 || rates->eta_E < 0.0 || rates->n_eq < 0.0
     || rates->J_eq < 0.0) {
    if(diagnostics != NULL) {
      diagnostics->provider_validation_failures++;
    }
  ...
  if(rates->kappa_a_N < 0.0 || rates->kappa_a_E < 0.0 || rates->kappa_s < 0.0
     || rates->kappa_tr < 0.0 || rates->eta_N_cc < 0.0 || rates->kappa_a_N_cc < 0.0) {
    if(diagnostics != NULL) {
      diagnostics->provider_validation_failures++;
    }
    return ghl_error_m1_microphysics_failure;
  ...
  for(int process = 0; process < ghl_m1_neutrino_pair_process_count; ++process) {
    ...
    if(!isfinite(eta_N) || !isfinite(eta_E) || eta_N < 0.0 || eta_E < 0.0
       || ((eta_N != 0.0 || eta_E != 0.0)
           && (rates->species == ghl_m1_neutrino_nux || rates->n_eq <= 0.0
               || rates->J_eq <= 0.0))) {
      if(diagnostics != NULL) {
        diagnostics->provider_validation_failures++;
      }
  ...
  if(ghl_m1_neutrino_check_lepton_weight(rates->species, rates->lepton_weight)
     != ghl_success) {
    if(diagnostics != NULL) {
      diagnostics->provider_validation_failures++;
    }
  ...
  if(fegetround() != FE_TONEAREST || rates->eta_N_cc > rates->eta_N
     ...
     || !rate_expected_product(
           rates->eta_N, rates->kappa_a_N, rates->n_eq, &product_underflow[0])
     || !rate_expected_product(
           rates->eta_E, rates->kappa_a_E, rates->J_eq, &product_underflow[1])
     || !rate_expected_product(
           rates->eta_N_cc, rates->kappa_a_N_cc, rates->n_eq, &product_underflow[2])
     || !rate_expected_product(
           rates->J_eq, rates->n_eq, rates->mean_energy, &product_underflow[3])) {
    if(diagnostics != NULL) {
      diagnostics->provider_validation_failures++;
    }
    return ghl_error_m1_microphysics_failure;
...
ghl_error_codes_t ghl_m1_neutrino_validate_single_species_rates(
  ...
  bool needs_partner
        = rates->eta_N != rates->eta_N_cc || rates->kappa_a_N != rates->kappa_a_N_cc;
  for(int process = 0; process < ghl_m1_neutrino_pair_process_count; ++process) {
    needs_partner
          |= rates->eta_N_pair[process] != 0.0 || rates->eta_E_pair[process] != 0.0;
  }
  if(needs_partner) {
    if(diagnostics != NULL) {
      diagnostics->provider_validation_failures++;
