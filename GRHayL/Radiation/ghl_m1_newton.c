...
double ghl_m1_newton_weighted_merit(
  ...
  /* This is an exported entry point; INFINITY is the function's established
   * failure value, so a missing operand reports "no admissible merit" rather
   * than faulting. */
  if(m1_params == NULL || metric == NULL || U == NULL || U_base == NULL
     || residual == NULL) {
    return INFINITY;
  }
