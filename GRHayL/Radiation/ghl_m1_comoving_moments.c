...
static ghl_error_codes_t ghl_m1_compute_comoving_moments_internal(
  ...
  const double FdotV = rad_state->F[0] * V_con[0] + rad_state->F[1] * V_con[1]
  ...
  for(int i = 0; i < 3; i++) {
    const double PijVj = closure->P[i][0] * V_cov[0] + closure->P[i][1] * V_cov[1]
  ...
  candidate.Hn
        = -(V_cov[0] * candidate.HU[0] + V_cov[1] * candidate.HU[1]
