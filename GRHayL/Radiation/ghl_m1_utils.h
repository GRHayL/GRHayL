...
static inline void ghl_m1_lower_spatial_tensor(
  ...
  for(int i = 0; i < 3; i++) {
    for(int j = 0; j < 3; j++) {
      ...
      for(int k = 0; k < 3; k++) {
        for(int l = 0; l < 3; l++) {
          sum += metric->gammaDD[i][k] * metric->gammaDD[j][l] * tensor_UU[k][l];
        }
      }
      tensor_DD[i][j] = sum;
    }
...
static inline double ghl_m1_compute_fd_delta(
      ...
      const double U_base,
      const double U,
      const double U_energy_base,
      const double U_energy) {
...
static inline bool ghl_m1_schedule_error_allows_retry(const ghl_error_codes_t error) {
...
static inline void ghl_m1_initialize_implicit_solve_diagnostics(
  ...
  *diagnostics = (ghl_m1_implicit_solve_diagnostics){ .fallback_substeps = 1,
...
static inline ghl_error_codes_t
ghl_m1_validate_direction(const ghl_m1_direction_t direction) {
...
static inline ghl_error_codes_t ghl_m1_validate_closure_tensor_psd(
   ...
   * metric scale. A zero/nonfinite metric was already rejected through A. */
  for(int i = 0; i < 3; ++i) {
    for(int j = 0; j <= i; ++j) {
      ...
      if(i == j) {
        ...
        /* Normalized entries and the preceding positive pivots bound all
         * off-diagonal factors of this 3x3 Cholesky decomposition. */
        L[i][j] = value / L[j][j];
      }
    }
  }
...
static inline ghl_error_codes_t ghl_m1_validate_transport_velocity(
  ...
  const ghl_error_codes_t error
        = ghl_m1_scaled_vector_norm(metric->gammaDD, velocity, &magnitude);
  if(error != ghl_success) {
    return error;
  }
