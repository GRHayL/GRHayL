...
void ghl_radiation_compute_pressure_tensor_thick(
  ...
  double tH4D[4];
  for (int a = 0; a < 4; a++) {
      tH4D[a] = F4->D[a]/W + coef*W*v4D[a]*((4.*W2 + 1.)*v_dot_F - 4.*W2*E);
  }
  for (int a = 0; a < 4; a++){
    for (int b = 0; b < 4; b++) {
      P_thick->DD[a][b]  = Jo3 * (4. * W2 * v4D[a]*v4D[b] + adm_aux->g4DD[a][b] + n4D[a]*n4D[b]);
...
void assemble_rT_lab_frame(
  ...
  for (int a = 0; a < 4; a++){
    for (int b = 0; b < 4; b++) {
      rT4DD->T4[a][b] = E*n4D[a]*n4D[b] + F4->D[a]*n4D[b] + F4->D[b]*n4D[a] + P4->DD[a][b];
    }
  }
...
void assemble_rT_fluid_frame(
  ...
  for (int a = 0; a < 4; a++){
    for (int b = 0; b < 4; b++) {
      rT4DD->T4[a][b] = J*u4D[a]*u4D[b] + H4->D[a]*u4D[b] + H4->D[b]*u4D[a] + K4->DD[a][b];
    }
  }
