...
ghl_error_codes_t ghl_m1_compute_physical_flux(
      ...
      double flux_F[3]) {
  if(metric_face == NULL || rad_state == NULL || closure == NULL || flux_E == NULL
     || flux_F == NULL) {
  ...
  return ghl_m1_compute_physical_flux_validated(
        metric_face, direction, rad_state, closure, flux_E, flux_F);
}
...
ghl_error_codes_t ghl_m1_compute_rusanov_flux(
  ...
  if(!isfinite(speed) || speed < 0.0 || !isfinite(physical_flux_E_L)
     || !isfinite(physical_flux_E_R) || !isfinite(state_L->E) || !isfinite(state_R->E)) {
    return ghl_error_m1_invalid_state;
  }
