...
static bool
ghl_m1_validate_metric_derivatives(const ghl_metric_quantities *restrict metric_derivs) {
...
ghl_error_codes_t ghl_m1_compute_geometry_sources(
  ...
  if(m1_params == NULL || metric == NULL || metric_derivs_x == NULL
     || metric_derivs_y == NULL || metric_derivs_z == NULL || curv == NULL
     || rad_state == NULL || closure == NULL || geometry_sources == NULL) {
  ...
  const double S_E = sqrt_detgamma * (alpha * P_contracted_K - F_dot_dalpha);
  if(!isfinite(S_E)) {
  ...
  for(int i = 0; i < 3; i++) {
    ...
    S[i] = sqrt_detgamma
           * (-rad_state->E * d_i->lapse + Fj_dibeta_j + 0.5 * alpha * Pjk_digamma_jk);
    if(!isfinite(S[i])) {
      return ghl_error_m1_invalid_state;
    }
  }
