...
static ghl_error_codes_t build_minerbo_workspace(
  ...
  for(int i = 0; i < 3; ++i) {
    ...
    for(int j = 0; j < 3; ++j) {
      ...
      ws->g_uu[i + 1][j + 1] = metric->gammaUU[i][j]
                               - metric->betaU[i] * metric->betaU[j] / (alpha * alpha);
    }
  }
  ...
  for(int mu = 0; mu < 4; ++mu) {
    for(int nu = 0; nu < 4; ++nu) {
      ws->proj_ud[mu][nu] = (mu == nu ? 1.0 : 0.0) + ws->u_u[mu] * u_d[nu];
    }
  }
  ...
  if(F_scale == 0.0) {
    ...
    for(int i = 0; i < 3; ++i) {
      for(int j = 0; j < 3; ++j) {
        F2_shape_ld += (long double)metric->gammaUU[i][j]
                       * F_shape[i + 1] * F_shape[j + 1];
      }
    }
...
static ghl_error_codes_t evaluate_minerbo(
  ...
  for(int mu = 0; mu < 4; ++mu) {
    for(int nu = 0; nu < 4; ++nu) {
      ...
      rTdd[mu][nu] = energy_scale * ws->n_d[mu] * ws->n_d[nu]
                     + ws->F_d[mu] * ws->n_d[nu] + ws->n_d[mu] * ws->F_d[nu]
                     + Pdd[mu][nu];
  ...
  if(!scaled_energy) {
    ...
    for(int i = 0; i < 3; ++i) {
      ...
      for(int j = 0; j < 3; ++j) {
        for(int k = 0; k < 3; ++k) {
          PijVj_ld += (long double)ws->metric->gammaUU[i][k]
                     * Pdd[k + 1][j + 1] * ws->VU[j];
        }
      }
    ...
    for(int i = 0; i < 3; ++i) {
      FdotV_over_E_ld += (long double)F_con[i] * ws->VU[i];
      for(int j = 0; j < 3; ++j) {
        PVV_over_E_ld += (long double)(Pdd[i + 1][j + 1] / energy_scale)
                        * ws->VU[i] * ws->VU[j];
      }
    }
    const double W2 = ws->W * ws->W;
    J = W2 * (1.0 - 2.0 * (double)FdotV_over_E_ld
              + (double)PVV_over_E_ld);
    ...
    for(int i = 0; i < 3; ++i) {
      ...
      HU_over_E[i] = ws->W * (F_con[i] - (double)PijVj_over_E_ld
                               - J * ws->VU[i]);
    }
    ...
    for(int i = 0; i < 3; ++i) {
      ...
      for(int j = 0; j < 3; ++j) {
        H2_over_E2_ld += (long double)ws->metric->gammaDD[i][j]
                        * HU_over_E[i] * HU_over_E[j];
      }
    }
...
static ghl_error_codes_t ghl_m1_compute_closure_minerbo_internal(
  ...
  if(g0 == 0.0 || g1 == 0.0 || fabs(g0) <= endpoint_roundoff
    ...
    for(iterations = 0; iterations < m1_params->closure_root_max_iterations;
      ...
      if(error != ghl_success) { increment_counter(3); return error; }
      fb = gm;
    }
