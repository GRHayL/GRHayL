...
/* Function    : interpolate_for_A_gauge_rhs_from_ADM()
...
void interpolate_for_A_gauge_rhs_from_ADM(
  ...
  for(int kk=0;kk<=1;kk++) for(int jj=0;jj<=1;jj++) for(int ii=0;ii<=1;ii++) {
        ...
        const double sqrtg = gauge_vars->sqrtg[kk][jj][ii];
        lapse_sqrtg[kk][jj][ii]=lapse*sqrtg;
        lapse_over_sqrtg[kk][jj][ii]=lapse/sqrtg;
      }
  ...
  const double lapse_sqrtg_jk = avg(2, lapse_sqrtg,       0,0, 0,1, 0,1);
  ...
  gauge_rhs_vars->sqrtg_Ax_interp[0] = lapse_sqrtg_jk*
  ...
  const double lapse_sqrtg_ik = avg(2, lapse_sqrtg,       0,1, 0,0, 0,1);
  ...
  gauge_rhs_vars->sqrtg_Ay_interp[0] = lapse_sqrtg_ik*
  ...
  const double lapse_sqrtg_ij = avg(2, lapse_sqrtg,       0,1, 0,1, 0,0);
  ...
  gauge_rhs_vars->sqrtg_Az_interp[0] = lapse_sqrtg_ij*
