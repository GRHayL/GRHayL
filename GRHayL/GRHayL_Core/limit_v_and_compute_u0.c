 ...
 * ghl_parameters::max_Lorentz_factor and computes ghl_primitive_quantities::u0.
 *
...
ghl_error_codes_t ghl_limit_v_and_compute_u0(
   ...
   * Let the maximum Lorenz factor be
   * \f$ W_\mathrm{max} \equiv 1 - \left( 1 - u^\mu u_\mu \right)^{-1/2} \f$.
   * Then, we want to compute \f$ u^0 \f$ and enforce this speed limit.
   ...
   * \f[
   * \begin{align}
   * \Omega &= \tilde{u}^i \tilde{u}_i \\
   *        & = \gamma_{i j} \alpha^{-2} (v^i + \beta^i)(v^j + \beta^j) \\
   ...
   * \f]
   *
   * where step 2 comes from Eq. 53 of \cite Duez_IGM and step 4 comes from
   * Eq. 56 of the same paper. The quantity \f$ \Omega \f$
   * is conveniently related to both the Lorenz factor and \f$ u^0 \f$, giving both
   * of the quantities we want.
   */
  const double utU[3] = {prims->vU[0] + ADM_metric->betaU[0], prims->vU[1] + ADM_metric->betaU[1], prims->vU[2] + ADM_metric->betaU[2]};
  double one_minus_one_over_W_squared = ghl_compute_vec2_from_vec3D(ADM_metric->gammaDD, utU)*ADM_metric->lapseinv2;
   ...
   * factor. We recompute \f$ v^i \f$ from the rescaled \f$ \tilde{u}^i \f$
   */
  const double one_minus_one_over_W_max_squared = 1.0 - params->inv_sq_max_Lorentz_factor;
  if(one_minus_one_over_W_squared > one_minus_one_over_W_max_squared) {
    const double correction_fac = sqrt(one_minus_one_over_W_max_squared/one_minus_one_over_W_squared);
    ...
    one_minus_one_over_W_squared = one_minus_one_over_W_max_squared;
    *speed_limited |= true;
  }
   ...
   * \f]
   */
  const double W = 1.0/sqrt(1.0-one_minus_one_over_W_squared);
