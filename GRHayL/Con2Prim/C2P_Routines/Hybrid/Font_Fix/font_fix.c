...
int font_fix(
  ...
  if (sdots<1.e-300) {
     ...
     * FONT FIX MAIN LOOP *
     **********************
     * The font fix routine, in theory, should always
     * successfully converge. However, if it fails
     * we iteratively relax the convergence conditions.
     * Only after 5 attempts do we give up.
     */
    ...
    for(int n=0; n<font_fix_attempts; n++) {
      ...
      const loop_tol = tol*pow(4,n); // tolerance multipliers are {0,4,16,64,256}
