...
double ghl_rusanov_average_finite(const double left, const double right) {
...
double ghl_rusanov_candidate_finite(
      ...
      const double physical_flux_R,
      const double speed) {
  const double average = ghl_rusanov_average_finite(physical_flux_L, physical_flux_R);
...
ghl_error_codes_t ghl_calculate_Rusanov_flux(
  ...
  for(int component = 0; component < component_count; ++component) {
    ...
    const double candidate = ghl_rusanov_candidate_finite(
  ...
  for(int component = 0; component < component_count; ++component) {
    flux[component] = ghl_rusanov_candidate_finite(
          state_L[component], state_R[component], physical_flux_L[component],
          physical_flux_R[component], speed);
  }
  return ghl_success;
