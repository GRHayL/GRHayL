...
void ghl_calculate_HLLE_fluxes_tabulated(
  ...
  const double rho_l_term1 = prims_l->rho*cmax*prims_l->vU[direction]*prims_l->u0;
  const double rho_r_term1 = prims_r->rho*cmin*prims_r->vU[direction]*prims_r->u0;
  const double rho_l_term2 = prims_l->rho*cmul*prims_l->u0;
  const double rho_r_term2 = -prims_r->rho*cmul*prims_r->u0;

  cons->rho = sqrt_detg_over_csum*(rho_l_term1 + rho_l_term2 + rho_r_term1 + rho_r_term2);
  ...
  cons->Y_e = sqrt_detg_over_csum*(prims_l->Y_e*rho_l_term1 + prims_l->Y_e*rho_l_term2 + prims_r->Y_e*rho_r_term1 + prims_r->Y_e*rho_r_term2);
}
