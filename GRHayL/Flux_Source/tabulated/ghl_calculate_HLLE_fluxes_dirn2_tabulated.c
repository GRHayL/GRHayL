...
const double wavespeed_scale =
      fmax(1.0, fmax(fabs(cmin_dirn2), fabs(cmax_dirn2)));
...
const double cmin_clamped = fmax(cmin_dirn2, 0.0);
const double cmax_clamped = fmax(cmax_dirn2, 0.0);
...
const double gamma_faceDD22 = metric_face->gammaDD[2][2];
  ...
  const double tmp_4 = BlU2 + tmp_3*u4lU3;
  ...
  const double tmp_11 = BlU0 + tmp_3*u4lU1;
  const double tmp_12 = BlU1 + tmp_3*u4lU2;
  const double tmp_14 = tmp_11*tmp_4*tmp_9;
  const double tmp_15 = tmp_12*tmp_4*tmp_9;
  const double tmp_16 = tmp_3*tmp_8/u4lU0;
  const double tmp_20 = gamma_faceDD01*tmp_11*tmp_12*tmp_9 + gamma_faceDD02*tmp_14 + gamma_faceDD12*tmp_15 + tmp_0*tmp_11*tmp_16 + tmp_1*tmp_12*tmp_16 + tmp_16*tmp_2*tmp_4;
  const double tmp_21 = beta_faceU0*tmp_0 + beta_faceU1*tmp_1 + beta_faceU2*tmp_2 - tmp_6;
  const double tmp_22 = ((tmp_3)*(tmp_3))*tmp_8;
  const double tmp_23 = gamma_faceDD00*((tmp_11)*(tmp_11))*tmp_9 + gamma_faceDD11*((tmp_12)*(tmp_12))*tmp_9 + gamma_faceDD22*tmp_10 + tmp_21*tmp_22;
  ...
  const double tmp_28 = -((beta_faceU2)*(beta_faceU2))*tmp_7 + tmp_27*(gamma_faceDD00*gamma_faceDD11 - ((gamma_faceDD01)*(gamma_faceDD01)));
  const double tmp_29 = P_l + _Rational_1_2*tmp_23 + tmp_20;
  const double tmp_30 = -tmp_10 + tmp_24*((u4lU3)*(u4lU3)) + tmp_28*tmp_29;
  const double tmp_31 = tmp_29*tmp_7;
  const double tmp_32 = tmp_24*u4lU0;
  const double tmp_33 = beta_faceU2*tmp_31 - tmp_16*tmp_4 + tmp_32*u4lU3;
  const double tmp_36 = -beta_faceU0*beta_faceU2*tmp_7 + tmp_27*(gamma_faceDD01*gamma_faceDD12 - gamma_faceDD02*gamma_faceDD11);
  const double tmp_37 = -tmp_14 + tmp_24*u4lU1*u4lU3 + tmp_29*tmp_36;
  const double tmp_38 = -beta_faceU1*beta_faceU2*tmp_7 + tmp_27*(-gamma_faceDD00*gamma_faceDD12 + gamma_faceDD01*gamma_faceDD02);
  const double tmp_39 = -tmp_15 + tmp_24*u4lU2*u4lU3 + tmp_29*tmp_38;
  ...
  const double tmp_44 = BrU2 + tmp_43*u4rU3;
  const double tmp_46 = tmp_8/((u4rU0)*(u4rU0));
  const double tmp_47 = ((tmp_44)*(tmp_44))*tmp_46;
  const double tmp_48 = BrU0 + tmp_43*u4rU1;
  const double tmp_49 = BrU1 + tmp_43*u4rU2;
  const double tmp_51 = tmp_44*tmp_46*tmp_48;
  const double tmp_52 = tmp_44*tmp_46*tmp_49;
  const double tmp_53 = tmp_43*tmp_8/u4rU0;
  const double tmp_57 = gamma_faceDD01*tmp_46*tmp_48*tmp_49 + gamma_faceDD02*tmp_51 + gamma_faceDD12*tmp_52 + tmp_0*tmp_48*tmp_53 + tmp_1*tmp_49*tmp_53 + tmp_2*tmp_44*tmp_53;
  const double tmp_58 = ((tmp_43)*(tmp_43))*tmp_8;
  const double tmp_59 = gamma_faceDD00*tmp_46*((tmp_48)*(tmp_48)) + gamma_faceDD11*tmp_46*((tmp_49)*(tmp_49)) + gamma_faceDD22*tmp_47 + tmp_21*tmp_58;
  const double tmp_60 = _Integer_2*tmp_57 + h_r*rhob_r + tmp_59;
  const double tmp_61 = P_r + _Rational_1_2*tmp_59 + tmp_57;
  const double tmp_62 = tmp_28*tmp_61 - tmp_47 + tmp_60*((u4rU3)*(u4rU3));
  const double tmp_63 = tmp_61*tmp_7;
  const double tmp_64 = tmp_60*u4rU0;
  const double tmp_65 = beta_faceU2*tmp_63 - tmp_44*tmp_53 + tmp_64*u4rU3;
  const double tmp_67 = tmp_36*tmp_61 - tmp_51 + tmp_60*u4rU1*u4rU3;
  const double tmp_68 = tmp_38*tmp_61 - tmp_52 + tmp_60*u4rU2*u4rU3;
  const double tmp_69 = cmin_weight*tmp_41;
  const double tmp_70 = -tmp_22 + tmp_24*((u4lU0)*(u4lU0)) - tmp_31;
  const double tmp_71 = beta_faceU0*tmp_31 - tmp_11*tmp_16 + tmp_32*u4lU1;
  const double tmp_72 = beta_faceU1*tmp_31 - tmp_12*tmp_16 + tmp_32*u4lU2;
  const double tmp_73 = -tmp_58 + tmp_60*((u4rU0)*(u4rU0)) - tmp_63;
  const double tmp_74 = beta_faceU0*tmp_63 - tmp_48*tmp_53 + tmp_64*u4rU1;
  const double tmp_75 = beta_faceU1*tmp_63 - tmp_49*tmp_53 + tmp_64*u4rU2;
  const double tmp_76 = rhob_l*tmp_42*u4lU3;
  const double tmp_77 = rhob_r*tmp_69*u4rU3;
  const double tmp_79 = rhob_l*tmp_41*u4lU0;
  const double tmp_81 = rhob_r*tmp_41*u4rU0;
  const double tmp_82 = tmp_40*tmp_6;
  cons->SD[0] = -dissipation_speed*(alpha_face*tmp_40*(gamma_faceDD00*tmp_74 + gamma_faceDD01*tmp_75 + gamma_faceDD02*tmp_65 + tmp_0*tmp_73) - tmp_41*(gamma_faceDD00*tmp_71 + gamma_faceDD01*tmp_72 + gamma_faceDD02*tmp_33 + tmp_0*tmp_70)) + tmp_42*(gamma_faceDD00*tmp_37 + gamma_faceDD01*tmp_39 + gamma_faceDD02*tmp_30 + tmp_0*tmp_33) + tmp_69*(gamma_faceDD00*tmp_67 + gamma_faceDD01*tmp_68 + gamma_faceDD02*tmp_62 + tmp_0*tmp_65);
  cons->SD[1] = -dissipation_speed*(alpha_face*tmp_40*(gamma_faceDD01*tmp_74 + gamma_faceDD11*tmp_75 + gamma_faceDD12*tmp_65 + tmp_1*tmp_73) - tmp_41*(gamma_faceDD01*tmp_71 + gamma_faceDD11*tmp_72 + gamma_faceDD12*tmp_33 + tmp_1*tmp_70)) + tmp_42*(gamma_faceDD01*tmp_37 + gamma_faceDD11*tmp_39 + gamma_faceDD12*tmp_30 + tmp_1*tmp_33) + tmp_69*(gamma_faceDD01*tmp_67 + gamma_faceDD11*tmp_68 + gamma_faceDD12*tmp_62 + tmp_1*tmp_65);
  cons->SD[2] = -dissipation_speed*(alpha_face*tmp_40*(gamma_faceDD02*tmp_74 + gamma_faceDD12*tmp_75 + gamma_faceDD22*tmp_65 + tmp_2*tmp_73) - tmp_41*(gamma_faceDD02*tmp_71 + gamma_faceDD12*tmp_72 + gamma_faceDD22*tmp_33 + tmp_2*tmp_70)) + tmp_42*(gamma_faceDD02*tmp_37 + gamma_faceDD12*tmp_39 + gamma_faceDD22*tmp_30 + tmp_2*tmp_33) + tmp_69*(gamma_faceDD02*tmp_67 + gamma_faceDD12*tmp_68 + gamma_faceDD22*tmp_62 + tmp_2*tmp_65);
  cons->rho = -dissipation_speed*(-tmp_79 + tmp_81) + tmp_76 + tmp_77;
  cons->tau = cmax_weight*(-rhob_l*tmp_41*u4lU3 + tmp_33*tmp_82) + cmin_weight*(-rhob_r*tmp_41*u4rU3 + tmp_65*tmp_82) - dissipation_speed*(tmp_40*tmp_6*tmp_73 - tmp_70*tmp_82 + tmp_79 - tmp_81);
  cons->Y_e = Y_e_l*tmp_76 + Y_e_r*tmp_77 - dissipation_speed*(-Y_e_l*tmp_79 + Y_e_r*alpha_face*rhob_r*tmp_40*u4rU0);
}
return ghl_success;
}
