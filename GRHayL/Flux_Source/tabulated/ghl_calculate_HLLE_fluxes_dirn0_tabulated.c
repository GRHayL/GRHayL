...
const double gamma_faceDD22 = metric_face->gammaDD[2][2];
  ...
  const double tmp_76 = rhob_l*tmp_42*u4lU1;
  const double tmp_77 = rhob_r*tmp_69*u4rU1;
...
void ghl_calculate_HLLE_fluxes_dirn0_tabulated(ghl_primitive_quantities *restrict prims_r, ghl_primitive_quantities *restrict prims_l, const ghl_eos_parameters *restrict eos, const ghl_metric_quantities *restrict metric_face, const double cmin_dirn0, const double cmax_dirn0, ghl_conservative_quantities *restrict cons) {
  ghl_abort_if_error(ghl_calculate_HLLE_fluxes_dirn0_tabulated_checked(prims_r, prims_l, eos, metric_face, cmin_dirn0, cmax_dirn0, cons));
}
