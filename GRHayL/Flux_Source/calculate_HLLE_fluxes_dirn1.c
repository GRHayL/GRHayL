...
const double gamma_faceDD22 = metric_face->adm_gzz;
  ...
  const double tmp_5 = BlU1 + tmp_4*u4lU2;
  ...
  const double tmp_22 = tmp_10*tmp_12*tmp_5;
  const double tmp_23 = gamma_faceDD02*tmp_10*tmp_12*tmp_15;
  const double tmp_24 = tmp_10*tmp_15*tmp_5;
  const double tmp_26 = tmp_4*tmp_9/u4lU0;
  const double tmp_28 = tmp_1*tmp_12*tmp_26;
  const double tmp_30 = tmp_2*tmp_26*tmp_5;
  const double tmp_32 = tmp_15*tmp_26*tmp_3;
  const double tmp_33 = 2*gamma_faceDD01*tmp_22 + gamma_faceDD11*tmp_11 + 2*gamma_faceDD12*tmp_24 + h_l*rhob_l + tmp_13 + tmp_16 + tmp_17*tmp_18 + 2*tmp_23 + 2*tmp_28 + 2*tmp_30 + 2*tmp_32;
  ...
  const double tmp_37 = -((beta_faceU1)*(beta_faceU1))*tmp_8 + tmp_36*(gamma_faceDD00*gamma_faceDD22 - ((gamma_faceDD02)*(gamma_faceDD02)));
  const double tmp_38 = P_l + gamma_faceDD01*tmp_22 + (1.0/2.0)*gamma_faceDD11*tmp_11 + gamma_faceDD12*tmp_24 + (1.0/2.0)*tmp_13 + (1.0/2.0)*tmp_16 + (1.0/2.0)*tmp_17*tmp_18 + tmp_23 + tmp_28 + tmp_30 + tmp_32;
  const double tmp_39 = -tmp_11 + tmp_33*((u4lU2)*(u4lU2)) + tmp_37*tmp_38;
  const double tmp_40 = tmp_38*tmp_8;
  const double tmp_41 = tmp_33*u4lU0;
  const double tmp_42 = beta_faceU1*tmp_40 - tmp_26*tmp_5 + tmp_41*u4lU2;
  ...
  const double tmp_51 = cmax_dirn1*tmp_50;
  ...
  const double tmp_53 = BrU1 + tmp_52*u4rU2;
  ...
  const double tmp_65 = tmp_53*tmp_55*tmp_57;
  const double tmp_66 = gamma_faceDD02*tmp_55*tmp_57*tmp_60;
  const double tmp_67 = tmp_53*tmp_55*tmp_60;
  const double tmp_69 = tmp_52*tmp_9/u4rU0;
  const double tmp_71 = tmp_1*tmp_57*tmp_69;
  const double tmp_73 = tmp_2*tmp_53*tmp_69;
  const double tmp_75 = tmp_3*tmp_60*tmp_69;
  const double tmp_76 = 2*gamma_faceDD01*tmp_65 + gamma_faceDD11*tmp_56 + 2*gamma_faceDD12*tmp_67 + h_r*rhob_r + tmp_17*tmp_62 + tmp_58 + tmp_61 + 2*tmp_66 + 2*tmp_71 + 2*tmp_73 + 2*tmp_75;
  const double tmp_77 = P_r + gamma_faceDD01*tmp_65 + (1.0/2.0)*gamma_faceDD11*tmp_56 + gamma_faceDD12*tmp_67 + (1.0/2.0)*tmp_17*tmp_62 + (1.0/2.0)*tmp_58 + (1.0/2.0)*tmp_61 + tmp_66 + tmp_71 + tmp_73 + tmp_75;
  const double tmp_78 = tmp_37*tmp_77 - tmp_56 + tmp_76*((u4rU2)*(u4rU2));
  const double tmp_79 = tmp_77*tmp_8;
  const double tmp_80 = tmp_76*u4rU0;
  const double tmp_81 = beta_faceU1*tmp_79 - tmp_53*tmp_69 + tmp_80*u4rU2;
  const double tmp_83 = tmp_45*tmp_77 - tmp_65 + tmp_76*u4rU1*u4rU2;
  const double tmp_84 = tmp_47*tmp_77 - tmp_67 + tmp_76*u4rU2*u4rU3;
  const double tmp_85 = cmin_dirn1*tmp_50;
  ...
  const double tmp_92 = cmax_dirn1*cmin_dirn1;
  const double tmp_94 = rhob_l*tmp_50*u4lU0;
  const double tmp_96 = rhob_r*tmp_50*u4rU0;
  const double tmp_97 = tmp_49*tmp_7;
  cons->S_x = tmp_0*(tmp_51*(gamma_faceDD00*tmp_46 + gamma_faceDD01*tmp_39 + gamma_faceDD02*tmp_48 + tmp_1*tmp_42) + tmp_85*(gamma_faceDD00*tmp_83 + gamma_faceDD01*tmp_78 + gamma_faceDD02*tmp_84 + tmp_1*tmp_81) - tmp_92*(alpha_face*tmp_49*(gamma_faceDD00*tmp_90 + gamma_faceDD01*tmp_81 + gamma_faceDD02*tmp_91 + tmp_1*tmp_89) - tmp_50*(gamma_faceDD00*tmp_87 + gamma_faceDD01*tmp_42 + gamma_faceDD02*tmp_88 + tmp_1*tmp_86)));
  cons->S_y = tmp_0*(tmp_51*(gamma_faceDD01*tmp_46 + gamma_faceDD11*tmp_39 + gamma_faceDD12*tmp_48 + tmp_2*tmp_42) + tmp_85*(gamma_faceDD01*tmp_83 + gamma_faceDD11*tmp_78 + gamma_faceDD12*tmp_84 + tmp_2*tmp_81) - tmp_92*(alpha_face*tmp_49*(gamma_faceDD01*tmp_90 + gamma_faceDD11*tmp_81 + gamma_faceDD12*tmp_91 + tmp_2*tmp_89) - tmp_50*(gamma_faceDD01*tmp_87 + gamma_faceDD11*tmp_42 + gamma_faceDD12*tmp_88 + tmp_2*tmp_86)));
  cons->S_z = tmp_0*(tmp_51*(gamma_faceDD02*tmp_46 + gamma_faceDD12*tmp_39 + gamma_faceDD22*tmp_48 + tmp_3*tmp_42) + tmp_85*(gamma_faceDD02*tmp_83 + gamma_faceDD12*tmp_78 + gamma_faceDD22*tmp_84 + tmp_3*tmp_81) - tmp_92*(alpha_face*tmp_49*(gamma_faceDD02*tmp_90 + gamma_faceDD12*tmp_81 + gamma_faceDD22*tmp_91 + tmp_3*tmp_89) - tmp_50*(gamma_faceDD02*tmp_87 + gamma_faceDD12*tmp_42 + gamma_faceDD22*tmp_88 + tmp_3*tmp_86)));
  cons->rho = tmp_0*(rhob_l*tmp_51*u4lU2 + rhob_r*tmp_85*u4rU2 - tmp_92*(-tmp_94 + tmp_96));
  cons->tau = tmp_0*(cmax_dirn1*(-rhob_l*tmp_50*u4lU2 + tmp_42*tmp_97) + cmin_dirn1*(-rhob_r*tmp_50*u4rU2 + tmp_81*tmp_97) - tmp_92*(tmp_49*tmp_7*tmp_89 - tmp_86*tmp_97 + tmp_94 - tmp_96));
}
}
