 ...
 *                reconstructed rhor and rhol values
...
void ghl_steepen_var(
  ...
  const bool contact_discontinuity_check =
                 Gamma_eff * params->ppm_shock_k0 * fabs(rho[PLUS_1]-rho[MINUS1]) * MIN(pressure[PLUS_1], pressure[MINUS1])
  ...
  if(contact_discontinuity_check && second_deriv_check && relative_change_check) {
    ...
    const double eta_tilde = -(1.0/6.0)*(d2rho_b_p1 - d2rho_b_m1)/d1rho_b;
