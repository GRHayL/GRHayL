...
ghl_error_codes_t NRPyLeakage_compute_neutrino_luminosities(
  ...
  const double tmp_6
        = NRPyLeakage_eta_nue_0 * tmp_4 + (1 - tmp_4) * (-muhat * tmp_0 + tmp_1);
  ...
  const double tmp_14
        = NRPyLeakage_eta_anue_0 * tmp_12 + (1 - tmp_12) * (muhat * tmp_0 - tmp_1);
  ...
  if(X_n > 0.0 && X_p > 0.0) {
    const double reaction_shift
          = nrpyl_compute_reaction_shift(T, muhat, eta_n_minus_eta_p);
  ...
  const double tmp_20
        = ((NRPyLeakage_gamma_0) * (NRPyLeakage_gamma_0)) * tmp_17 / (tmp_18 + 1);
  const double tmp_21 = -1.0 / 2.0 * tmp_20 - 1;
  const double tmp_23 = (1.0 / ((NRPyLeakage_hc3) * (NRPyLeakage_hc3)));
  const double tmp_24 = pow(T, 8);
  const double tmp_26 = (1.0 / 3.0) * ((M_PI) * (M_PI) * (M_PI)) * NRPyLeakage_beta
                        * pow(NRPyLeakage_gamma_0, 6) * ((tmp_17) * (tmp_17) * (tmp_17))
                        * tmp_23 * tmp_24 * (tmp_18 + 1) * exp(-tmp_18)
  ...
  const double tmp_37
        = (1.0 / 64.0) * ((NRPyLeakage_hc3) * (NRPyLeakage_hc3)) * tmp_29 * tmp_32
          * (tmp_15 * tmp_2 * tmp_23 * tmp_31 * tmp_36 + tmp_30 * tmp_34 * tmp_36)
          / (tmp_15 * tmp_24);
  const double tmp_38
        = tmp_11
          + EnsureFinite(
                NRPyLeakage_enable_pair_nue_anue * tmp_37
                * EnsureFinite(
                      NRPyLeakage_C1pC2_nue_anue * tmp_35
                      / ((exp(tmp_14 + tmp_33) + 1) * (exp(tmp_33 + tmp_6) + 1))))
          + EnsureFinite(
                NRPyLeakage_enable_plasmon_nue_anue * tmp_27
                * EnsureFinite(
                      ((NRPyLeakage_C_V) * (NRPyLeakage_C_V)) * tmp_26
                      / ((exp(tmp_14 + tmp_21) + 1) * (exp(tmp_21 + tmp_6) + 1))));
  const double tmp_39
        = tmp_38
          + NRPyLeakage_enable_beta_nue
  ...
  const double tmp_52
        = NRPyLeakage_units_cgs_to_geom_Q * W * ((alpha) * (alpha))
          * sqrt(
                gammaxx * gammayy * gammazz - gammaxx * ((gammayz) * (gammayz))
                - ((gammaxy) * (gammaxy)) * gammazz + 2 * gammaxy * gammaxz * gammayz
                - ((gammaxz) * (gammaxz)) * gammayy);
  const double tmp_54
        = tmp_38
  ...
  const double Q_free_nux
        = tmp_11
          + EnsureFinite(
                NRPyLeakage_enable_pair_nux_anux * tmp_37
                * EnsureFinite(
                      NRPyLeakage_C1pC2_nux_anux * tmp_35
                      / ((exp(tmp_33) + 1) * (exp(tmp_33) + 1))))
          + EnsureFinite(
                NRPyLeakage_enable_plasmon_nux_anux * tmp_27
                * EnsureFinite(
                      tmp_26 * tmp_46 / ((exp(tmp_21) + 1) * (exp(tmp_21) + 1))));
  const double kappa_nue
        = EnsureFinite(tmp_43 * tmp_45) + EnsureFinite(tmp_43 * tmp_47)
          + EnsureFinite(tmp_42 * Y_np * tmp_48 * beta_nue_absorption.energy);
  const double kappa_anue
        = EnsureFinite(tmp_42 * tmp_45 * tmp_57) + EnsureFinite(tmp_42 * tmp_47 * tmp_57)
          + EnsureFinite(tmp_42 * Y_pn * tmp_48 * beta_anue_absorption.energy);
  const double kappa_nux = EnsureFinite(tmp_42 * tmp_45 * tmp_61)
                           + EnsureFinite(tmp_42 * tmp_47 * tmp_61);
  const double Q_eff_nue
        = nrpyl_effective_emission_rate(tmp_39, tau->nue[1], kappa_nue, tmp_41 * tmp_49);
  ...
  return nrpyl_sanitize_luminosities(lum) ? ghl_error_nrpyleakage_nonfinite_output
                                          : ghl_success;
}
