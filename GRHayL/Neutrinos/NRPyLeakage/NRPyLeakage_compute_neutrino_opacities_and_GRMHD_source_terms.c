...
ghl_error_codes_t NRPyLeakage_compute_neutrino_opacities_and_GRMHD_source_terms(
  ...
  const double tmp_15
        = NRPyLeakage_eta_anue_0 * tmp_13 + (1 - tmp_13) * (muhat * tmp_0 - tmp_1);
  ...
  const double tmp_29
        = NRPyLeakage_enable_pair_nue_anue
          * EnsureFinite(
                NRPyLeakage_C1pC2_nue_anue * tmp_28
                / ((exp(tmp_15 + tmp_21) + 1) * (exp(tmp_21 + tmp_6) + 1)));
  ...
  const double tmp_37
        = NRPyLeakage_enable_plasmon_nue_anue
          * EnsureFinite(
                ((NRPyLeakage_C_V) * (NRPyLeakage_C_V)) * tmp_36
                / ((exp(tmp_15 + tmp_35) + 1) * (exp(tmp_35 + tmp_6) + 1)));
  ...
  const double tmp_53
        = EnsureFinite(tmp_45 * tmp_49) + EnsureFinite(tmp_41 * tmp_42 * tmp_45)
  ...
  const double tmp_65
        = EnsureFinite(tmp_41 * tmp_62) + EnsureFinite(tmp_48 * tmp_62)
  ...
  const double tmp_73
        = EnsureFinite(tmp_49 * tmp_71) + EnsureFinite(tmp_41 * tmp_42 * tmp_71);
  const double tmp_74 = 4 * ((T) * (T) * (T) * (T)) * tmp_8;
  const double tmp_75
        = tmp_66 + EnsureFinite(tmp_29 * tmp_68) + EnsureFinite(tmp_37 * tmp_69);
  const double tmp_76
        = tmp_75
  ...
  const double tmp_81
        = EnsureFinite(tmp_49 * tmp_79) + EnsureFinite(tmp_41 * tmp_42 * tmp_79)
  ...
  const double tmp_86
        = EnsureFinite(tmp_49 * tmp_84) + EnsureFinite(tmp_41 * tmp_42 * tmp_84)
  ...
  const double Q_free_nux
        = tmp_66
          + EnsureFinite(
                NRPyLeakage_enable_pair_nux_anux * tmp_68
                * EnsureFinite(
                      NRPyLeakage_C1pC2_nux_anux * tmp_28
                      / ((exp(tmp_21) + 1) * (exp(tmp_21) + 1))))
          + EnsureFinite(
                NRPyLeakage_enable_plasmon_nux_anux * tmp_69
                * EnsureFinite(
                      tmp_36 * tmp_47 / ((exp(tmp_35) + 1) * (exp(tmp_35) + 1))));
  ...
  const double Q_eff_nue
        = nrpyl_effective_emission_rate(tmp_76, tau->nue[1], tmp_81, tmp_74 * tmp_78);
  const double Q_eff_anue
        = nrpyl_effective_emission_rate(tmp_82, tau->anue[1], tmp_86, tmp_74 * tmp_83);
  const double Q_eff_nux = nrpyl_effective_emission_rate(
        Q_free_nux, tau->nux[1], tmp_73, tmp_70 * tmp_74);
  *R_source
        = NRPyLeakage_amu * NRPyLeakage_units_cgs_to_geom_R * (-R_eff_nue + R_eff_anue);
  *Q_source = nrpyl_matter_energy_source(Q_eff_nue, Q_eff_anue, Q_eff_nux);
  ...
  kappa->nux[0]
        = NRPyLeakage_units_geom_to_cgs_L
          * (EnsureFinite(tmp_49 * tmp_87) + EnsureFinite(tmp_41 * tmp_42 * tmp_87));
  ...
  return replaced_sources || replaced_opacities ? ghl_error_nrpyleakage_nonfinite_output
                                                : ghl_success;
}
