...
ghl_error_codes_t NRPyLeakage_compute_neutrino_opacities(
  ...
  const double tmp_4
        = NRPyLeakage_eta_nue_0 * tmp_0 + (1 - tmp_0) * (-muhat * tmp_1 + tmp_2);
  ...
  const double tmp_6 = NRPyLeakage_N_A * NRPyLeakage_sigma_0 * ((T) * (T)) * rho_cgs
  ...
  const double tmp_11
        = B_p * (tmp_9 + (1.0 / 6.0) * ((NRPyLeakage_C_V - 1) * (NRPyLeakage_C_V - 1)));
  ...
  const double tmp_20
        = NRPyLeakage_eta_anue_0 * tmp_18 + (1 - tmp_18) * (muhat * tmp_1 - tmp_2);
  ...
  double tmp_21_fd, tmp_22_fd, tmp_24_fd, tmp_26_fd, tmp_28_fd4, tmp_28_fd2, tmp_30_fd5,
        tmp_30_fd3;
  ...
  kappa->nux[0]
        = NRPyLeakage_units_geom_to_cgs_L
          * (EnsureFinite(tmp_17 * tmp_28) + EnsureFinite(tmp_10 * tmp_28 * tmp_6));
  kappa->nux[1]
        = NRPyLeakage_units_geom_to_cgs_L
          * (EnsureFinite(tmp_17 * tmp_30) + EnsureFinite(tmp_10 * tmp_30 * tmp_6));

  // Step 5: Make sure public results are finite; otherwise use the established floor.
  return nrpyl_sanitize_opacities(kappa) ? ghl_error_nrpyleakage_nonfinite_output
                                         : ghl_success;
}
