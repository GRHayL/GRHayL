...
[TestData](https://github.com/GRHayL/TestData) repository. Tests with
...
the input data by a small test-specific relative factor, commonly of the form

\f[
\mathrm{input}_\mathrm{pert}
  = \mathrm{input} \times (1 + \mathrm{rand}(-1,1) \times \epsilon)
\f]

where \f$ \epsilon \f$ is the perturbation magnitude chosen by each test
