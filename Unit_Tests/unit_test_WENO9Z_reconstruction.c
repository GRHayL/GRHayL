...
int main(int argc, char **argv) {
  FILE* infile = fopen_with_check("WENO9Z_reconstruction_input.bin", "rb");
  ...
  infile = fopen_with_check("WENO9Z_reconstruction_output.bin", "rb");
  FILE* inpert = fopen_with_check("WENO9Z_reconstruction_output_pert.bin", "rb");
  ...
  for(int index = NGHOSTS; index < arraylength - 4; index++) {
      double var_r, var_l;
      ghl_weno9z_reconstruction(&var[index - 5], &var_r, &var_l);

      char mname[20];
      sprintf(mname, "WENO9Z");
  ...
  ghl_info("WENO9Z reconstruction test has passed!\n");
  free(var);
