...
static inline uint64_t m1_neutrino_seeded_next_u64(
...
static inline double m1_neutrino_seeded_unit(
      m1_neutrino_seeded_rng *restrict rng) {
...
static inline double m1_neutrino_seeded_metric_norm(
  ...
  for(int i = 0; i < 3; ++i)
    for(int j = 0; j < 3; ++j)
      norm_squared += (long double)vector[i] * metric_tensor[i][j]
                    * vector[j];
...
static inline void m1_neutrino_seeded_make_metric(
  ...
  *metric = (ghl_metric_quantities){0};
  ...
  const double determinant = a * (d * f - e * e)
                          - b * (b * f - c * e)
                          + c * (b * e - c * d);
...
static inline void m1_neutrino_seeded_make_primitives(
  ...
  *prims = (ghl_primitive_quantities){0};
  ...
  prims->press = prims->rho * prims->eps *
                 m1_neutrino_seeded_uniform(rng, 0.8, 1.4);
  ...
  switch(species) {
    case ghl_m1_neutrino_nue:
      ...
      rates->kappa_a_N_cc = rates->kappa_a_N *
                            m1_neutrino_seeded_uniform(rng, 0.15, 0.85);
...
static inline void m1_neutrino_seeded_make_geometry_data(
  ...
  for(int direction = 0; direction < 3; ++direction) {
    ...
    metric_derivs[direction].lapse =
        m1_neutrino_seeded_uniform(rng, -0.15, 0.15);
    for(int j = 0; j < 3; ++j) {
      metric_derivs[direction].betaU[j] =
          m1_neutrino_seeded_uniform(rng, -0.15, 0.15);
      for(int k = 0; k < 3; ++k)
...
static inline void m1_neutrino_seeded_make_case(
  ...
  m1_neutrino_seeded_make_primitives(
      rng, moving, &test_case->metric, &test_case->prims);
  m1_neutrino_seeded_make_state(
      rng, anisotropic, &test_case->metric, &test_case->state);
  ...
  for(int species = 0; species < ghl_m1_neutrino_species_count; ++species)
    m1_neutrino_seeded_make_rates(
        rng, (ghl_m1_neutrino_species_t)species,
        &test_case->rates[species]);
  m1_neutrino_seeded_make_geometry_data(
      rng, test_case->metric_derivs, &test_case->curv);
}
...
static inline double m1_neutrino_seeded_flux_factor(
  ...
  const double norm = m1_neutrino_seeded_metric_norm(
      metric->gammaUU, state->F);
  return norm / state->E;
}

static inline void m1_neutrino_seeded_default_parameters(
