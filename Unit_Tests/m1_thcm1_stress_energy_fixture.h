...
static inline void m1_thcm1_stress_energy_fixture_error(
  ...
  if(error != NULL && error_size > 0) {
    snprintf(
          error, error_size, "%s",
          message != NULL ? message : "stress-energy fixture error");
  }
}
...
static inline int m1_thcm1_stress_energy_fixture_build_metric(
  ...
  ghl_initialize_metric(
        input[M1_THCM1_STRESS_ENERGY_LAPSE], input[M1_THCM1_STRESS_ENERGY_SHIFT0],
        input[M1_THCM1_STRESS_ENERGY_SHIFT1], input[M1_THCM1_STRESS_ENERGY_SHIFT2],
  ...
  return metric->lapse > 0.0 && isfinite(metric->lapse) && metric->detgamma > 0.0
         && isfinite(metric->detgamma) && metric->sqrt_detgamma > 0.0
         && isfinite(metric->sqrt_detgamma);
}
...
static inline int m1_thcm1_stress_energy_fixture_build_primitives(
  ...
  if(input == NULL || metric == NULL || prims == NULL) {
    return 0;
  }
  *prims = (ghl_primitive_quantities){ 0 };
  double v_con[3] = { 0.0, 0.0, 0.0 };
  for(int i = 0; i < 3; ++i) {
    v_con[i] = (input[M1_THCM1_STRESS_ENERGY_V0 + i] + metric->betaU[i]) / metric->lapse;
  }
  double v2 = 0.0;
  for(int i = 0; i < 3; ++i) {
    for(int j = 0; j < 3; ++j) {
...
static inline int m1_thcm1_stress_energy_fixture_evaluate(
  ...
  ghl_m1_neutrino_state state
        = { .N = input[M1_THCM1_STRESS_ENERGY_N],
            ...
            .F = { input[M1_THCM1_STRESS_ENERGY_F0], input[M1_THCM1_STRESS_ENERGY_F1],
  ...
  if(!m1_thcm1_stress_energy_fixture_build_metric(input, &metric)
     ...
     || ghl_m1_initialize(
              M1_THCM1_STRESS_ENERGY_EPSILON_C, M1_THCM1_STRESS_ENERGY_ENERGY_FLOOR,
              M1_THCM1_STRESS_ENERGY_ZETA_MIN, M1_THCM1_STRESS_ENERGY_FD_EPSILON_REL,
              M1_THCM1_STRESS_ENERGY_FD_EPSILON_ABS,
              M1_THCM1_STRESS_ENERGY_NEWTON_MAX_ITERATIONS,
              M1_THCM1_STRESS_ENERGY_NEWTON_TOLERANCE, &params)
              != ghl_success) {
  ...
  if(ghl_m1_compute_closure_with_primitives(
           &params, &metric, &prims, &radiation, &closure)
           != ghl_success
     || ghl_m1_compute_neutrino_stress_energy(
              &params, &metric, &state, &closure, &stress)
              != ghl_success) {
    m1_thcm1_stress_energy_fixture_error(
  ...
  static const int components[M1_THCM1_STRESS_ENERGY_FIXTURE_OUTPUT_COUNT][2]
        = { { 0, 0 }, { 0, 1 }, { 0, 2 }, { 0, 3 }, { 1, 1 },
            { 1, 2 }, { 1, 3 }, { 2, 2 }, { 2, 3 }, { 3, 3 } };
  for(size_t component = 0; component < M1_THCM1_STRESS_ENERGY_FIXTURE_OUTPUT_COUNT;
      ++component) {
...
static inline int m1_thcm1_stress_energy_fixture_normalization(
  ...
  if(baseline == NULL || perturbed == NULL || normalization == NULL) {
    return 0;
  }
  const double scale = fmax(
        1.0e-14, fmax(fabs(baseline[M1_THCM1_STRESS_ENERGY_E]),
...
static inline int m1_thcm1_run_stress_energy_fixtures(
  ...
  if(!m1_thcm1_fixture_load(
           ...
           M1_THCM1_STRESS_ENERGY_FIXTURE_OUTPUT_COUNT, &collection, error,
           error_size)) {
    return 0;
  }
  ...
  for(size_t index = 0; index < collection.record_count; ++index) {
    ...
    double perturbed[M1_THCM1_STRESS_ENERGY_FIXTURE_OUTPUT_COUNT] = { 0 };
    ...
    if(!m1_thcm1_stress_energy_fixture_evaluate(
             record->perturbed_input, perturbed, error, error_size)) {
      m1_thcm1_fixture_free(&collection);
      return 0;
    }
  ...
  m1_thcm1_fixture_free(&collection);
  ghl_info(
        "THC_M1 stress-energy fixtures: %zu trusted/perturbed agreement pairs passed\n",
        completed);
  return 1;
}
