...
int main(int argc, char **argv) {
  ...
  for(int i=0; i<neos-1; i++) {
    ...
    if(!isfinite(eos.p_ppoly[i])
       || relative_error(expected_left, eos.p_ppoly[i]) > 1e-14
       || relative_error(expected_left, expected_right) > 1e-14)
      ghl_error("Pressure transition %d is invalid: expected %.15e, got %.15e, adjacent %.15e.\n",
  ...
  error = ghl_initialize_hybrid_eos_functions_and_params(
        1.0, 0.0, 100.0, 1, one_rho_ppoly, one_Gamma_ppoly,
        1.0, 2.0, &one_piece_eos);
  ...
  error = ghl_initialize_hybrid_eos_functions_and_params(
        1.0, 0.0, 100.0, 2, two_rho_ppoly, two_Gamma_ppoly,
        1.0, 2.0, &two_piece_eos);
  const double expected_left = pow(two_rho_ppoly[0], two_Gamma_ppoly[0]);
  const double expected_K1 = pow(two_rho_ppoly[0],
                                 two_Gamma_ppoly[0]-two_Gamma_ppoly[1]);
