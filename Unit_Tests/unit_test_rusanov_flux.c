...
static void check_shared_rusanov_boundaries(void) {
  ...
  if(ghl_calculate_Rusanov_flux(
         NULL, state_R, flux_L, flux_R, 3, 0.5, output) !=
         ghl_error_flux_source_invalid_input ||
     ghl_calculate_Rusanov_flux(
         state_L, NULL, flux_L, flux_R, 3, 0.5, output) !=
         ghl_error_flux_source_invalid_input ||
     ghl_calculate_Rusanov_flux(
         state_L, state_R, NULL, flux_R, 3, 0.5, output) !=
         ghl_error_flux_source_invalid_input ||
     ghl_calculate_Rusanov_flux(
         state_L, state_R, flux_L, NULL, 3, 0.5, output) !=
         ghl_error_flux_source_invalid_input ||
     ghl_calculate_Rusanov_flux(
         state_L, state_R, flux_L, flux_R, 3, 0.5, NULL) !=
         ghl_error_flux_source_invalid_input ||
     ghl_calculate_Rusanov_flux(
         state_L, state_R, flux_L, flux_R, 0, 0.5, output) !=
         ghl_error_flux_source_invalid_input ||
     ghl_calculate_Rusanov_flux(
         state_L, state_R, flux_L, flux_R, 3, NAN, output) !=
         ghl_error_flux_source_invalid_input ||
     ghl_calculate_Rusanov_flux(
         state_L, state_R, flux_L, flux_R, 3, -0.5, output) !=
         ghl_error_flux_source_invalid_input)
  ...
  if(ghl_calculate_Rusanov_flux(
         huge_state_L, huge_state_R, zero_flux_L, zero_flux_R, 1, DBL_MAX,
         rejected) != ghl_error_flux_source_invalid_input || rejected[0] != 155.0)
    fail_case("shared Rusanov candidate overflow was published", 405);

  if(ghl_calculate_Rusanov_flux(
         state_L, state_R, flux_L, flux_R, 3, 0.5, output) != ghl_success)
    fail_case("shared Rusanov finite boundary case failed", 406);
  for(int i = 0; i < 3; ++i) {
    const double expected = 0.5 * (flux_L[i] + flux_R[i])
                          - 0.25 * (state_R[i] - state_L[i]);
...
static void check_m1_rusanov_boundaries(
  ...
  if(ghl_m1_compute_physical_flux(
         NULL, ghl_m1_dirn0, &state_L, &closure_L, &physical_E,
         physical_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_physical_flux(
         metric, ghl_m1_dirn0, NULL, &closure_L, &physical_E,
         physical_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_physical_flux(
         metric, ghl_m1_dirn0, &state_L, NULL, &physical_E,
         physical_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_physical_flux(
         metric, ghl_m1_dirn0, &state_L, &closure_L, NULL,
         physical_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_physical_flux(
         metric, ghl_m1_dirn0, &state_L, &closure_L, &physical_E,
         NULL) != ghl_error_m1_null_pointer)
    fail_case("M1 physical flux NULL boundary failed", 411);
  if(ghl_m1_compute_physical_flux(
         metric, (ghl_m1_direction_t)3, &state_L, &closure_L,
         &physical_E, physical_F) != ghl_error_m1_invalid_state)
  ...
  if(ghl_m1_compute_physical_flux(
         &bad_metric, ghl_m1_dirn0, &state_L, &closure_L,
         &physical_E, physical_F) != ghl_error_m1_invalid_metric)
    fail_case("M1 physical flux invalid metric was accepted", 413);
  ghl_m1_closure bad_closure = closure_L;
  bad_closure.P[0][0] = NAN;
  if(ghl_m1_compute_physical_flux(
         metric, ghl_m1_dirn0, &state_L, &bad_closure,
         &physical_E, physical_F) != ghl_error_m1_invalid_state)
  ...
  if(ghl_m1_compute_physical_flux(
         &huge_lapse_metric, ghl_m1_dirn0, &high_state, &high_closure,
         &physical_E, physical_F) != ghl_error_m1_invalid_state)
  ...
  if(ghl_m1_compute_rusanov_flux(
         NULL, &state_R, 0.1, flux_F_L, 0.2, flux_F_R, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_rusanov_flux(
         &state_L, NULL, 0.1, flux_F_L, 0.2, flux_F_R, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, 0.1, NULL, 0.2, flux_F_R, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, 0.1, flux_F_L, 0.2, NULL, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, 0.1, flux_F_L, 0.2, flux_F_R, 0.5,
         NULL, rusanov_F) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, 0.1, flux_F_L, 0.2, flux_F_R, 0.5,
         &rusanov_E, NULL) != ghl_error_m1_null_pointer)
    fail_case("typed M1 Rusanov NULL boundary failed", 418);
  if(ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, 0.1, flux_F_L, 0.2, flux_F_R, NAN,
         &rusanov_E, rusanov_F) != ghl_error_m1_invalid_state ||
     ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, 0.1, flux_F_L, 0.2, flux_F_R, -1.0,
         &rusanov_E, rusanov_F) != ghl_error_m1_invalid_state ||
     ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, NAN, flux_F_L, 0.2, flux_F_R, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_invalid_state ||
     ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, 0.1, flux_F_L, NAN, flux_F_R, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_invalid_state)
    fail_case("typed M1 Rusanov scalar validation failed", 419);
  ghl_m1_rad_state bad_state = state_L;
  bad_state.E = NAN;
  if(ghl_m1_compute_rusanov_flux(
         &bad_state, &state_R, 0.1, flux_F_L, 0.2, flux_F_R, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_invalid_state)
    fail_case("typed M1 Rusanov left state validation failed", 420);
  bad_state = state_R;
  bad_state.E = NAN;
  if(ghl_m1_compute_rusanov_flux(
         &state_L, &bad_state, 0.1, flux_F_L, 0.2, flux_F_R, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_invalid_state)
  ...
  if(ghl_m1_compute_rusanov_flux(
         &state_L, &state_R, 0.1, bad_flux_F, 0.2, flux_F_R, 0.5,
         &rusanov_E, rusanov_F) != ghl_error_m1_invalid_state ||
     rusanov_E != rusanov_before[0] ||
  ...
  memcpy(rusanov_F, rusanov_before + 1, sizeof(rusanov_F));
  if(ghl_m1_compute_rusanov_flux(
         &huge_state_L, &huge_state_R, 0.0, flux_F_L, 0.0, flux_F_R,
         DBL_MAX, &rusanov_E, rusanov_F) != ghl_error_m1_invalid_state ||
     rusanov_E != rusanov_before[0] ||
     memcmp(rusanov_F, rusanov_before + 1, sizeof(rusanov_F)) != 0)
  ...
  if(ghl_m1_compute_number_rusanov_flux(
         1.0, 1.2, 0.1, 0.2, 0.5, NULL) != ghl_error_m1_null_pointer ||
     ghl_m1_compute_number_rusanov_flux(
         NAN, 1.2, 0.1, 0.2, 0.5, &number_flux) != ghl_error_m1_invalid_state ||
     ghl_m1_compute_number_rusanov_flux(
         1.0, 1.2, NAN, 0.2, 0.5, &number_flux) != ghl_error_m1_invalid_state ||
     ghl_m1_compute_number_rusanov_flux(
         1.0, 1.2, 0.1, NAN, 0.5, &number_flux) != ghl_error_m1_invalid_state ||
     ghl_m1_compute_number_rusanov_flux(
         1.0, 1.2, 0.1, 0.2, NAN, &number_flux) != ghl_error_m1_invalid_state ||
     ghl_m1_compute_number_rusanov_flux(
         1.0, 1.2, 0.1, 0.2, -1.0, &number_flux) != ghl_error_m1_invalid_state)
    fail_case("scalar M1 Rusanov validation failed", 424);
  number_flux = 181.0;
  if(ghl_m1_compute_number_rusanov_flux(
         0.0, DBL_MAX, 0.0, 0.0, DBL_MAX, &number_flux) !=
         ghl_error_m1_invalid_state || number_flux != 181.0)
...
static void check_physical_flux(
  ...
  if(ghl_m1_compute_physical_flux(
         metric, direction, state, closure, &actual_E, actual_F) != ghl_success)
  ...
  const double expected_E = metric->lapse * raised_F[d]
                          - metric->betaU[d] * state->E;
  ...
  for(int i = 0; i < 3; ++i) {
    ...
    const double expected_F = metric->lapse * P_mixed
                            - metric->betaU[d] * state->F[i];
    check_close(actual_F[i], expected_F,
                "physical momentum flux mismatch", case_index);
  }
}

static void check_case(const ghl_m1_parameters *restrict m1_params,
                       const ghl_metric_quantities *restrict metric,
                       const ghl_primitive_quantities *restrict prims,
  ...
  for(int direction = 0; direction < 3; ++direction) {
    check_physical_flux(metric, (ghl_m1_direction_t)direction,
                        state_L, &closure_L, case_index);
    check_physical_flux(metric, (ghl_m1_direction_t)direction,
                        state_R, &closure_R, case_index);
  }
  ...
  double actual_F[3] = {NAN, NAN, NAN};
  if(ghl_m1_compute_rusanov_flux(
         state_L, state_R, flux_E_L, flux_F_L, flux_E_R, flux_F_R,
         speed, &actual_E, actual_F) != ghl_success)
    fail_case("typed Rusanov flux rejected finite input", case_index);

  const double expected_E = 0.5 * (flux_E_L + flux_E_R)
                          - 0.5 * speed * (state_R->E - state_L->E);
  ...
  for(int i = 0; i < 3; ++i) {
    ...
    check_close(actual_F[i], expected_F,
                "typed Rusanov momentum mismatch", case_index);
  }
  ...
  double actual_N = NAN;
  if(ghl_m1_compute_number_rusanov_flux(
         N_L, N_R, nflux_L, nflux_R, speed, &actual_N) != ghl_success)
    fail_case("scalar Rusanov flux rejected finite input", case_index);
  const double expected_N = 0.5 * (nflux_L + nflux_R)
                          - 0.5 * speed * (N_R - N_L);
  ...
  if(ghl_m1_compute_rusanov_flux(
         state_L, state_R, flux_E_L, flux_F_L, flux_E_R, flux_F_R,
         -1.0, &rejected_E, rejected_F) != ghl_error_m1_invalid_state ||
     rejected_E != 91.0 || rejected_F[0] != 92.0 ||
     rejected_F[1] != 93.0 || rejected_F[2] != 94.0)
    fail_case("invalid typed Rusanov input was not transactional", case_index);

  double rejected_N = 95.0;
  if(ghl_m1_compute_number_rusanov_flux(
         N_L, N_R, nflux_L, nflux_R, -1.0, &rejected_N) !=
         ghl_error_m1_invalid_state || rejected_N != 95.0)
...
static void check_high_energy_closure(
  ...
  for(size_t energy_index = 0;
      energy_index < sizeof(energies) / sizeof(energies[0]); ++energy_index) {
    for(size_t flux_index = 0;
      ...
      const ghl_m1_rad_state state = {
          .E = energy,
          .F = {flux_factors[flux_index] * energy, 0.0, 0.0}};
      ...
      if(!isfinite(closure.chi) || !isfinite(closure.xi) ||
        ...
        fail_case("finite high-energy closure published invalid metadata",
                  100 + case_index);
      for(int i = 0; i < 3; ++i)
        for(int j = 0; j < 3; ++j)
          if(!isfinite(closure.P[i][j] / energy))
            fail_case("finite high-energy closure published a nonfinite tensor",
                      100 + case_index);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  if(!m1_thcm1_rusanov_check_generic_fixture(
    ...
    fail_case(fixture_error[0] != '\0' ? fixture_error :
              "generic Rusanov fixture evaluation failed", -2);
  else
  ...
  check_case(&m1_params, &flat_metric, &flat_prims,
             &flat_L, &flat_R, 0);
  ...
  check_case(&m1_params, &curved_metric, &curved_prims,
             &curved_L, &curved_R, 1);
