...
//
//         const size_t index = ir + ghl_eos->N_rho * (it + ghl_eos->N_T * iy);
//         const double press = ghl_eos->table_all[index + NRPyEOS_press_key];
//         const double muhat = ghl_eos->table_all[index + NRPyEOS_muhat_key];
//
//         // ds/dn, ds/dt, ds/dy
//         const double dsdn = sna_dentrdr + ele_dentrdr;
//         const double dsdt = sna_dentrdt + ele_dentrdt;
//         const double dsdy = sna_dentrdy + ele_dentrdy;
//         // dp/dn, dp/dt, dp/dy
//         const double dpdn = sna_dpresdr + ele_dpresdr;
//         const double dpdt = sna_dpresdt + ele_dpresdt;
//         const double dpdy = sna_dpresdy + ele_dpresdy;
//         // de/dn, de/dt, de/dy
//         const double dedn = press / rho / rho + temp * dsdn;
//         const double dedt = temp * dsdt;
//         const double dedy = -muhat + temp * dsdy;
//
//         ghl_eos->table_all[index + NRPyEOS_dPdrho_key] = dpdrhoe;
//         ghl_eos->table_all[index + NRPyEOS_dPdeps_key] = dpderho;
//         ghl_eos->table_all[index + NRPyEOS_depsdT_key] = dedt;
//       }
//     }
//   }
// }
...
compose_table_to_ghl_eos(const compose_eos_table *compose_table, ghl_eos_parameters *ghl_eos) {
  ...
  const size_t size = ghl_eos->N_rho * ghl_eos->N_T * ghl_eos->N_Ye;
  const size_t size_all = size * NRPyEOS_ntablekeys;
  ...
  for(int iy = 0; iy < ghl_eos->N_Ye; iy++) {
    for(int it = 0; it < ghl_eos->N_T; it++) {
      for(int ir = 0; ir < ghl_eos->N_rho; ir++) {
        const size_t compose_index = compose_eos_table_index(compose_table, ir, it, iy);
        const size_t ghl_eos_index = ir + ghl_eos->N_rho * (it + ghl_eos->N_T * iy);
        ...
        // Q[6]: eps/(n_b * m_n) - 1, dimensionless, so eps dimensionless
        const double eps = (compose_table->Q[6][compose_index] + 1) * rho;
