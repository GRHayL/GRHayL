...
static size_t read_grid_size(FILE *restrict infile, int *restrict dirlength) {
  if(fread(dirlength, sizeof(*dirlength), 1, infile) != 1)
    ghl_error("Could not read the grid size from HLL_flux_input.bin.\n");
  if(*dirlength < 5)
    ghl_error("HLL_flux_input.bin requires a grid size of at least 5; got %d.\n", *dirlength);

  const size_t n = (size_t)*dirlength;
  if(n > SIZE_MAX/n || n*n > SIZE_MAX/n)
    ghl_error("The grid size in HLL_flux_input.bin is too large.\n");
