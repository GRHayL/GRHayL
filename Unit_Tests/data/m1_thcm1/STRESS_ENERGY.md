...
and ten covariant tensor components per pair. The trusted values are the
compiled THC_M1 `assemble_rT` outputs; the current public test recomputes the
GRHayL baseline and uses the retained THC_M1 perturbation response as its
local acceptance envelope.
