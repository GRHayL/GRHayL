...
current GRHayL only on the baseline input: the trusted THC baseline is checked
directly, and the paired THC perturbation supplies the response envelope. These
are reference-test bounds, not production solver settings.
...
The replay recomputes GRHayL's baseline stress-energy tensor and lowers it with
the full ADM four-metric. The trusted THC_M1 perturbed result supplies the
strict propagated-response envelope; the current perturbed endpoint is not
needed for this stored replay. THC_M1, `THCM1_ROOT`, and `Verification/` remain
...
input conventions, and the strict transport comparison policy.
