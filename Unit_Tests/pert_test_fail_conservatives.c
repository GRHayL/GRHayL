...
void ghl_pert_test_fail_conservatives(
  ...
  const double momentum_abs_tol = fmax(
        1.0e-30,
        8.0 * DBL_EPSILON
              * fmax(
                    fabs(cons_trusted->SD[0]),
  ...
  if(ghl_pert_test_fail_with_tolerance(
           cons_trusted->SD[0], cons->SD[0], cons_pert->SD[0], 8.0e-14,
           momentum_abs_tol)) {
  ...
  if(ghl_pert_test_fail_with_tolerance(
           cons_trusted->SD[1], cons->SD[1], cons_pert->SD[1], 8.0e-14,
           momentum_abs_tol)) {
  ...
  if(ghl_pert_test_fail_with_tolerance(
           cons_trusted->SD[2], cons->SD[2], cons_pert->SD[2], 8.0e-14,
           momentum_abs_tol)) {
