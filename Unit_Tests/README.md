...
| `unit_test_m1_thcm1_blended_rusanov` | Constant-volume and variable-volume four-point transport | The corresponding pointwise or prepared-face discrete transport operation. |
...
THC_M1 outputs. The stored-replay adapters evaluate current GRHayL once at the
baseline input, compare that output with the retained baseline, and use the
retained perturbation response as the error envelope. The named local
two-state policy records are the only exceptions. A replay
...
mutable campaign results used to establish a current campaign status. Thus a
passing replay is not a fresh Verification result, and no current target
admission, alias, or fixture promotion is asserted here. A future fixture
refresh remains gated by the Verification-owned accepted receipt and status;
missing or admission-blocked surfaces must remain blocked rather than being
