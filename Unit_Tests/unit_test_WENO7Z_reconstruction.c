...
int main(int argc, char **argv) {
  FILE* infile = fopen_with_check("WENO7Z_reconstruction_input.bin", "rb");
  ...
  infile = fopen_with_check("WENO7Z_reconstruction_output.bin", "rb");
  FILE* inpert = fopen_with_check("WENO7Z_reconstruction_output_pert.bin", "rb");
  ...
  for(int index = NGHOSTS; index < arraylength - 3; index++) {
      double var_r, var_l;
      ghl_weno7z_reconstruction(&var[index - 4], &var_r, &var_l);

      char mname[20];
      sprintf(mname, "WENO7Z");
  ...
  ghl_info("WENO7Z reconstruction test has passed!\n");
  free(var);
