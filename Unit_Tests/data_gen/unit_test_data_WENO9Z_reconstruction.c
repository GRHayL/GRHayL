...
int main(int argc, char **argv) {
  ...
  FILE* outfile = fopen_with_check("WENO9Z_reconstruction_input.bin", "wb");
  ...
  outfile = fopen_with_check("WENO9Z_reconstruction_output.bin", "wb");
  FILE *outpert = fopen("WENO9Z_reconstruction_output_pert.bin", "wb");

  for(int index = NGHOSTS; index < arraylength - 4; index++) {
    ghl_weno9z_reconstruction(&var[index - 5], &var_r[index], &var_l[index]);
  }
  ...
  for(int index = NGHOSTS; index < arraylength - 4; index++) {
    ghl_weno9z_reconstruction(&varpert[index - 5], &var_r[index], &var_l[index]);
  }
