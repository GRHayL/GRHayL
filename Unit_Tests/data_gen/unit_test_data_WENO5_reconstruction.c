...
int main(int argc, char **argv) {
  ...
  FILE* outfile = fopen_with_check("WENO5_reconstruction_input.bin", "wb");
  ...
  outfile = fopen_with_check("WENO5_reconstruction_output.bin", "wb");
  FILE *outpert = fopen("WENO5_reconstruction_output_pert.bin", "wb");

  for(int index=NGHOSTS; index<arraylength-2; index++) {
    ghl_weno5_reconstruction(&var[index-3], &var_r[index], &var_l[index]);
  }
  ...
  for(int index=NGHOSTS; index<arraylength-2; index++) {
    ghl_weno5_reconstruction(&varpert[index-3], &var_r[index], &var_l[index]);
  }
