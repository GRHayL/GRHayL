...
int main(int argc, char **argv) {
  ...
  FILE* outfile = fopen_with_check("WENO7Z_reconstruction_input.bin", "wb");
  ...
  outfile = fopen_with_check("WENO7Z_reconstruction_output.bin", "wb");
  FILE *outpert = fopen("WENO7Z_reconstruction_output_pert.bin", "wb");

  for(int index = NGHOSTS; index < arraylength - 3; index++) {
    ghl_weno7z_reconstruction(&var[index - 4], &var_r[index], &var_l[index]);
  }
  ...
  for(int index = NGHOSTS; index < arraylength - 3; index++) {
    ghl_weno7z_reconstruction(&varpert[index - 4], &var_r[index], &var_l[index]);
  }
