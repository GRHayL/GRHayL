...
int main(int argc, char **argv) {
  ...
  if(error != ghl_success
     || !isfinite(rectangular_eos.eps_min) || !isfinite(rectangular_eos.eps_max)
     || !isfinite(rectangular_eos.entropy_min) || !isfinite(rectangular_eos.entropy_max)
  ...
  if(error != ghl_success || !isfinite(simple_zero_eos.eps_min)
     || !isfinite(simple_zero_eos.entropy_min)
     || simple_zero_eos.eps_min != 0.0 || simple_zero_eos.entropy_min != 0.0
     || !isinf(simple_zero_eos.eps_max) || simple_zero_eos.eps_max < 0.0
     || !isinf(simple_zero_eos.entropy_max) || simple_zero_eos.entropy_max < 0.0)
  ...
  error = ghl_initialize_hybrid_eos_functions_and_params(
      rho_b_min, 0.0, 100.0, neos, rho_ppoly, Gamma_ppoly,
      k_ppoly0, Gamma_th, &hybrid_zero_eos);
  ...
  error = ghl_initialize_hybrid_eos_functions_and_params(
      rho_b_min, -1.0, 100.0, neos, rho_ppoly, Gamma_ppoly,
      k_ppoly0, Gamma_th, &hybrid_default_zero_eos);
  ...
  for(int eos_type = 0; eos_type < 2; eos_type++) {
    ...
    bool check1 = (!isfinite(prims.rho) || !isfinite(prims.press)
                || !isfinite(prims.vU[0]) || !isfinite(prims.vU[1]) || !isfinite(prims.vU[2])
                || !isfinite(prims.eps) || !isfinite(prims.entropy)
                || fabs(prims.rho     - eos.rho_atm) > 1e-50
    ...
    bool check2 = false;
    if(eos_type ==2)
      check2 = (!isfinite(prims.Y_e) || !isfinite(prims.temperature)
  ...
  const bool check_inverse = (!isfinite(vec_inv[0]) || !isfinite(vec_inv[1])
                           || !isfinite(vec_inv[2]) || !isfinite(vec_inv[3])
                           || relative_error(vec_inv[0], 10.0) > rel_tol
  ...
  if(!isfinite(new_metric.lapse)
  || !isfinite(new_metric.betaU[0]) || !isfinite(new_metric.betaU[1]) || !isfinite(new_metric.betaU[2])
