...
static inline bool checked_product(
      const double a,
      const double b,
      double *restrict result) {
  if(!isfinite(a) || !isfinite(b)
     || (a != 0.0 && fabs(b) > DBL_MAX/fabs(a)))
    return false;
  *result = a*b;
...
static inline void checked_multiply_divide(
  ...
  if(!checked_product(a, b, &product)
    ...
    ghl_error("Invalid ET Legacy %s arithmetic at sample %d, direction %d\n",
              quantity, current_sample, current_direction);
}
...
static inline ghl_error_codes_t compute_h_and_cs2(const ghl_eos_parameters *restrict eos,
  ...
  if(!checked_product(prims->rho, prims->vU[2], &rho_vz)
    ...
    ghl_error("Invalid ET Legacy sound-speed arithmetic at sample %d, direction %d\n",
              current_sample, current_direction);
  return ghl_success;
}

static inline ghl_error_codes_t compute_h(
      const ghl_eos_parameters *restrict eos,
      ghl_primitive_quantities *restrict prims,
      double *restrict h) {
  (void)eos;
  checked_multiply_divide(
        prims->press, prims->vU[0], prims->vU[2], "enthalpy", h);
  return ghl_success;
}
...
static inline double inverse_spacing_for_direction(
  ...
  switch(flux_dirn) {
    ...
    case 1: return invdy;
    case 2: return invdz;
    default: ghl_error("Invalid flux direction %d\n", flux_dirn);
  }
  return 0.0;
}
...
int main(int argc, char **argv) {
  ...
  ghl_error_codes_t (*calculate_characteristic_speed)(ghl_primitive_quantities *restrict, ghl_primitive_quantities *restrict,
    ...
    for(int k=ghostzone; k<dirlength-(ghostzone-1); k++)
      for(int j=ghostzone; j<dirlength-(ghostzone-1); j++)
        for(int i=ghostzone; i<dirlength-(ghostzone-1); i++) {
          ...
          checked_multiply_divide(rho_r[index], Bx_r[index], vy_r[index], "right u0", &prims_r.u0);
          checked_multiply_divide(rho_l[index], Bx_l[index], vy_l[index], "left u0", &prims_l.u0);

          double cmin, cmax;
          ghl_error_codes_t error = calculate_characteristic_speed(
                &prims_r, &prims_l, &eos,
  ...
  for(int k=ghostzone; k<dirlength-ghostzone; k++)
    for(int j=ghostzone; j<dirlength-ghostzone; j++)
      for(int i=ghostzone; i<dirlength-ghostzone; i++) {
        ...
        current_sample = index;
        current_direction = 3;
        checked_multiply_divide(rho[index], Bx[index], vy[index], "source u0", &prims.u0);

        ghl_conservative_quantities cons_sources;
        const ghl_error_codes_t error = ghl_calculate_source_terms(
            &eos, &prims, &metric,
