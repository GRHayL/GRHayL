...
#define str_equals(str1, str2) !strcmp(str1, str2)

int main(int argc, char **argv) {

  if( argc != 4 )
    grhayl_error("Correct usage is %s <EOS table path> <Con2Prim routine> <test type (1=rho vs T; 2=Pmag vs W)>\n");
  ...
  char outfile[256];
  con2prim_method_t main_routine=-1;
  if( str_equals(argv[2], "Palenzuela1D") ) {
    grhayl_info("Selected Con2Prim routine: Palenzuela1D\n");
    main_routine = Palenzuela1D;
    sprintf(outfile, "palenzuela_energy_%s.asc", outfile_suffix);
  }
  else if( str_equals(argv[2], "Palenzuela1D_entropy") ) {
    grhayl_info("Selected Con2Prim routine: Palenzuela1D_entropy\n");
    main_routine = Palenzuela1D_entropy;
    sprintf(outfile, "palenzuela_entropy_energy_%s.asc", outfile_suffix);
  }
  else if( str_equals(argv[2], "Newman1D") ) {
    grhayl_info("Selected Con2Prim routine: Newman1D\n");
    main_routine = Newman1D;
    sprintf(outfile, "newman_energy_%s.asc", outfile_suffix);
  }
  else if( str_equals(argv[2], "Newman1D_entropy") ) {
    grhayl_info("Selected Con2Prim routine: Newman1D_entropy\n");
    main_routine = Newman1D_entropy;
    sprintf(outfile, "newman_entropy_%s.asc", outfile_suffix);
  }
  else
    grhayl_error("Unsupported Con2Prim routine: %s\n", argv[2]);
