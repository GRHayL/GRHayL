 ...
 * finite/status validation, and the baseline/response comparator.  This adapter
 ...
 * source_a1_a2_rate_normalized_v1 policy uses the trusted source perturbation
 * as its response envelope.  Ordinary replay evaluates the current API only
 * at the baseline input.  The six named zero-flux policy records remain
...
static inline void m1_thcm1_source_fixture_error(
  ...
  if(error != NULL && error_size > 0)
    snprintf(error, error_size, "%s", message != NULL ? message : "source fixture error");
}

static inline int m1_thcm1_source_fixture_species(
      const char *restrict name) {
  if(name == NULL) return -1;
  if(strcmp(name, "nue") == 0) return ghl_m1_neutrino_nue;
  if(strcmp(name, "anue") == 0) return ghl_m1_neutrino_anue;
  if(strcmp(name, "nux") == 0) return ghl_m1_neutrino_nux;
  return -1;
}
...
static inline int m1_thcm1_source_fixture_same_input(
  ...
  for(size_t i = 0; i < M1_THCM1_SOURCE_FIXTURE_INPUT_COUNT; ++i)
    if(baseline[i] != perturbed[i]) return 0;
  return 1;
}
...
static inline int m1_thcm1_source_fixture_build_metric(
  ...
  ghl_initialize_metric(
      input[M1_THCM1_SOURCE_LAPSE],
      input[M1_THCM1_SOURCE_SHIFT0], input[M1_THCM1_SOURCE_SHIFT1],
      input[M1_THCM1_SOURCE_SHIFT2],
...
static inline int m1_thcm1_source_fixture_build_rates(
  ...
  if(input == NULL || rates == NULL || species < 0 || species >= 3) return 0;
  *rates = (ghl_m1_neutrino_rates){0};
  ...
  for(int process = 0; process < ghl_m1_neutrino_pair_process_count; ++process) {
    rates->eta_N_pair[process] =
        input[M1_THCM1_SOURCE_ETA_N_PAIR0 + process];
    rates->eta_E_pair[process] =
        input[M1_THCM1_SOURCE_ETA_E_PAIR0 + process];
  }
  return 1;
}
...
static inline int m1_thcm1_source_fixture_local_policy_case(const char *case_id) {
  ...
  for(size_t i = 0; i < sizeof(prefixes)/sizeof(prefixes[0]); ++i) {
    ...
    if(strncmp(case_id, prefixes[i], length) == 0 &&
       m1_thcm1_source_fixture_species(case_id + length) >= 0) return 1;
  }
  return 0;
}
...
static inline int m1_thcm1_source_fixture_evaluate_one(
  ...
  if(!m1_thcm1_source_fixture_build_metric(input, &metric) ||
     ...
     ghl_m1_initialize(
         ...
         M1_THCM1_SOURCE_FD_EPSILON_ABS,
         M1_THCM1_SOURCE_NEWTON_MAX_ITERATIONS,
         M1_THCM1_SOURCE_NEWTON_TOLERANCE, &m1_params) != ghl_success) {
    m1_thcm1_source_fixture_error(error, error_size,
                                   "source fixture input initialization failed");
    return 0;
  }
  for(int i = 0; i < 3; ++i)
  ...
  if(!(metric.lapse > 0.0) || !isfinite(metric.lapse)) {
    m1_thcm1_source_fixture_error(error, error_size,
                                  "source fixture lapse is invalid");
    return 0;
  }
  ...
  if(!isfinite(v2) || v2 < 0.0 || v2 >= 1.0) {
    m1_thcm1_source_fixture_error(error, error_size,
                                  "source fixture velocity is invalid");
    return 0;
  }
  ...
  if(m1_thcm1_source_fixture_local_policy_case(record->case_id)) {
    ...
    if(state.F[0] != 0.0 || state.F[1] != 0.0 || state.F[2] != 0.0 ||
       ghl_m1_compute_closure_with_primitives(
         &m1_params, &metric, &prims, &radiation, &closure) != ghl_success ||
    ...
    if(!isfinite(trace) || fabsl(trace - state.E) >
  ...
  const ghl_error_codes_t source_error =
      ghl_m1_compute_neutrino_interaction_sources(
          ...
          &ef_sources, &number_source);
  if(source_error != ghl_success) {
    if(error != NULL && error_size > 0)
      snprintf(error, error_size, "source fixture %s runtime source failed (%d)",
               record->case_id != NULL ? record->case_id : "<unknown>",
               (int)source_error);
    return 0;
  }
  double matter_energy = NAN;
  double matter_momentum[3] = {NAN, NAN, NAN};
  const ghl_error_codes_t matter_error =
      ghl_m1_compute_neutrino_matter_coupling_sources(
          &metric, &ef_sources, &matter_energy, matter_momentum);
  if(matter_error != ghl_success) {
    if(error != NULL && error_size > 0)
      snprintf(error, error_size, "source fixture %s runtime matter coupling failed (%d)",
               record->case_id != NULL ? record->case_id : "<unknown>",
               (int)matter_error);
    return 0;
  }
...
static inline int m1_thcm1_source_fixture_normalization(
  ...
  const double n_scale = fmax(fabs(baseline[M1_THCM1_SOURCE_N]),
                              fabs(perturbed[M1_THCM1_SOURCE_N]));
  const double e_scale = fmax(fabs(baseline[M1_THCM1_SOURCE_E]),
                              fabs(perturbed[M1_THCM1_SOURCE_E]));
  ...
  const double rate_n = fmax(
      fabs(baseline[M1_THCM1_SOURCE_ETA_N]) +
          baseline_n_absorption,
      fabs(perturbed[M1_THCM1_SOURCE_ETA_N]) +
          perturbed_n_absorption);
  const double rate_e = fmax(
      fabs(baseline[M1_THCM1_SOURCE_ETA_E]) +
          baseline_e_absorption,
      fabs(perturbed[M1_THCM1_SOURCE_ETA_E]) +
          perturbed_e_absorption);
  ...
  for(size_t i = 1; i < 5; ++i) normalization[i] = e_normalization;
  for(size_t i = 5; i < 9; ++i) normalization[i] = volume * e_normalization;
  for(size_t i = 0; i < M1_THCM1_SOURCE_FIXTURE_OUTPUT_COUNT; ++i)
    if(!isfinite(normalization[i]) || !(normalization[i] > 0.0)) return 0;
  return 1;
}

/* Ordinary records reach the current source API once at the baseline input.
 * The six named zero-flux policy cases have local admissibility/exchange
 * checks, not reference agreement.  records_run counts only baseline/response
 * agreement records. */
static inline int m1_thcm1_run_instantaneous_source_fixtures(
  ...
  if(!m1_thcm1_fixture_load(
         ...
         M1_THCM1_SOURCE_FIXTURE_INPUT_COUNT,
         M1_THCM1_SOURCE_FIXTURE_OUTPUT_COUNT,
  ...
  for(size_t index = 0; index < collection.record_count; ++index) {
    ...
    snprintf(expected_pair_id, sizeof(expected_pair_id), "pair%zu:%s",
             index / 3, record->seed_id != NULL ? record->seed_id : "<missing>");
    if(species < 0 || strcmp(record->origin, "source_campaign") != 0 ||
       ...
       (record->sensitivity_count == 0 &&
        !m1_thcm1_source_fixture_same_input(record->baseline_input,
                                             record->perturbed_input))) {
      m1_thcm1_source_fixture_error(error, error_size,
                                    "invalid instantaneous source fixture metadata");
      m1_thcm1_fixture_free(&collection);
      return 0;
    }
    ...
    if(!m1_thcm1_source_fixture_normalization(
      ...
      m1_thcm1_source_fixture_error(error, error_size,
                                    "instantaneous source normalization failed");
      m1_thcm1_fixture_free(&collection);
      return 0;
    }
    for(size_t component = 0; component < M1_THCM1_SOURCE_FIXTURE_OUTPUT_COUNT; ++component)
    ...
    if(m1_thcm1_source_fixture_local_policy_case(record->case_id)) {
      ...
      if(!m1_thcm1_source_fixture_evaluate_one(
             record, record->perturbed_input, species, perturbed,
             error, error_size) ||
         !m1_thcm1_fixture_finite_vector(
  ...
  ghl_info("THC_M1 source fixtures: %zu named zero-flux policy pairs passed local admissibility/exchange checks\n",
           local_policy_cases);
  return 1;
}
