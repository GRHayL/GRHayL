...
enum {
  PROVIDER_RANDOM_CASES = 128,
  TABLE_RANDOM_CASES = 32
};
...
static double provider_rng_between(
      provider_rng *restrict rng, const double lower, const double upper) {
...
static void require_error(
  ...
  if(actual != expected)
    ghl_error("M1 rate-provider case %d: %s returned %d, expected %d\n",
...
static void provider_set_table_corners(
      ghl_eos_parameters *restrict eos, const int key,
      const double replacement, double saved[8]) {
  int corner = 0;
...
static bool write_provider_fixture_scalar_int(
      const hid_t file, const char *restrict name, const int value) {
  ...
  const bool ok = dataset >= 0 && H5Dwrite(
      dataset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &value) >= 0;
  if(dataset >= 0)
    H5Dclose(dataset);
  H5Sclose(space);
  return ok;
...
static bool write_provider_fixture_scalar_double(
      const hid_t file, const char *restrict name, const double value) {
  const hsize_t dimensions[1] = {1};
  const hid_t space = H5Screate_simple(1, dimensions, NULL);
  if(space < 0)
    return false;
  ...
  const bool ok = dataset >= 0 && H5Dwrite(
      dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &value) >= 0;
  if(dataset >= 0)
    H5Dclose(dataset);
  H5Sclose(space);
  return ok;
...
static bool write_provider_fixture_vector(
      const hid_t file, const char *restrict name,
      const double *restrict values, const int count) {
  ...
  const hid_t dataset = H5Dcreate2(
      file, name, H5T_NATIVE_DOUBLE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  const bool ok = dataset >= 0 && H5Dwrite(
      dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, values) >= 0;
  if(dataset >= 0)
    H5Dclose(dataset);
  H5Sclose(space);
  return ok;
...
static bool create_provider_fixture(char *restrict path, const size_t path_size) {
  ...
  for(int iy = 0; iy < PROVIDER_FIXTURE_NYE; ++iy) {
    for(int it = 0; it < PROVIDER_FIXTURE_NTEMP; ++it) {
      for(int ir = 0; ir < PROVIDER_FIXTURE_NRHO; ++ir) {
        const int index = ir + PROVIDER_FIXTURE_NRHO *
            (it + PROVIDER_FIXTURE_NTEMP * iy);
  ...
  if(file < 0)
    return false;
  bool ok = write_provider_fixture_scalar_int(file, "have_rel_cs2", 1) &&
      write_provider_fixture_scalar_int(file, "pointsrho", PROVIDER_FIXTURE_NRHO) &&
      write_provider_fixture_scalar_int(file, "pointstemp", PROVIDER_FIXTURE_NTEMP) &&
      ...
      write_provider_fixture_vector(file, "logtemp", logtemp, PROVIDER_FIXTURE_NTEMP) &&
...
static void validate_rate_bundle(
  ...
  for(int species = 0; species < ghl_m1_neutrino_species_count; ++species) {
    ...
    require_condition(rates[species].species == species,
                      "published rate species is not indexed consistently",
                      case_index);
    require_condition(isfinite(rates[species].mean_energy) &&
                      rates[species].mean_energy > 0.0,
                      "published mean energy is not positive and finite",
                      case_index);
  }
}
...
static void test_nrpyleakage_raw_kernel(void) {
  ...
  for(int invalid_species = 0;
      invalid_species < ghl_m1_nrpyleakage_species_count; ++invalid_species) {
    double invalid_eta[ghl_m1_nrpyleakage_species_count] = {
        eta[0], eta[1], eta[2]};
    ...
    require_error(
        ...
        "direct NRPyLeakage invalid degeneracy", 3074 + invalid_species);
    require_condition(
        memcmp(&staged, &untouched, sizeof(staged)) == 0,
        "invalid NRPyLeakage degeneracy changed output",
  ...
  require_error(
      ghl_m1_nrpyleakage_compute_raw_rates_from_thermo(
          &invalid_thermo, eta, &staged),
      ghl_error_m1_microphysics_failure,
      "direct NRPyLeakage invalid temperature", 3075);
  require_condition(
      memcmp(&staged, &untouched, sizeof(staged)) == 0,
...
static void test_nrpyleakage_rate_overflow_policy(void) {
  ...
  require_error(
      ghl_m1_nrpyleakage_compute_raw_rates_from_thermo(
          &thermo, eta, &strict_raw),
      ghl_error_m1_microphysics_failure,
      "strict NRPyLeakage rate-overflow rejection", 3110);
  require_condition(
...
static void test_nrpyleakage_kernel_representability_edges(void) {
  ...
  require_error(
      ghl_m1_nrpyleakage_compute_raw_rates_from_thermo(
          &thermo, eta, &staged),
      ghl_error_m1_microphysics_failure,
      "strict rejection of overflowing FD moment", 3120);
  require_condition(
      memcmp(&staged, &untouched, sizeof(staged)) == 0,
  ...
  require_error(
      ghl_m1_nrpyleakage_compute_raw_rates_from_thermo(
          &thermo, eta, &staged),
      ghl_error_m1_microphysics_failure,
      "strict rejection of nonpositive FD moment", 3120);
  require_condition(
      memcmp(&staged, &untouched, sizeof(staged)) == 0,
  ...
  require_error(
      ghl_m1_nrpyleakage_compute_raw_rates_from_thermo(
          &thermo, eta, &staged),
      ghl_error_m1_microphysics_failure,
      "strict rejection of nonfinite FD argument", 3121);
  require_condition(
      memcmp(&staged, &untouched, sizeof(staged)) == 0,
  ...
  require_error(
      ghl_m1_nrpyleakage_compute_raw_rates_from_thermo(
          &thermo, extreme_eta, &staged),
      ghl_error_m1_microphysics_failure,
      "strict rejection of underflowing FD factor", 3122);
  require_condition(
      memcmp(&staged, &untouched, sizeof(staged)) == 0,
...
static void compute_expected_reference_rates(
  ...
  const double thermal_number = rho /
      fmax(provider->baryon_mass_code, DBL_MIN);
  const int mask = provider->channel_mask;

  for(int species = 0; species < ghl_m1_neutrino_species_count; ++species) {
    const double lepton_weight = species == ghl_m1_neutrino_nue ? 1.0 :
        species == ghl_m1_neutrino_anue ? -1.0 : 0.0;
    ...
    const double mean_energy = fmax(
        provider->min_mean_energy,
        theta * (3.1514 + 0.25 * fabs(degeneracy)));
    ...
    expected[species].kappa_tr = expected[species].kappa_a_E +
        expected[species].kappa_s;
    expected[species].n_eq = fmax(0.0, n_eq);
    expected[species].J_eq = mean_energy * expected[species].n_eq;
    expected[species].eta_N = expected[species].kappa_a_N *
        expected[species].n_eq;
    expected[species].eta_E = expected[species].kappa_a_E *
        expected[species].J_eq;
    expected[species].kappa_a_N_cc = lepton_weight == 0.0 ? 0.0 :
        kappa_a_N_cc;
  ...
  const double process_scales[] = {
      provider->pair_scale * rho * theta * theta,
  ...
  for(int process = 0; process < ghl_m1_neutrino_pair_process_count; ++process) {
    ...
    for(int species = ghl_m1_neutrino_nue;
      ...
      expected[species].eta_N_pair[process] = kappa_N *
          electron_n_eq_geometric_mean;
      expected[species].eta_E_pair[process] = kappa_N *
          expected[species].mean_energy * expected[species].J_eq;
    }
  }
}
...
static void compare_reference_rates(
  ...
  for(int species = 0; species < ghl_m1_neutrino_species_count; ++species) {
    ...
    for(size_t i = 0; i < sizeof(actual_scalars) / sizeof(actual_scalars[0]); ++i)
      require_condition(fabs(actual_scalars[i] - expected_scalars[i]) <=
                        2.0e-11 * fmax(1.0, fabs(expected_scalars[i])),
                        "reference provider formula mismatch", case_index);
    for(int process = 0;
        process < ghl_m1_neutrino_pair_process_count; ++process)
      require_condition(fabs(actual[species].eta_N_pair[process] -
                             expected[species].eta_N_pair[process]) <=
                        2.0e-11 * fmax(1.0,
                            fabs(expected[species].eta_N_pair[process])) &&
...
static void initialize_sentinel_rates(
...
static void test_invalid_provider_contexts(
      const ghl_primitive_quantities *restrict prims) {
  static const char *const labels[] = {
      "backend below enum range", "backend above enum range",
      "unknown channel bit", "failure policy below enum range",
      "failure policy above enum range", "table policy below enum range",
      "table policy above enum range", "reference density conversion",
      "reference opacity conversion", "reference emissivity conversion",
      "nonpositive temperature conversion", "nonpositive baryon mass",
      "negative charged-current scale", "negative scattering scale",
      "negative pair scale", "negative bremsstrahlung scale",
      "negative plasmon scale", "nonpositive minimum mean energy",
      "nonpositive equilibrium recovery rate", "missing configured EOS"};
  ...
  for(int variant = 0; variant < variant_count; ++variant) {
    ...
    require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                  ghl_success, "invalid-context baseline initialization",
                  4000 + variant);
    switch(variant) {
      case 0:
      ...
      case 4:
        ...
        break;
      case 5:
        provider.table_bounds_policy =
            (ghl_neutrino_rate_table_bounds_policy_t)-1;
        break;
      case 6:
        provider.table_bounds_policy =
            (ghl_neutrino_rate_table_bounds_policy_t)2;
        break;
      case 7:
    ...
    require_error(error, ghl_error_m1_microphysics_failure, labels[variant],
                  4000 + variant);
    require_condition(memcmp(&cache, &cache_before, sizeof(cache)) == 0 &&
                      same_rate_bundle(rates, rates_before),
                      "invalid provider context was not transactional",
                      4000 + variant);
    require_condition(diagnostics.failures == 1 &&
                      diagnostics.last_error == error,
                      "invalid provider context diagnostics are incomplete",
                      4000 + variant);
  }
}

static void test_nonfinite_provider_contexts(
      const ghl_primitive_quantities *restrict prims) {
  static const char *const labels[] = {
      "nonfinite multiplicity", "nonfinite charged-current scale",
      "nonfinite scattering scale", "nonfinite pair scale",
      "nonfinite bremsstrahlung scale", "nonfinite plasmon scale",
      "nonfinite temperature conversion", "nonfinite baryon mass",
      "nonfinite equilibrium recovery rate", "nonfinite minimum mean energy"
  };

  for(int variant = 0;
      variant < (int)(sizeof(labels) / sizeof(labels[0])); ++variant) {
    ghl_neutrino_rate_provider_context provider;
    require_error(
        ghl_neutrino_rate_provider_initialize_default(&provider), ghl_success,
        "nonfinite-context baseline initialization", 4050 + variant);
    switch(variant) {
      case 0: provider.nu_x_multiplicity = NAN; break;
      case 1: provider.charged_current_scale = NAN; break;
      case 2: provider.scattering_scale = NAN; break;
      case 3: provider.pair_scale = NAN; break;
      case 4: provider.bremsstrahlung_scale = NAN; break;
      case 5: provider.plasmon_scale = NAN; break;
      case 6: provider.temperature_code_to_mev = NAN; break;
      case 7: provider.baryon_mass_code = NAN; break;
      case 8: provider.equilibrium_recovery_rate = NAN; break;
      case 9: provider.min_mean_energy = NAN; break;
      default: ghl_error("M1 rate-provider nonfinite-context test has an unknown variant\n");
    }
    ...
    require_condition(memcmp(&cache, &cache_before, sizeof(cache)) == 0 &&
                      same_rate_bundle(rates, rates_before),
                      "nonfinite provider context was not transactional",
                      4050 + variant);
    require_condition(diagnostics.failures == 1 &&
                      diagnostics.last_error == error,
                      "nonfinite provider context diagnostics are incomplete",
                      4050 + variant);
  }
}
...
static void test_invalid_primitive_keys(void) {
  ghl_neutrino_rate_provider_context provider;
  require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                ghl_success, "invalid-key provider initialization", 4100);
  ghl_neutrino_rate_provider_cache cache;
  ghl_neutrino_rate_provider_cache_initialize(&cache);
  ghl_neutrino_rate_provider_diagnostics diagnostics = {0};
  ...
  require_error(ghl_neutrino_rate_provider_compute_cell(
                    &provider, &cache, &diagnostics, NULL, &prims,
                    valid_rates),
  ...
  for(int case_index = 0; case_index < 5; ++case_index) {
    ...
    require_error(error, ghl_error_m1_microphysics_failure,
                  "invalid primitive key", 4101 + case_index);
    require_condition(memcmp(&cache, &cache_before, sizeof(cache)) == 0 &&
                      same_rate_bundle(rates, rates_before),
                      "invalid primitive key was not transactional",
                      4101 + case_index);
  }
}
...
static void test_temperature_recovery(void) {
  ghl_neutrino_rate_provider_context provider;
  require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                ghl_success, "temperature-recovery provider initialization",
                4200);
  ghl_primitive_quantities prims = {0};
  prims.rho = 0.8;
...
static void test_default_provider(provider_rng *restrict rng) {
  ghl_neutrino_rate_provider_context provider;
  require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                ghl_success, "default provider initialization", 0);
  require_error(ghl_neutrino_rate_provider_initialize_default(NULL),
                ghl_error_m1_null_pointer, "NULL default provider initialization",
                0);

  ghl_neutrino_rate_provider_cache cache;
  ghl_neutrino_rate_provider_cache_initialize(&cache);
  ghl_neutrino_rate_provider_cache_initialize(NULL);
  require_condition(!cache.thermo_valid && !cache.rates_valid,
  ...
  for(int case_index = 0; case_index < PROVIDER_RANDOM_CASES; ++case_index) {
    ...
    provider.channel_mask = channel_masks[case_index %
                                          (int)(sizeof(channel_masks) /
                                                sizeof(channel_masks[0]))];
    ...
    const ghl_error_codes_t error = ghl_neutrino_rate_provider_compute_cell(
        &provider, &cache, &diagnostics, NULL, &prims, rates);
    require_error(error, ghl_success, "random reference provider call",
                  case_index);
    validate_rate_bundle(rates, case_index);
    compare_reference_rates(&provider, &prims, rates, case_index);
    require_condition(diagnostics.active_channel_mask == provider.channel_mask,
  ...
  require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                ghl_success, "cache test provider initialization", 1000);
  ...
  require_condition(diagnostics.cache_misses == 1 && diagnostics.cache_hits == 0,
  ...
  require_condition(diagnostics.cache_hits == 1,
                    "repeated provider call did not hit the cache", 1001);
  require_condition(same_rate_bundle(first, second),
                    "cache hit changed the frozen rate bundle", 1001);

  prims.rho *= 1.01;
  error = ghl_neutrino_rate_provider_compute_cell(
      &provider, &cache, &diagnostics, NULL, &prims, second);
  require_error(error, ghl_success, "changed-key provider call", 1002);
  require_condition(diagnostics.cache_misses == 2,
                    "changed primitive key did not miss the cache", 1002);
  prims.rho = 1.25;
  prims.temperature *= 1.01;
  error = ghl_neutrino_rate_provider_compute_cell(
      &provider, &cache, &diagnostics, NULL, &prims, second);
  require_error(error, ghl_success, "changed-temperature provider call", 1005);
  require_condition(diagnostics.cache_misses == 3,
                    "changed temperature key did not miss the cache", 1005);
  prims.temperature = 0.8;
  prims.Y_e = 0.38;
  error = ghl_neutrino_rate_provider_compute_cell(
      &provider, &cache, &diagnostics, NULL, &prims, second);
  require_error(error, ghl_success, "changed-electron-fraction provider call", 1006);
  require_condition(diagnostics.cache_misses == 4,
  ...
  require_condition(provider.eos_generation != old_generation &&
                    diagnostics.cache_misses == 5,
...
static void test_provider_cache_provenance(void) {
  ...
  for(int variant = 0;
    ...
    ghl_neutrino_rate_provider_context provider;
    require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                  ghl_success, "cache-provenance provider initialization",
                  1100 + variant);
    ghl_neutrino_rate_provider_cache cache;
    ghl_neutrino_rate_provider_cache_initialize(&cache);
    ghl_neutrino_rate_provider_diagnostics diagnostics = {0};
    ...
    switch(variant) {
      case 0: provider.channel_mask = 0; break;
      case 1: provider.failure_policy = ghl_neutrino_rate_failure_transparent; break;
      case 2: provider.table_bounds_policy = ghl_neutrino_rate_table_bounds_clamp; break;
      case 3: provider.temperature_code_to_mev = 2.0; break;
      case 4: provider.baryon_mass_code = 2.0; break;
      case 5: provider.charged_current_scale = 2.0e-2; break;
      case 6: provider.scattering_scale = 1.0e-2; break;
      case 7: provider.pair_scale = 2.0e-4; break;
      case 8: provider.bremsstrahlung_scale = 2.0e-4; break;
      case 9: provider.plasmon_scale = 2.0e-5; break;
      case 10: provider.min_mean_energy = 2.0e-12; break;
      case 11: provider.equilibrium_recovery_rate = 2.0; break;
      default: ghl_error("M1 rate-provider cache-provenance test has an unknown variant\n");
    }
    ...
    require_condition(diagnostics.cache_hits == 0 &&
                      diagnostics.cache_misses == 2 &&
                      memcmp(&cache.provider_snapshot, &provider,
                             sizeof(provider)) == 0,
                      "provider context change incorrectly reused cache",
                      1100 + variant);
  }
}
...
static void test_provider_cache_snapshot_mismatches(void) {
  ...
  for(int variant = 0; variant < (int)(sizeof(labels) / sizeof(labels[0]));
    ...
    ghl_neutrino_rate_provider_context provider;
    require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                  ghl_success, "snapshot-mismatch provider initialization",
                  1120 + variant);
    ghl_neutrino_rate_provider_cache cache;
    ghl_neutrino_rate_provider_cache_initialize(&cache);
    ghl_neutrino_rate_provider_diagnostics diagnostics = {0};
    ...
    if(variant == 0)
      ...
      require_condition(cache.eos_snapshot == NULL,
                        "snapshot-mismatch seed unexpectedly retained EOS",
                        1120 + variant);
    }

    require_error(ghl_neutrino_rate_provider_compute_cell(
                      &provider, &cache, &diagnostics, eos_argument, &prims,
                      rates),
    ...
    require_condition(diagnostics.cache_hits == 0 &&
                      ...
                      "stale cache provenance was reused or not refreshed",
                      1120 + variant);
  }
}
...
static void test_provider_cache_same_rho_temperature_changed_ye(void) {
  ...
  ghl_neutrino_rate_provider_context provider;
  require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                ghl_success, "same-key provider initialization", 1130);
  ghl_neutrino_rate_provider_cache cache;
  ghl_neutrino_rate_provider_cache_initialize(&cache);
  ghl_neutrino_rate_provider_diagnostics diagnostics = {0};
  ghl_primitive_quantities prims = {0};
...
static void test_recovery_and_transactional_failures(void) {
  ghl_neutrino_rate_provider_context provider;
  require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                ghl_success, "recovery provider initialization", 2000);
  ghl_neutrino_rate_provider_cache cache;
  ghl_neutrino_rate_provider_cache_initialize(&cache);
  ghl_neutrino_rate_provider_diagnostics diagnostics = {0};
  ghl_primitive_quantities prims = {0};
  ...
  ghl_m1_neutrino_rates rates[ghl_m1_neutrino_species_count];
  initialize_sentinel_rates(rates);
  const ghl_error_codes_t initial_error =
  ...
  require_error(abort_error, ghl_error_m1_microphysics_failure,
                "abort-policy invalid primitive", 2001);
  require_condition(memcmp(&cache, &cache_before, sizeof(cache)) == 0,
                    "abort-policy failure changed the cache", 2001);
  require_condition(same_rate_bundle(abort_rates, rates_before),
                    "abort-policy failure changed output rates", 2001);
  require_condition(abort_diagnostics.failures == 1 &&
                    abort_diagnostics.last_error == abort_error,
  ...
  const ghl_error_codes_t transparent_error =
      ghl_neutrino_rate_provider_compute_cell(
          &provider, &transparent_cache, &transparent_diagnostics, NULL,
          &invalid_prims, transparent_rates);
  require_error(transparent_error, ghl_success, "transparent recovery", 2002);
  validate_rate_bundle(transparent_rates, 2002);
  require_condition(transparent_diagnostics.last_recovery ==
                        ghl_neutrino_rate_recovery_transparent &&
                    transparent_diagnostics.transparent_recoveries == 1 &&
                    !transparent_cache.thermo_valid &&
                    !transparent_cache.rates_valid,
                    "transparent recovery did not preserve its transaction",
                    2002);
  ...
  const ghl_error_codes_t equilibrium_error =
      ghl_neutrino_rate_provider_compute_cell(
  ...
  require_condition(equilibrium_diagnostics.last_recovery ==
   ...
   * diagnostics arm. */
  ghl_m1_neutrino_rates equilibrium_no_diagnostics[
      ghl_m1_neutrino_species_count];
  initialize_sentinel_rates(equilibrium_no_diagnostics);
  require_error(
      ghl_neutrino_rate_provider_compute_cell(
          &provider, NULL, NULL, NULL, &invalid_prims,
          equilibrium_no_diagnostics),
  ...
  require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                ghl_success, "hold-last provider initialization", 2004);
  ...
  const ghl_error_codes_t hold_seed_error =
      ghl_neutrino_rate_provider_compute_cell(
  ...
  const ghl_error_codes_t hold_error = ghl_neutrino_rate_provider_compute_cell(
      &provider, &hold_last_cache, &hold_last_diagnostics, NULL,
      &invalid_prims, hold_last_rates);
  require_error(hold_error, ghl_error_m1_microphysics_failure,
                "hold-last invalid primitive", 2005);
  require_condition(memcmp(&hold_last_cache, &hold_cache_before,
                           sizeof(hold_last_cache)) == 0 &&
  ...
  require_error(ghl_neutrino_rate_provider_initialize_default(&provider),
                ghl_success, "multiplicity provider initialization", 2006);
  ...
  const ghl_neutrino_rate_provider_cache bad_context_cache_before =
      bad_context_cache;
  const ghl_m1_neutrino_rates bad_context_rates_before[
      ghl_m1_neutrino_species_count] = {
          bad_context_rates[0], bad_context_rates[1], bad_context_rates[2]};
  const ghl_error_codes_t context_error =
      ghl_neutrino_rate_provider_compute_cell(
          &provider, &bad_context_cache, &bad_context_diagnostics, NULL, &prims,
          bad_context_rates);
  require_error(context_error, ghl_error_m1_microphysics_failure,
                "invalid multiplicity context", 2006);
  require_condition(memcmp(&bad_context_cache, &bad_context_cache_before,
                           sizeof(bad_context_cache)) == 0 &&
                    same_rate_bundle(bad_context_rates, bad_context_rates_before),
                    "invalid context changed transactional outputs", 2006);

  require_error(ghl_neutrino_rate_provider_compute_cell(
                    NULL, NULL, NULL, NULL, &prims, rates),
...
#ifdef GHL_DISABLE_HDF5
  require_error(production_init, ghl_error_used_disabled_hdf5,
                "disabled-HDF5 production provider initialization", 2010);
  ghl_neutrino_rate_provider_context disabled_backend;
  require_error(
      ghl_neutrino_rate_provider_initialize_default(&disabled_backend),
      ghl_success, "disabled-HDF5 backend-gate initialization", 2012);
  ...
  const ghl_m1_neutrino_rates disabled_rates_before[
      ghl_m1_neutrino_species_count] = {
          disabled_rates[0], disabled_rates[1], disabled_rates[2]};
  ghl_neutrino_rate_provider_diagnostics disabled_diagnostics = {0};
  require_error(
      ghl_neutrino_rate_provider_compute_cell(
          &disabled_backend, &disabled_cache, &disabled_diagnostics, NULL,
          &prims, disabled_rates),
      ghl_error_used_disabled_hdf5,
      "disabled-HDF5 NRPyLeakage backend gate", 2012);
  require_condition(
      memcmp(&disabled_cache, &disabled_cache_before,
             sizeof(disabled_cache)) == 0 &&
  ...
  ghl_m1_neutrino_rates disabled_tabulated_rates[
      ghl_m1_neutrino_species_count];
  initialize_sentinel_rates(disabled_tabulated_rates);
  const ghl_m1_neutrino_rates disabled_tabulated_before[
      ghl_m1_neutrino_species_count] = {
...
#else
  require_error(production_init, ghl_success,
                "HDF5 production provider initialization", 2010);
  require_condition(production.backend == ghl_neutrino_rate_backend_nrpyleakage &&
...
static void test_recovery_publication_and_post_thermo_failures(void) {
  ...
  require_error(ghl_neutrino_rate_provider_initialize_default(&hold_provider),
                ghl_success, "post-thermo hold-last initialization", 2021);
  ...
  require_error(ghl_neutrino_rate_provider_compute_cell(
                    &hold_provider, &hold_cache, &hold_diagnostics, NULL,
                    &prims, hold_rates),
  ...
  const ghl_m1_neutrino_rates hold_rates_before_failure[
      ghl_m1_neutrino_species_count] = {
  ...
  require_condition(memcmp(&hold_cache, &hold_cache_before,
                           sizeof(hold_cache)) == 0 &&
                    same_rate_bundle(hold_rates, hold_rates_before_failure),
                    "post-thermo failure changed transactional outputs", 2022);
  require_condition(hold_diagnostics.failures == 1 &&
                    hold_diagnostics.last_error == hold_error &&
                    hold_diagnostics.last_recovery ==
                        ghl_neutrino_rate_recovery_none &&
  ...
  reference_hold_provider.failure_policy =
  ...
  ghl_m1_neutrino_rates reference_hold_seed[
      ghl_m1_neutrino_species_count];
  require_error(
      ghl_neutrino_rate_provider_compute_cell(
  ...
  const ghl_neutrino_rate_provider_cache reference_hold_expected =
      reference_hold_cache;
  ghl_neutrino_rate_provider_diagnostics reference_hold_diagnostics = {0};
  ghl_m1_neutrino_rates reference_hold_rates[
      ghl_m1_neutrino_species_count];
  ...
  require_condition(
      memcmp(&reference_hold_cache, &reference_hold_expected,
             sizeof(reference_hold_cache)) == 0,
...
static void test_production_provider_context_validation(
  ...
  static const char *const labels[] = {
      "production NaN minimum energy", "production nonzero minimum energy",
      "production reference backend without table EOS", "production NULL EOS",
      "production non-tabulated EOS type", "production non-StellarCollapse table",
      "production density conversion", "production temperature conversion",
      "production opacity conversion", "production emissivity conversion",
      "production baryon-mass conversion", "production charged-current scale",
      "production scattering scale", "production pair scale",
      "production bremsstrahlung scale", "production plasmon scale"
  };
  const int variant_count = (int)(sizeof(labels) / sizeof(labels[0]));
  for(int variant = 0; variant < variant_count; ++variant) {
    ...
    switch(variant) {
      case 0: provider.min_mean_energy = NAN; break;
      case 1: provider.min_mean_energy = 1.0; break;
      case 2: provider.use_tabulated_eos = false; break;
      case 3: eos_argument = NULL; break;
      case 4: bad_eos.eos_type = ghl_eos_hybrid; break;
      case 5: bad_eos.table_type = ghl_eos_table_unknown; break;
      case 6: provider.rho_code_to_cgs *= 2.0; break;
      case 7: provider.temperature_code_to_mev = 2.0; break;
      case 8: provider.opacity_cgs_to_code *= 2.0; break;
      case 9: provider.emissivity_cgs_to_code *= 2.0; break;
      case 10: provider.baryon_mass_code *= 2.0; break;
      case 11: provider.charged_current_scale = 2.0; break;
      case 12: provider.scattering_scale = 2.0; break;
      case 13: provider.pair_scale = 2.0; break;
      case 14: provider.bremsstrahlung_scale = 2.0; break;
      case 15: provider.plasmon_scale = 2.0; break;
      default: ghl_error("M1 rate-provider production-context test has an unknown variant\n");
    }
...
static void test_table_provider(const char *restrict table_path) {
  ...
  const ghl_error_codes_t eos_error =
      ghl_initialize_tabulated_eos_functions_and_params(
          table_path, 1.0e-7, -1.0, -1.0, 0.5, -1.0, -1.0,
          2.0, -1.0, -1.0, &eos);
  require_error(eos_error, ghl_success, "table initialization", 3000);
  require_condition(isfinite(eos.table_rho_min) && eos.table_rho_min > 0.0 &&
                    eos.table_rho_min < eos.table_rho_max &&
                    isfinite(eos.table_T_min) && eos.table_T_min > 0.0 &&
                    eos.table_T_min < eos.table_T_max &&
  ...
  require_error(ghl_neutrino_rate_provider_initialize_nrpyleakage(&provider),
                ghl_success, "table-backed provider initialization", 3000);
  require_condition(provider.min_mean_energy == 0.0,
                    "production initializer selected a physical energy floor",
                    3000);
  ...
  test_production_provider_context_validation(
      &provider, &eos, &context_prims);
  ghl_neutrino_rate_provider_cache cache;
  ghl_neutrino_rate_provider_cache_initialize(&cache);
  ghl_neutrino_rate_provider_diagnostics diagnostics = {0};
  ghl_m1_neutrino_rates rates[ghl_m1_neutrino_species_count];
  provider_rng table_rng = {
      .state = UINT64_C(0x4d315f5441424c45)};
  ...
  for(int case_index = 0; case_index < TABLE_RANDOM_CASES; ++case_index) {
    ...
    const double temperature_fraction =
        provider_rng_between(&table_rng, 0.05, 0.95);
    const double ye_fraction = provider_rng_between(&table_rng, 0.05, 0.95);
    ghl_primitive_quantities prims = {0};
    prims.rho = exp(log(eos.table_rho_min) + rho_fraction *
                    (log(eos.table_rho_max) - log(eos.table_rho_min)));
    prims.temperature = exp(log(eos.table_T_min) + temperature_fraction *
                            (log(eos.table_T_max) - log(eos.table_T_min)));
    prims.Y_e = eos.table_Y_e_min + ye_fraction *
                (eos.table_Y_e_max - eos.table_Y_e_min);
    prims.eps = 1.0;
    const ghl_error_codes_t error = ghl_neutrino_rate_provider_compute_cell(
        &provider, &cache, &diagnostics, &eos, &prims, rates);
    require_error(error, ghl_success, "random table provider call", 3001 + case_index);
    validate_rate_bundle(rates, 3001 + case_index);
    require_condition(diagnostics.beta_kirchhoff_mismatch_valid[
                          ghl_m1_neutrino_nue] &&
                      diagnostics.beta_kirchhoff_mismatch_valid[
                          ghl_m1_neutrino_anue],
                      "table provider omitted beta Kirchhoff diagnostics",
                      3001 + case_index);
    for(int process = 0;
        process < ghl_m1_neutrino_pair_process_count; ++process) {
      require_condition(
          ...
          3001 + case_index);
      require_condition(fabs(
                            rates[ghl_m1_neutrino_nue].eta_N_pair[process] -
                            rates[ghl_m1_neutrino_anue].eta_N_pair[process]) <=
                        2.0e-12 * fmax(1.0, fmax(
                            fabs(rates[ghl_m1_neutrino_nue].eta_N_pair[process]),
                            fabs(rates[ghl_m1_neutrino_anue].eta_N_pair[process]))),
                        "table provider broke shared electron number emissivity",
                        3001 + case_index);
      require_condition(rates[ghl_m1_neutrino_nux].eta_N_pair[process] == 0.0 &&
                        rates[ghl_m1_neutrino_nux].eta_E_pair[process] == 0.0,
                        "table provider populated nu_x process arrays",
                        3001 + case_index);
    }
  }
  ...
  provider_set_table_corners(
      &eos, NRPyEOS_mu_e_key, NAN, saved_hold_mu_e_corners);
  ghl_neutrino_rate_provider_diagnostics hold_diagnostics = {0};
  ghl_m1_neutrino_rates held_rates[ghl_m1_neutrino_species_count];
  initialize_sentinel_rates(held_rates);
  require_error(ghl_neutrino_rate_provider_compute_cell(
                    &provider, &cache, &hold_diagnostics, &eos,
                    &cached_prims, held_rates),
                ghl_success, "pre-cache hold-last recovery", 3160);
  require_condition(
      same_rate_bundle(held_rates, cache.rates) &&
      hold_diagnostics.failures == 1 &&
  ...
  require_condition(
      same_rate_bundle(held_rates, cache.rates) &&
      same_rate_bundle(held_rates, first),
      "pre-cache hold-last without diagnostics changed rates", 3161);
  provider_restore_table_corners(
      &eos, NRPyEOS_mu_e_key, saved_hold_mu_e_corners);
  ...
  require_error(
      NRPyEOS_eps_from_rho_Ye_T(
          &eos, cached_prims.rho, cached_prims.Y_e,
          cached_prims.temperature, &recovery_eps),
  ...
  require_error(
      ghl_neutrino_rate_provider_compute_cell(
          &provider, &cache, &diagnostics, &eos,
          &recovered_temperature, rates),
      ghl_success, "table temperature recovery", 3044);
  validate_rate_bundle(rates, 3044);
  require_condition(cache.thermo_valid && isfinite(cache.thermo_T) &&
                    cache.thermo_T > 0.0,
                    "table temperature recovery did not publish thermodynamics",
                    3044);
  ...
  const ghl_m1_neutrino_rates fallback_rates_before[
      ghl_m1_neutrino_species_count] = {rates[0], rates[1], rates[2]};
  diagnostics = (ghl_neutrino_rate_provider_diagnostics){0};
  const ghl_error_codes_t fallback_error =
      ghl_neutrino_rate_provider_compute_cell(
          &provider, &cache, &diagnostics, &fallback_bounds,
          &cached_prims, rates);
  require_error(fallback_error, ghl_error_table_max_rho,
                "legacy table-bound fallback", 3045);
  require_condition(
  ...
  require_error(
      ghl_neutrino_rate_provider_compute_cell(
          &provider, &cache, &diagnostics, &malformed_bounds,
  ...
  for(int bound = 0; bound < 3; ++bound) {
    ...
    require_error(bounds_error, ghl_error_m1_microphysics_failure,
                  "abort-policy table bounds", 3042 + bound);
    require_condition(memcmp(&cache, &cache_before, sizeof(cache)) == 0 &&
                      same_rate_bundle(unchanged, first),
                      "abort-policy table bounds changed outputs", 3042 + bound);
    require_condition(diagnostics.table_bound_hits == 1,
                      bound_names[bound], 3042 + bound);
  }

  const double above_bounds[3] = {
      2.0 * eos.table_rho_max,
      2.0 * eos.table_T_max,
      0.5 * (eos.table_Y_e_max + 1.0)};
  require_condition(eos.table_Y_e_max < 1.0,
                    "generated table cannot exercise an upper Ye bound", 3047);
  for(int bound = 0; bound < 3; ++bound) {
    ghl_primitive_quantities out_of_bounds = cached_prims;
    if(bound == 0)
    ...
    const ghl_error_codes_t bounds_error =
        ghl_neutrino_rate_provider_compute_cell(
            &provider, &cache, &diagnostics, &eos, &out_of_bounds, unchanged);
    require_error(bounds_error, ghl_error_m1_microphysics_failure,
                  "upper-bound table input", 3047 + bound);
    require_condition(memcmp(&cache, &cache_before, sizeof(cache)) == 0 &&
                      same_rate_bundle(unchanged, first),
                      "upper-bound table input changed outputs", 3047 + bound);
    require_condition(diagnostics.table_bound_hits == 1,
                      bound_names[bound], 3047 + bound);
  }
  ...
  require_condition(diagnostics.table_bound_hits > 0 &&
...
int main(int argc, char **argv) {
  if(argc > 2)
    ghl_error("Usage: %s [StellarCollapse EOS table path|--generated-fixture]\n",
              argv[0]);
  ...
  ghl_info("M1 rate-provider randomized/property tests passed (%d reference cases%s)\n",
