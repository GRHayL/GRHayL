...
static inline int m1_thcm1_transport_prepare_volume_weighted(
  ...
  if(cell_volumes == NULL || state_stencil == NULL ||
     physical_flux_L == NULL || physical_flux_R == NULL ||
     weighted_state == NULL || weighted_flux_L == NULL ||
     weighted_flux_R == NULL) {
    m1_thcm1_fixture_set_error(error, error_size,
                               "null volume-preparation argument");
    return 0;
  }
  if(!isfinite(face_volume) || !(face_volume > 0.0)) {
    m1_thcm1_fixture_set_error(error, error_size,
                               "face volume must be finite and positive");
    return 0;
  }
  for(int cell = 0; cell < 4; ++cell) {
    ...
    for(int component = 0;
      ...
      if(!isfinite(state_stencil[cell][component])) {
        m1_thcm1_fixture_set_error(error, error_size,
                                   "transport state must be finite");
        return 0;
      }
  }
  for(int component = 0;
      component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component)
    if(!isfinite(physical_flux_L[component]) ||
       !isfinite(physical_flux_R[component])) {
      m1_thcm1_fixture_set_error(error, error_size,
                                 "transport physical flux must be finite");
      return 0;
    }
  ...
  for(int cell = 0; cell < 4; ++cell)
    for(int component = 0;
      ...
      if(!isfinite(candidate_state[cell][component])) {
        m1_thcm1_fixture_set_error(error, error_size,
                                   "weighted stencil state is not finite");
        return 0;
      }
  for(int component = 0;
      component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
    ...
    if(!isfinite(candidate_flux_L[component]) ||
      ...
      m1_thcm1_fixture_set_error(error, error_size,
                                 "weighted physical flux is not finite");
      return 0;
    }
  }
...
static inline int m1_thcm1_transport_compute_weighted_normalization(
  ...
  for(int cell = 0; cell < 4; ++cell) {
    ...
    if(!isfinite(volume) || !(volume > 0.0)) return 0;
  }
  for(int component = 0;
      component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
    ...
    if(!isfinite(flux_L) || !isfinite(flux_R) ||
       !isfinite(state_L) || !isfinite(state_C))
      return 0;
    const double scale = fmax(
        ...
        fmax(speed * fabs(input[M1_THCM1_TRANSPORT_CELL_VOLUME_START] * state_L),
             speed * fabs(input[M1_THCM1_TRANSPORT_CELL_VOLUME_START + 1] *
                          state_C)));
...
/* Explicit two-state diagnostic helper.  Standard fixture replay uses the
 * baseline/response comparator below; this helper remains for tests of the
 * stronger current-baseline/current-perturbed campaign contract.  The
...
static inline int m1_thcm1_transport_compare_paired(
      ...
      char *restrict error,
      const size_t error_size) {
  if(record == NULL || computed_normalization == NULL ||
     computed_baseline == NULL || computed_perturbed == NULL ||
     record->normalization == NULL || record->baseline_output == NULL ||
     record->perturbed_output == NULL || record->output_count == 0) {
    m1_thcm1_fixture_set_error(error, error_size,
                               "invalid transport paired comparison argument");
    return 0;
  }
  if(report != NULL)
    *report = (m1_thcm1_fixture_comparison_report){
        .max_scaled_error = 0.0, .component = 0, .gate = "baseline"};
  const double *expected[2] = {record->baseline_output,
                               record->perturbed_output};
  ...
  for(size_t component = 0; component < record->output_count; ++component) {
    ...
    for(int gate = 0; gate < 3; ++gate) {
      const double actual = gate < 2
          ? computed[gate][component]
      ...
      double denominator = fmax(1.0e-300,
          fmax(fabs(actual), fabs(reference)));
      ...
      if(gate == 2) {
        ...
        const double baseline_scale = fmax(1.0e-300,
            fmax(fabs(computed[0][component]), fabs(expected[0][component])));
        const double perturbed_scale = fmax(1.0e-300,
      ...
      if(!m1_thcm1_fixture_compare_one(actual, reference, denominator,
                                       bound, 0.0, 0.0,
                                       &scaled_error)) {
        if(report != NULL) {
          report->max_scaled_error = fmax(report->max_scaled_error,
                                          scaled_error);
        ...
        if(error != NULL && error_size > 0)
          snprintf(error, error_size,
                   "transport %s component %zu %s comparison failed",
