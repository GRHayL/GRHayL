...
int main(int argc, char **argv) {
  ...
  for(int index=0; index<arraylength; index++) {
    ...
    /* The legacy fixture was generated by a limiter that snapped every state
     * in its trigger band to W_max, even when that increased the velocity.
     * The current limiter intentionally rejects such upward rescaling.  For
     * those rows, analytically undo the legacy radial utilde rescaling before
     * comparison so density, velocity direction, and perturbation bars remain
     * checked. */
    bool legacy_speed_limit_boundary = false;
    double W = 0.0;
    double W_trusted = 0.0;
    if(cons.rho > 0.0) {
      W = metric_adm.lapse*prims.u0;
      W_trusted = cons_undens.rho/prims_trusted.rho;
      legacy_speed_limit_boundary =
            !diagnostics.speed_limited
         && W <= params.max_Lorentz_factor
         && W - 1.0 > 0.9999999*(params.max_Lorentz_factor - 1.0)
         && fabs(W_trusted - params.max_Lorentz_factor)
              <= 512.0*DBL_EPSILON*params.max_Lorentz_factor;
    }

    ghl_primitive_quantities prims_expected = prims_trusted;
    ghl_primitive_quantities prims_expected_pert = prims_pert;
    if(legacy_speed_limit_boundary) {
      prims_expected.rho = cons_undens.rho/W;
      prims_expected_pert.rho *= W_trusted/W;

      const double velocity_factor = W_trusted/W
            * sqrt((W*W - 1.0)/(W_trusted*W_trusted - 1.0));
      for(int i = 0; i < 3; ++i) {
        prims_expected.vU[i] = velocity_factor
              * (prims_trusted.vU[i] + metric_adm.betaU[i]) - metric_adm.betaU[i];
        prims_expected_pert.vU[i] = velocity_factor
              * (prims_pert.vU[i] + metric_adm.betaU[i]) - metric_adm.betaU[i];
      }
    }

    if( ghl_pert_test_fail(prims_expected.rho, prims.rho, prims_expected_pert.rho) )
      ghl_error("Test unit_test_hybrid_Noble2D has failed for variable rho.\n"
                   "  rho trusted %.14e computed %.14e perturbed %.14e\n"
                   "  rel.err. %.14e %.14e\n", prims_expected.rho, prims.rho, prims_expected_pert.rho,
                                               relative_error(prims_expected.rho, prims.rho),
                                               relative_error(prims_expected.rho, prims_expected_pert.rho));
    ...
    if( ghl_pert_test_fail(prims_expected.vU[0], prims.vU[0], prims_expected_pert.vU[0]) )
      ghl_error("Test unit_test_hybrid_Noble2D has failed for variable vx.\n"
                   "  vx trusted %.14e computed %.14e perturbed %.14e\n"
                   "  rel.err. %.14e %.14e\n", prims_expected.vU[0], prims.vU[0], prims_expected_pert.vU[0],
                                               relative_error(prims_expected.vU[0], prims.vU[0]),
                                               relative_error(prims_expected.vU[0], prims_expected_pert.vU[0]));

    if( ghl_pert_test_fail(prims_expected.vU[1], prims.vU[1], prims_expected_pert.vU[1]) )
      ghl_error("Test unit_test_hybrid_Noble2D has failed for variable vy.\n"
                   "  vy trusted %.14e computed %.14e perturbed %.14e\n"
                   "  rel.err. %.14e %.14e\n", prims_expected.vU[1], prims.vU[1], prims_expected_pert.vU[1],
                                               relative_error(prims_expected.vU[1], prims.vU[1]),
                                               relative_error(prims_expected.vU[1], prims_expected_pert.vU[1]));

    if( ghl_pert_test_fail(prims_expected.vU[2], prims.vU[2], prims_expected_pert.vU[2]) )
      ghl_error("Test unit_test_hybrid_Noble2D has failed for variable vz.\n"
                   "  vz trusted %.14e computed %.14e perturbed %.14e\n"
                   "  rel.err. %.14e %.14e\n", prims_expected.vU[2], prims.vU[2], prims_expected_pert.vU[2],
                                               relative_error(prims_expected.vU[2], prims.vU[2]),
                                               relative_error(prims_expected.vU[2], prims_expected_pert.vU[2]));
  }
