...
static double expected_limiter(
  ...
  *sawtooth = false;
  ...
  if(opposite_nonzero_sign(dup, duc) && opposite_nonzero_sign(dum, duc)) {
    *sawtooth = true;
  }
  return 0.0;
}
...
static double
expected_opacity(const double kappa_face, const double delta, const double mindiss) {
...
static void check_full_four_point_operator(ghl_m1_parameters *restrict params) {
  ...
  for(int case_index = 0; case_index < 128; ++case_index) {
    ...
    check_close(
          diagnostics.opacity_suppression, opacity,
          "four-point opacity diagnostic mismatch");
    check_close(
          diagnostics.face_speed, face_speed,
    ...
    for(int component = 0; component < ghl_m1_neutrino_transport_component_count;
      ...
      const double expected
            = metric.sqrt_detgamma
              * (flux_high - dissipation * (1.0 - phi) * (flux_high - flux_low));
      check_close(flux_tilde[component], expected, "four-point transport flux mismatch");
      check_close(
            diagnostics.phi[component], phi, "four-point limiter diagnostic mismatch");
    ...
    for(int component = 0; component < ghl_m1_neutrino_transport_component_count;
        ++component) {
      check_close(
            no_diagnostic_flux[component], flux_tilde[component],
...
static void check_volume_preparation_adapter(void) {
  ...
  if(!m1_thcm1_transport_prepare_volume_weighted(
           ...
           weighted_state, weighted_flux_L, weighted_flux_R, error, sizeof(error))) {
    fail_test("valid volume preparation was rejected");
  }
  for(int cell = 0; cell < 4; ++cell) {
    for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
      check_close(
            weighted_state[cell][component], cell_volumes[cell] * state[cell][component],
            "weighted stencil state mismatch");
    }
  }
  for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
    check_close(
          weighted_flux_L[component], face_volume * physical_flux_L[component],
          "weighted left face flux mismatch");
    check_close(
          weighted_flux_R[component], face_volume * physical_flux_R[component],
  ...
  if(m1_thcm1_transport_prepare_volume_weighted(
           cell_volumes, NAN, state, physical_flux_L, physical_flux_R, weighted_state,
           weighted_flux_L, weighted_flux_R, error, sizeof(error))) {
  ...
  const double overflow_volumes[4] = { DBL_MAX, 1.0, 1.0, 1.0 };
  if(m1_thcm1_transport_prepare_volume_weighted(
           overflow_volumes, 1.0, overflow_state, physical_flux_L, physical_flux_R,
           weighted_state, weighted_flux_L, weighted_flux_R, error, sizeof(error))) {
...
static void check_prepared_transport_local_contract(void) {
  ...
  if(ghl_m1_initialize(1.0e-8, 1.0e-12, 1.0e-8, 1.0e-6, 1.0e-10, 100, 1.0e-10, &params)
     != ghl_success) {
  ...
  ghl_initialize_metric(
        1.0, 0.0, 0.0, 0.0, spatial_metric, 0.0, 0.0, spatial_metric, 0.0,
  ...
  for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
    check_close(
          prepared_flux[component], pointwise_flux[component],
  ...
  for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
    detected_double_densitization |= !m1_nearly_equal(
          double_densitized[component], prepared_flux[component], 1.0e-12, 1.0e-14);
  }
  ...
  for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
    diagnostics_unchanged
          &= invalid_diagnostics.phi[component] == diagnostics_before.phi[component]
  ...
  for(int changed_cell = 0; changed_cell < 4; ++changed_cell) {
    ...
    bool changed = false;
    for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
      changed |= !m1_nearly_equal(
            mutated_flux[component], varying_flux[component], 1.0e-12, 1.0e-14);
    }
    if(!changed) {
      ghl_error(
            "unit_test_m1_thcm1_blended_rusanov: unchanged cell volume "
            "%d in local mutation\n",
            changed_cell);
  ...
  for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT; ++component) {
    changed_face |= !m1_nearly_equal(
          mutated_face_flux[component], varying_flux[component], 1.0e-12, 1.0e-14);
  }
  if(!changed_face) {
...
static void
check_extreme_transport_arithmetic(const ghl_m1_parameters *restrict params) {
...
static const char *m1_thcm1_transport_direction(const char *case_id) {
  ...
  if(case_id != NULL && strstr(case_id, "__d1__") != NULL) {
    return "d1";
  }
  if(case_id != NULL && strstr(case_id, "__d2__") != NULL) {
    return "d2";
  }
  return "unknown";
}
...
static int m1_thcm1_load_variable_transport_fixture(
  ...
  return m1_thcm1_fixture_load(
        ...
        M1_THCM1_TRANSPORT_INPUT_COUNT, M1_THCM1_TRANSPORT_OUTPUT_COUNT, collection,
        error, error_size);
}
...
static void check_variable_transport_fixtures(const char *restrict fixture_dir) {
  ...
  for(size_t shard = 0; shard < shard_count; ++shard) {
    ...
    if(m1_thcm1_transport_file_exists(path)) {
      ...
      ghl_error(
            "unit_test_m1_thcm1_blended_rusanov: missing required "
  ...
  for(size_t shard = 0; shard < shard_count; ++shard) {
    ...
    if(!m1_thcm1_load_variable_transport_fixture(
      ...
      fail_test(error[0] != '\0' ? error : "variable-volume fixture load failed");
      free(path);
      for(size_t cleanup = 0; cleanup < shard_count; ++cleanup) {
        m1_thcm1_fixture_free(&collections[cleanup]);
      }
    ...
    free(path);
    if(strcmp(collections[shard].policy, "strict_relative_2e-12_propagated_response_v1")
       != 0) {
  ...
  for(size_t shard = 0; shard < shard_count; ++shard) {
    for(size_t index = 0; index < collections[shard].record_count; ++index) {
      ...
      double outputs[2][M1_THCM1_TRANSPORT_OUTPUT_COUNT] = { { 0.0 }, { 0.0 } };
      ...
      for(size_t field = 0; field < M1_THCM1_TRANSPORT_INPUT_COUNT; ++field) {
        changed_consumed_input |= inputs[0][field] != inputs[1][field];
      }
      for(int role = 0; role < 2; ++role) {
        const double *input = inputs[role];
        for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT;
            ++component) {
        ...
        double physical_flux_R[M1_THCM1_TRANSPORT_OUTPUT_COUNT];
        for(int cell = 0; cell < 4; ++cell) {
          cell_volumes[cell] = input[M1_THCM1_TRANSPORT_CELL_VOLUME_START + cell];
        }
        for(int cell = 0; cell < 4; ++cell) {
          for(int component = 0; component < M1_THCM1_TRANSPORT_OUTPUT_COUNT;
              ++component) {
        ...
        if(!m1_thcm1_transport_prepare_volume_weighted(
                 cell_volumes, face_volume, state, physical_flux_L, physical_flux_R,
                 weighted_states[role], weighted_fluxes_L[role], weighted_fluxes_R[role],
                 error, sizeof(preparation_error))
           || !m1_thcm1_transport_compute_weighted_normalization(
                 input, normalizations[role])) {
          status_ok = 0;
          if(failures == 0) {
            fail_test(
                  error[0] != '\0' ? error : "variable-volume input preparation failed");
          }
          break;
        }
      }
      if(status_ok) {
        for(int role = 0; role < 2; ++role) {
          ...
          if(ghl_m1_initialize(
                   1.0e-8, 1.0e-12, 1.0e-8, 1.0e-6, 1.0e-10, 100, 1.0e-10, &params)
             != ghl_success) {
            status_ok = 0;
            break;
          }
          /* Each role owns its limiter and dissipation controls. */
          params.minmod_theta = input[M1_THCM1_TRANSPORT_THETA_INDEX];
          params.mindiss = input[M1_THCM1_TRANSPORT_MINDISS_INDEX];
          ghl_m1_four_point_transport_diagnostics diagnostics = { 0 };
          if(m1_thcm1_call_volume_weighted_transport(
                   &params, weighted_states[role], weighted_fluxes_L[role],
      ...
      if(!status_ok
         || !m1_thcm1_transport_compare_paired(
               record, computed_normalization, outputs[0], outputs[1], &report,
               compare_error, sizeof(compare_error))) {
        ++failures;
        if(failures == 1) {
          ghl_error(
                "unit_test_m1_thcm1_blended_rusanov: variable-volume "
                "comparison failed direction=%s case=%s pair=%s "
                "component=%zu mode=%s: %s\n",
                m1_thcm1_transport_direction(record->case_id), record->case_id,
                record->pair_id, report.component,
                report.gate != NULL ? report.gate : "unknown",
                compare_error[0] != '\0' ? compare_error : "prepared evaluation failed");
        }
      }
    }
  }
  if(failures != 0) {
    ...
    ghl_info(
          "unit_test_m1_thcm1_blended_rusanov: %zu variable-volume "
          "THC_M1 pairs passed baseline/perturbed/response gates "
...
static void check_transport_fixtures(const char *restrict fixture_dir) {
  ...
  for(size_t shard = 0; shard < shard_count; ++shard) {
    ...
    if(!m1_thcm1_fixture_load(
             path, "neutrino_four_point_transport_flux", 50, 5, &collections[shard],
             error, sizeof(error))) {
  ...
  static const size_t expected_category_counts[6][4]
        = { { 19128, 19128, 19128, 0 },     { 16056, 20664, 20664, 0 },
            { 19128, 19128, 19128, 0 },     { 28692, 28692, 0, 0 },
            { 14346, 14346, 14346, 14346 }, { 18792, 18792, 19800, 0 } };
  ...
  for(size_t shard = 0; shard < shard_count; ++shard) {
    for(size_t index = 0; index < collections[shard].record_count; ++index) {
      ...
      for(int role = 0; role < 2; ++role) {
        ...
        if(!(input[0] > 0.0) || input[6] != input[0] || input[7] != input[0]
        ...
        if(input[0] != 1.0) {
          saw_nonunit_constant_volume = true;
        }
      }
      ...
      for(int role = 0; role < 2; ++role) {
        ...
        for(int component = 0; component < 5; ++component) {
          ...
          computed_normalization[component]
                = fmax(computed_normalization[component], fmax(1.0e-300, scale));
        }
      }

      double outputs[2][5] = { { 0.0 }, { 0.0 } };
      int status_ok = 1;
      for(int role = 0; role < 2; ++role) {
        const double *input = role == 0 ? base : pert;
        ghl_m1_parameters params = { 0 };
  ...
  if(failures != 0) {
    ...
    ghl_info(
          "unit_test_m1_thcm1_blended_rusanov: %zu strict transport pairs passed\n",
          total_records);
  }
  for(size_t shard = 0; shard < shard_count; ++shard) {
    m1_thcm1_fixture_free(&collections[shard]);
...
static void check_four_point_branch_boundaries(ghl_m1_parameters *restrict params) {
  double phi = 41.0;
  bool sawtooth = true;
  if(ghl_m1_compute_four_point_flux_limiter(NULL, 1.0, 1.0, 1.0, &phi, &sawtooth)
           != ghl_error_m1_null_pointer
     || ghl_m1_compute_four_point_flux_limiter(params, 1.0, 1.0, 1.0, NULL, &sawtooth)
              != ghl_error_m1_null_pointer
     || ghl_m1_compute_four_point_flux_limiter(params, 1.0, 1.0, 1.0, &phi, NULL)
              != ghl_error_m1_null_pointer) {
    fail_test("limiter NULL arguments were not rejected");
  }

  params->minmod_theta = NAN;
  if(ghl_m1_compute_four_point_flux_limiter(params, 1.0, 1.0, 1.0, &phi, &sawtooth)
     != ghl_error_m1_invalid_state) {
  ...
  /* theta == 0 is inside the documented [0, 2] range and selects the fully
   * dissipative low-order flux.  The overflowing-ratio case is the regression:
   * forming dum/duc first makes 0 * inf a NaN, and fmin(1.0, NaN) returns 1.0,
   * which silently published the undissipated flux under ghl_success. */
  params->minmod_theta = 0.0;
  if(ghl_m1_compute_four_point_flux_limiter(params, 1.0, 1.0, 1.0, &phi, &sawtooth)
           != ghl_success
     || !m1_nearly_equal(phi, 0.0, 0.0, 0.0) || sawtooth) {
    fail_test("zero limiter theta did not select the low-order flux");
  }
  ...
  }
  {
    ...
    if(ghl_m1_compute_four_point_blended_flux(
             10.0, 2.0, phi, sawtooth, 1.0, &zero_theta_flux)
             != ghl_success
       || !m1_nearly_equal(zero_theta_flux, 2.0, 0.0, 0.0)) {
      fail_test("zero limiter theta did not blend to the low-order flux");
    }
  }
  /* The same overflowing ratio must still saturate to phi = 1 for theta > 0. */
  params->minmod_theta = 1.0;
  if(ghl_m1_compute_four_point_flux_limiter(
           params, 1.0e308, 1.0e-308, 1.0e308, &phi, &sawtooth)
           != ghl_success
     || !m1_nearly_equal(phi, 1.0, 0.0, 0.0) || sawtooth) {
    fail_test("positive limiter theta lost its saturated monotone branch");
  }

  params->minmod_theta = 1.0;
  if(ghl_m1_compute_four_point_flux_limiter(params, NAN, 1.0, 1.0, &phi, &sawtooth)
           != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_flux_limiter(params, 1.0, NAN, 1.0, &phi, &sawtooth)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_flux_limiter(params, 1.0, 1.0, NAN, &phi, &sawtooth)
              != ghl_error_m1_invalid_state) {
  ...
  for(size_t i = 0; i < sizeof(signs) / sizeof(signs[0]); ++i) {
    for(size_t j = 0; j < sizeof(signs) / sizeof(signs[0]); ++j) {
      for(size_t k = 0; k < sizeof(signs) / sizeof(signs[0]); ++k) {
        const bool monotone
              = signs[i] == signs[j] && signs[j] == signs[k] && signs[j] != 0.0;
        const bool alternating
              = signs[i] == -signs[j] && signs[k] == -signs[j] && signs[j] != 0.0;
        if(ghl_m1_compute_four_point_flux_limiter(
                 params, signs[i], signs[j], signs[k], &phi, &sawtooth)
                 != ghl_success
  ...
  double A = 42.0;
  if(ghl_m1_compute_four_point_opacity_suppression(NULL, 1.0, 1.0, &A)
           != ghl_error_m1_null_pointer
     || ghl_m1_compute_four_point_opacity_suppression(params, 1.0, 1.0, NULL)
              != ghl_error_m1_null_pointer) {
    fail_test("opacity NULL arguments were not rejected");
  }
  if(ghl_m1_compute_four_point_opacity_suppression(params, NAN, 1.0, &A)
           != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_opacity_suppression(params, -1.0, 1.0, &A)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_opacity_suppression(params, 1.0, NAN, &A)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_opacity_suppression(params, 1.0, 0.0, &A)
              != ghl_error_m1_invalid_state) {
    fail_test("invalid opacity geometry was accepted");
  }
  params->mindiss = NAN;
  if(ghl_m1_compute_four_point_opacity_suppression(params, 1.0, 1.0, &A)
     != ghl_error_m1_invalid_state) {
  ...
  double blended = 43.0;
  if(ghl_m1_compute_four_point_blended_flux(NAN, 1.0, 0.5, false, 0.5, &blended)
           != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_blended_flux(1.0, NAN, 0.5, false, 0.5, &blended)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_blended_flux(1.0, 1.0, NAN, false, 0.5, &blended)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_blended_flux(1.0, 1.0, -0.1, false, 0.5, &blended)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_blended_flux(1.0, 1.0, 0.5, false, NAN, &blended)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_blended_flux(1.0, 1.0, 0.5, false, -0.1, &blended)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_blended_flux(1.0, 1.0, 0.5, false, 1.1, &blended)
              != ghl_error_m1_invalid_state
     || ghl_m1_compute_four_point_blended_flux(1.0, 1.0, 0.5, false, 0.5, NULL)
              != ghl_error_m1_null_pointer) {
  ...
  const double pointwise_flux_before[ghl_m1_neutrino_transport_component_count]
        = { 201.0, 202.0, 203.0, 204.0, 205.0 };
  if(ghl_m1_compute_neutrino_four_point_transport_flux(
...
static void check_subnormal_speed_dissipation(ghl_m1_parameters *restrict params) {
  double stencil[4][M1_THCM1_TRANSPORT_OUTPUT_COUNT] = { { 0.0 } };
  double flux_L[M1_THCM1_TRANSPORT_OUTPUT_COUNT] = { 0.0 };
  double flux_R[M1_THCM1_TRANSPORT_OUTPUT_COUNT] = { 0.0 };
...
int main(int argc, char **argv) {
  ...
  ghl_m1_parameters params = { 0 };
  if(ghl_m1_initialize(1.0e-10, 1.0e-12, 1.0e-8, 1.0e-6, 1.0e-12, 20, 1.0e-10, &params)
     != ghl_success) {
  ...
  if(ghl_m1_compute_four_point_flux_limiter(&params, 0.2, 0.5, 0.4, &phi, &sawtooth)
           != ghl_success
     || !m1_nearly_equal(phi, 0.4, 2.0e-13, 2.0e-14) || sawtooth) {
    fail_test("limited monotone limiter branch failed");
  }
  if(ghl_m1_compute_four_point_flux_limiter(&params, -1.0, 1.0, -0.5, &phi, &sawtooth)
           != ghl_success
     || phi != 0.0 || !sawtooth) {
    fail_test("sawtooth limiter branch failed");
  }
  if(ghl_m1_compute_four_point_flux_limiter(&params, 0.0, 1.0, 0.5, &phi, &sawtooth)
           != ghl_success
     || phi != 0.0 || sawtooth) {
  ...
  double A = NAN;
  if(ghl_m1_compute_four_point_opacity_suppression(&params, 0.5, 1.0, &A) != ghl_success
     || A != 1.0) {
    fail_test("thin opacity branch failed");
  }
  if(ghl_m1_compute_four_point_opacity_suppression(&params, 4.0, 1.0, &A) != ghl_success
     || A != 0.25) {
    fail_test("thick opacity branch failed");
  }
  params.mindiss = 0.2;
  if(ghl_m1_compute_four_point_opacity_suppression(&params, 100.0, 1.0, &A)
           != ghl_success
     || A != 0.2) {
  ...
  double blended = NAN;
  if(ghl_m1_compute_four_point_blended_flux(2.0, 1.0, 0.25, false, 0.4, &blended)
     != ghl_success) {
    fail_test("ordinary blend failed");
  }
  check_close(blended, 2.0 - 0.4 * 0.75, "ordinary blend formula failed");
  if(ghl_m1_compute_four_point_blended_flux(2.0, 1.0, 0.25, true, 0.4, &blended)
     != ghl_success) {
    fail_test("sawtooth blend failed");
  }
  check_close(blended, 2.0 - 0.75, "sawtooth blend formula failed");
  if(ghl_m1_compute_four_point_blended_flux(2.0, 1.0, 1.0, false, 0.0, &blended)
           != ghl_success
     || blended != 2.0) {
    fail_test("unit limiter blend failed");
  }
  if(ghl_m1_compute_four_point_blended_flux(1.0e308, -1.0e308, 1.0, false, 1.0, &blended)
           != ghl_success
     || blended != 1.0e308) {
    fail_test("large zero-weight blend rejected its high endpoint");
  }
  if(ghl_m1_compute_four_point_blended_flux(1.0e308, -1.0e308, 0.5, false, 0.5, &blended)
           != ghl_success
     || !isfinite(blended) || !m1_nearly_equal(blended, 5.0e307, 2.0e-15, 0.0)) {
  ...
  A = 42.0;
  if(ghl_m1_compute_four_point_opacity_suppression(&params, 1.0, 0.0, &A)
           != ghl_error_m1_invalid_state
     || A != 42.0) {
    fail_test("invalid opacity input was not transactional");
  }
  blended = 43.0;
  if(ghl_m1_compute_four_point_blended_flux(2.0, 1.0, 1.1, false, 0.5, &blended)
           != ghl_error_m1_invalid_state
     || blended != 43.0) {
