...
Use two independent seats to produce or review one GRHayL candidate. Keep initial work independent, then resolve findings against the frozen brief, claim-specific repository ground truth, and executable checks. Agreement is never stronger than evidence.
...
- Use exactly two fresh-context seats in every delegated phase. If two complete independent results are unavailable, stop rather than degrade silently.
- Root launches every wave. Neither root nor seats recursively invoke Dialectic or Trialectic; seats do not create subagents.
- Give both seats the same frozen brief, evidence boundaries, acceptance criteria, candidate, and validation record. Vary only their complementary roles. Do not reveal peer work before both initial results freeze.
...
- `review` — Default. Root prepares one complete candidate; two seats review it. Use when repository patterns and validators substantially determine the change.
- `design` — Two seats independently propose complete designs; root selects or synthesizes one coherent design, prepares a candidate, then uses a fresh pair for review. Use only when materially different valid designs remain after discovery.
- `dual-build` — Two seats independently build complete isolated candidates; root selects or synthesizes one coherent candidate, then uses a fresh pair for review. Use only when independent implementations provide a meaningful oracle or reduce material scientific or integration risk.
...
Limits: one proposal or dual-build wave; three candidate-review cycles; one review-and-decision wave per cycle; one root correction batch between consecutive cycles; one agent-recovery wave for mechanical or malformed output; and one retry for a diagnosed transient validation failure.
...
### Seat 2: API, integration, and release evidence

Check public headers, structs and enums, declarations versus definitions, manifests, `configure` selection, HDF5 guards, dispatch, installation surface, API/ABI compatibility, tests and fixtures, Doxygen and KB claims, CI selection, generated ownership, direct consumers, and GRHayLib impact. Prefer established GRHayL patterns and the smallest sufficient implementation. Reject unsupported abstraction and claims that outrun evidence.

For non-code work, use a first-principles domain verifier and an adversarial repository-evidence editor.
...
In `design`, each seat returns one complete design covering targets, interfaces, assumptions, existing components to reuse, risks, compatibility, generated ownership, and validation; end with `PROPOSAL COMPLETE`. In `dual-build`, each seat creates one complete candidate only in its assigned area and reports inspected inputs, rationale, reused machinery, validation, and limitations; end with `DRAFT COMPLETE`.

Freeze both results before comparison. Select one coherent proposal or candidate, or synthesize only compatible evidence-supported elements. Never average incompatible behavior or include an element merely to represent both seats. If none satisfies the brief, return `NO ACCEPTABLE CANDIDATE`. Retire proposal/build seats; use a fresh pair for candidate review.
