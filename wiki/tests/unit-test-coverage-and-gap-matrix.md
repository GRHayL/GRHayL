...
| Neutrinos | `unit_test_nrpyleakage_physics.c`, `unit_test_nrpyleakage_optically_thin_gas.c`, `unit_test_nrpyleakage_constant_density_sphere.c`, `unit_test_nrpyleakage_luminosities.c`, `nrpyleakage_main.h`, `unit_test_code_error.c` | self-contained physics regression; replay fixture test; helper-only; expected-failure; mixed HDF5/no-HDF5 coverage | [Neutrinos tests and fixtures](../gems/neutrinos/tests-and-fixtures.md), [HDF5 sample tables](hdf5-sample-tables.md) | The physics executable checks stable Fermi evaluation, blocking and shifted moment identities, invalid blocking input, and the asymmetric optical-depth stencil without HDF5. Table-backed replay uses shared key `1`; local key `0` generation is not normal CI replay. Invalid Fermi-Dirac key paths also route through expected-failure coverage. |
