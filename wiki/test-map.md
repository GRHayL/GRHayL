...
| `Unit_Tests/unit_test_nrpyleakage_constant_density_sphere.c` | `GRHayL/Neutrinos/NRPyLeakage/` | Constant-density sphere opacities and optical-depth iteration; see [tests and fixtures](gems/neutrinos/tests-and-fixtures.md). | CLI EOS table path plus `nrpyleakage_constant_density_sphere_{unperturbed,perturbed}.bin`. | Comparison results are enforced in a serial loop with grid-coordinate diagnostics. Flat stencils still leave unequal-direction face metrics untested. |
...
| `Unit_Tests/unit_test_nrpyleakage_physics.c` | `GRHayL/Neutrinos/NRPyLeakage/` | Table-free high-precision nucleon-blocking and beta-moment references, population bounds and normalization, threshold-moment orientation, spectral detailed balance for both beta channels, and strongly blocked common/mixed-underflow limits. | None. | Runs without HDF5; CI executes it once per compiler and OS neutrino matrix. Tolerances follow published Fermi-fit error bounds plus floating-point operation allowance. |
