...
| `Unit_Tests/unit_test_nrpyleakage_constant_density_sphere.c` | `GRHayL/Neutrinos/NRPyLeakage/` | Constant-density sphere opacities and optical-depth iteration; see [tests and fixtures](gems/neutrinos/tests-and-fixtures.md). | CLI EOS table path plus `nrpyleakage_constant_density_sphere_{unperturbed,perturbed}.bin`. | Comparison results are enforced in a serial loop with grid-coordinate diagnostics; the table-free physics test separately checks unequal face metrics and distinct neighbors. |
| `Unit_Tests/unit_test_nrpyleakage_luminosities.c` | `GRHayL/Neutrinos/NRPyLeakage/` | Fermi-Dirac branch checks and neutrino luminosity replay; see [tests and fixtures](gems/neutrinos/tests-and-fixtures.md). | CLI EOS table path plus `nrpyleakage_luminosities_{unperturbed,perturbed}.bin`. | Comparison results are enforced with row diagnostics; the generator writes matched row pairs. |
...
| `Unit_Tests/unit_test_nrpyleakage_physics.c` | `GRHayL/Neutrinos/NRPyLeakage/` | Table-free independent nucleon-blocking references; high-precision arithmetic beta-moment constants; population bounds and roundoff normalization; stable order-zero Fermi edge branches; threshold-moment orientation; spectral detailed balance for both beta channels; strongly blocked common/mixed-underflow limits; and a hand-computed asymmetric optical-depth stencil. | None. | Runs without HDF5; CI executes it once per compiler and OS neutrino matrix. Blocking tolerances include published fit errors and floating-point allowance. Beta constants guard implementation arithmetic; they are not an independent reaction-rate model. |
...
| `Unit_Tests/unit_test_tabulated_eos_compose.c` | `tools/compose/`, tabulated EOS, Con2Prim, Flux_Source, and NRPyLeakage runtimes | Reads every node/all 19 fields independently; checks loader units/order, enthalpy, relativistic sound speed, midpoint interpolation, `eps/P/S/h` inverses from distinct valid initial guesses, and six-output order/ranges; requires no-fallback Palenzuela recovery; compares characteristic speed, HLLE/entropy fluxes, and source terms with analytic goldens; contains owner-accepted combined-leakage regression goldens for the analytic and regularized SRO-141 inputs; and checks cleanup. These goldens detect code drift; the separate external qualification supplies the physics comparison. Python failure and branch coverage live under `Unit_Tests/compose/`. | CLI regularized StellarCollapse table path; focused Ubuntu GCC CI builds an asymmetric synthetic table, while full table 141 is external manual qualification data. | HDF5-only; converter output must pass with runtime sound-speed cleaning disabled. |
