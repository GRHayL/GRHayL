...
| `Unit_Tests/unit_test_code_error.c` | error handling across modules | Expected error-code paths for EOS, C2P, velocity limiting, tabulated EOS, and NRPyLeakage; see [Core tests and fixtures](core/tests-and-fixtures.md) and [EOS tests and fixtures](gems/eos/tests-and-fixtures.md). | Optional `SLy4_3335_rho391_temp163_ye66.h5`; `test.h5` path appears in source. | `run_tests.sh` expects process failure for keys `0` through `82`; HDF5-only keys are skipped in no-HDF5 builds. |
