...
Do not run the repository `Doxyfile` directly when only validating docs. Keep
all output in a unique temporary directory while preserving repo-root-relative
inputs:

```sh
(
  audit_tmp=$(mktemp -d "${TMPDIR:-/tmp}/grhayl-doxygen.XXXXXX") || exit 1
  cleanup_doxygen() {
    rm -rf -- "$audit_tmp"
  }
  trap cleanup_doxygen EXIT
  trap 'exit 129' HUP
  trap 'exit 130' INT
  trap 'exit 143' TERM
  ...
  } | doxygen - >"$audit_tmp/stdout" 2>"$audit_tmp/stderr" || doxygen_status=$?

  cat "$audit_tmp/stdout"
  cat "$audit_tmp/stderr" >&2
  exit "$doxygen_status"
)
```

`doxygen -` reads configuration from standard input. Review its exit status and
warning output; a successful generation does not prove API behavior. Compare
warning sets only with the same Doxygen version and configuration.
...
| `test/data_gen/` | generated by `configure`/`make datagen` | Contains compiled data-generator executables; building does not execute them. |
| `build/lib/libghl*` | compiled output | Shared-library names include configured extension and versioned symlink flow. |

`generate_makefile.sh` generated/derived outputs. This route is currently
broken; paths below describe intended/generated locations, not a supported
build command:

| Path or pattern | Boundary | Notes |
| --- | --- | --- |
| `Makefile` | generated by `generate_makefile.sh` | Current output contains malformed Induction targets, a GRHayLib source, and an invalid checkout-local include root. Do not run it. |
...
Do not hand-edit generated Makefiles as durable docs. `configure` is the
supported generator path represented by current build evidence. Legacy
`generate_makefile.sh` repair or retirement needs maintainer confirmation.

Configured cleanup boundaries differ:

- `make clean` removes library objects, normal test executables/helper objects,
  and built shared libraries, but does not remove data-generator executables or
  their objects;
- `make realclean` then removes the build directory, all of `test/`, and the
  generated top-level `Makefile`; and
- neither target removes root-level downloaded/generated fixture files.
