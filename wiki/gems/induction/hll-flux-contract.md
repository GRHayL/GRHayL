...
Here `Btilde` means the densitized field used by this implementation,
`sqrt(gamma) B`. The `B` path multiplies the complete HLL result by one
caller-supplied `psi6 = sqrt(gamma)` evaluated at the selected `A_i` point;
the routine does not densitize individual packed face values.

Both source files are compiled by
[GRHayL/Induction/make.code.defn](../../../GRHayL/Induction/make.code.defn).
Both routines return one scalar, have no side effects, and provide no status
code or input validation.
...
Test helpers evaluate only `2 <= i,j,k < dirlength-2`. `A_dir` and the offset
arrays are helper conventions, not public HLL API arguments.
...
- `c1_min + c1_max` and `c2_min + c2_max` must both be nonzero; both public
  routines divide by these sums without a guard. Production Flux_Source
  `cmin/cmax` are non-negative magnitudes, not signed extrema.
...
Its input generator samples `cmin` and `cmax` independently from `[-10,10]`
and only rejects exact zero sums. Therefore this tests HLL algebra and packing,
not physical speed sign conventions or Flux_Source-to-Induction coupling.
...
Local HLL outputs are generated by the same `compute_A_flux_with_*` helpers used
by replay, so they are same-kernel regression references. ET Legacy outputs are
downloaded external fixtures and cover only the `B` path. The ordinary runner
and all five compiler workflows configure both replay families; tracked files
alone do not establish a current execution result.

## Evidence Status

- **Declared/defined/build-configured:** both functions and `ghl_HLL_vars` are
  public; both definitions are in the Induction manifest in either HDF5 mode.
- **Direct replay:** local test calls `B` and `Btilde` for all three `A_i`;
  ET Legacy calls `B` for all three.
- **Coverage gaps:** no `Btilde` ET Legacy comparison, zero-denominator check,
  realistic characteristic-speed input, or end-to-end Flux_Source speed call.
- **Downstream:** GRHayLib lists Induction directories for compilation, but no
  call site appears in the checked-in `implementations/GRHayLib/` tree.
