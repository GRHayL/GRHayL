...
Each routine takes `const double U[4]` plus `Ur` and `Ul` output pointers. For the left face of cell `i`, the 4-point stencil is centered around that face and corresponds to values from `i-2` through `i+1`. The routines reconstruct the right and left values at the left face, matching the Reconstruction gem's left-face contract described in [`docs/raw/Reconstruction.dox`](../../../docs/raw/Reconstruction.dox).
...
Direct PLM coverage lives in [`Unit_Tests/unit_test_PLM_reconstruction.c`](../../../Unit_Tests/unit_test_PLM_reconstruction.c). The test dispatches over `ghl_minmod_reconstruction`, `ghl_mc_reconstruction`, and `ghl_superbee_reconstruction`, then compares right/left face outputs against trusted and perturbed PLM fixture data.

Fixture generation lives in [`Unit_Tests/data_gen/unit_test_data_PLM_reconstruction.c`](../../../Unit_Tests/data_gen/unit_test_data_PLM_reconstruction.c). The generator writes `PLM_reconstruction_input.bin`, `PLM_reconstruction_output.bin`, and `PLM_reconstruction_output_pert.bin` for the three PLM methods.

The broader test routing table is [`wiki/test-map.md`](../../test-map.md), which records PLM reconstruction as direct Reconstruction/PLM coverage and notes that this PLM test does not cover PPM.
