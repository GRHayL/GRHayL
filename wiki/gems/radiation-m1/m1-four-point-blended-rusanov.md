...
The implementation accepts `0 <= theta <= 2`. A zero difference does not
qualify as a same-sign or opposite-sign pattern, so it takes the zero-limiter
case. The decision is made independently for all five components; a sawtooth
