...
The source-policy dispatcher receives a frozen metric, frozen fluid primitives,
frozen rate bundle, validated `state_input`, validated `state_transport`, the
coordinate-time stage `dt`, and positive baryon-density normalization. Lower-
level homogeneous and thin source calls receive one validated `state_in`,
which is their source base; they do not receive a separate transport-predicted
state. The paired electron-flavor source call receives `state_input[2]` and
`state_transport[2]` arrays in its coupled interface and uses the transport
array as its paired source base.
The library forms `dt_alpha = alpha * dt` and uses that lapse-weighted interval
for all local radiation-source evolution, stiffness decisions, and implicit
...
The source-policy dispatcher is transactional and uses the transport-predicted
state as its source base. Homogeneous and thin wrappers are transactional
around their single `state_in` source base. The existing finite-difference /
Newton machinery handles the local implicit solve, with line search and
fallback substepping. Any thin, thick, or scattering-regime reductions are
internal source handling; they do not expose a second M1 transport or closure
method.
...
For the dispatcher, every successful update is measured relative to
`state_transport`, not `state_input`. The dispatcher returns the updated state,
radiation increments, the total radiation-number increment, the charged-current
lepton increment, equal-and-opposite densitized matter energy/momentum
increments, and path diagnostics. It never owns an RK counter or publishes a
Newton guess.

## Failure and fallback behavior

Dispatcher outputs are initialized transactionally to `state_transport` and a
zero exchange packet. Invalid inputs, rate/provider failure, closure rejection,
