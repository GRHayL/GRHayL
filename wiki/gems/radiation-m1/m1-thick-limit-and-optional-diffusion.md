...
The helper returns a validity flag. The candidate must be finite and strictly
positive to be usable by the diffusion correction. A nonpositive candidate is
reported as invalid rather than repaired into a diffusion state.
...
The [M1 test guide](../../../Unit_Tests/README.m1.md) identifies the test as a
local diffusion-kernel check, not evidence for full-grid thick-limit
convergence. The shared source and API are
