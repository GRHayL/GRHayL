...
| `./Unit_Tests/unit_test_hybrid_failure.c` | Expected `Noble2D` maximum-iteration and singular failures, fixture-free `Noble1D_entropy2` support, and independent curved-metric Font1D round trips. Entropy2 cases perform forward primitive-to-conservative round trips for Simple EOS and both pieces of a two-piece Hybrid EOS, including magnetic field; they also check selector and mutating-main-to-backup success, diagnostics, direct-root density-limit ownership, initialization-error propagation, nonpositive density, iteration failure, and negative pressure. | Hard-coded cases; no external binary fixture. | Run by `./.github/run_tests.sh` as `./test/unit_test_hybrid_failure`; also covered by workflow `c2p-failure` jobs. | No HDF5 table. Simple and Hybrid EOS setup is internal to the test. | None. |
...
the hybrid Con2Prim generator and byte-compares all 13 files it produces with a
fresh `GRHayL/TestData` checkout; it does not publish rewritten fixtures.
