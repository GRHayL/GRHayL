...
| `./Unit_Tests/unit_test_c2p_nn_guess.c` | Direct tabulated NN primitive-guess coverage for model validation, bounded `x` inference fallback, root/embedded/legacy HDF5 model loads, malformed HDF5 dataset paths, and failed-load preservation of an existing model. | No external binary fixture. HDF5-enabled runs use a unique private temporary directory and clean up their fixed model basenames on normal and handled failure exits. | Run by `./.github/run_tests.sh` as `./test/unit_test_c2p_nn_guess`; also covered by workflow `c2p-failure` jobs through matrix entry `c2p_nn_guess`. | HDF5-enabled builds cover loader paths. With `GHL_DISABLE_HDF5`, the test still compiles and runs validation/inference-fallback checks without creating HDF5 temporary files. | None. The HDF5 files are created and removed by the test itself, not by a data generator. |
...
| `./Unit_Tests/unit_test_hybrid_failure.c` | Expected `Noble2D` maximum-iteration and singular failures, plus fixture-free `Noble1D_entropy2` support coverage. Entropy2 cases perform forward primitive-to-conservative round trips for Simple EOS and both pieces of a two-piece Hybrid EOS, including curved metric and magnetic field; they also check selector and backup success, diagnostics, initialization-error propagation, nonpositive density, iteration failure, and negative pressure. | Hard-coded cases; no external binary fixture. | Run by `./.github/run_tests.sh` as `./test/unit_test_hybrid_failure`; also covered by workflow `c2p-failure` jobs. | No HDF5 table. Simple and Hybrid EOS setup is internal to the test. | None. |
