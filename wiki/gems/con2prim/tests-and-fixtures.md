...
| `./Unit_Tests/unit_test_hybrid_failure.c` | Expected `Noble2D` maximum-iteration and singular failures, fixture-free `Noble1D_entropy2` support, and independent curved-metric Font1D round trips. Font1D cases cover iterative and exact-shortcut `n_iter`, below-EOS-density evaluation, plus closure and reconservation on a later piece of a multi-piece EOS. The test also checks utilde limiting is monotone across its roundoff-slack boundary. Entropy2 cases cover Simple and equivalent one-piece Hybrid EOS round trips, reject mismatched-Gamma metadata, and check direct diagnostics, multi-method recovery, an exact `W=4` seed, backup routing, density-limit ownership, and failure codes. | Hard-coded cases; no external binary fixture. | Run by `./.github/run_tests.sh` as `./test/unit_test_hybrid_failure`; also covered by workflow `c2p-failure` jobs. | No HDF5 table. Simple and Hybrid EOS setup is internal to the test. | None. |
...
downloads fixtures. Generated outputs are implementation-derived regression
oracles, not independent evidence for changed solver physics. The fixture-free
forward primitive-to-conservative round trips in `unit_test_hybrid_failure.c`
provide the independent Font1D closure check, including a multi-piece cold EOS.
