...
| `./Unit_Tests/unit_test_hybrid_failure.c` | Expected `Noble2D` maximum-iteration and singular failures, fixture-free `Noble1D_entropy2` support, and independent curved-metric Font1D round trips. Font1D cases cover iterative and exact-shortcut `n_iter`, plus closure and reconservation on a later piece of a multi-piece EOS. Entropy2 cases perform forward primitive-to-conservative round trips for Simple EOS and both pieces of a two-piece Hybrid EOS, including magnetic field; they also check selector and start-state-sensitive main-to-backup routing, all-method failure diagnostics, direct-root density-limit ownership, initialization-error propagation, nonpositive density, iteration failure, and negative pressure. | Hard-coded cases; no external binary fixture. | Run by `./.github/run_tests.sh` as `./test/unit_test_hybrid_failure`; also covered by workflow `c2p-failure` jobs. | No HDF5 table. Simple and Hybrid EOS setup is internal to the test. | None. |
| `./Unit_Tests/unit_test_apply_conservative_limits.c` | Conservative floor/limit application and diagnostic flags around `ghl_apply_conservative_limits`, including poisoned diagnostics initialization and synthetic conservative energies below, at, and above the configured atmosphere floor. | `metric_Bfield_initial_data.bin`, `apply_conservative_limits_input.bin`, `apply_conservative_limits_output.bin`, `apply_conservative_limits_output_pert.bin`. `./.github/run_tests.sh` downloads these from `con2prim/` before running the binary. | Run by `./.github/run_tests.sh` as `./test/unit_test_apply_conservative_limits`; also covered by workflow `c2p-routines` jobs. | No HDF5 table. | `./Unit_Tests/data_gen/unit_test_data_con2prim_multi_method_hybrid.c` writes these fixtures. Ordinary runner path downloads fixtures; it does not regenerate them. |
