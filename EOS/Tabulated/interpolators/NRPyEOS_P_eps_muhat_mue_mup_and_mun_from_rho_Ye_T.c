 ...
 * (c) 2022 Leo Werneck
 */
void NRPyEOS_P_eps_muhat_mue_mup_and_mun_from_rho_Ye_T(const NRPyEOS_params *restrict eos_params,
                                                       const double rho,
                                                       const double Y_e,
                                                       const double T,
                                                       double *restrict P,
  ...
  if( report.error ) {
    fprintf(stderr,"(NRPyEOS) Inside NRPyEOS_P_eps_muhat_mue_mup_and_mun_from_rho_Ye_T. Error message: %s (key = %d)",report.message,report.error_key);
  }
