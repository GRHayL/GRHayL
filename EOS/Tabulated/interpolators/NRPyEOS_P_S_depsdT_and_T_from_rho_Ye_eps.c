 ...
 * (c) 2022 Leo Werneck
 */
void NRPyEOS_P_S_depsdT_and_T_from_rho_Ye_eps(const NRPyEOS_params *restrict eos_params,
                                              const double rho,
                                              const double Y_e,
                                              const double eps,
                                              double *restrict P,
  ...
  if( report.error ) {
    fprintf(stderr,"(NRPyEOS) Inside NRPyEOS_P_S_depsdT_and_T_from_rho_Ye_eps. Error message: %s (key = %d)",report.message,report.error_key);
  }
