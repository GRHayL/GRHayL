...
void interpolate_for_A_i_rhs(
  ...
  double gupxx_jk = avg2(gauge_vars->gupxx, 0,0, 0,1, 0,1);
  double gupxy_jk = avg2(gauge_vars->gupxy, 0,0, 0,1, 0,1);
  double gupxz_jk = avg2(gauge_vars->gupxz, 0,0, 0,1, 0,1);
  ...
  // Note that gupij is \tilde{\gamma}^{ij}, so we need to multiply by \psi^{-4}.
  double gupxy_ik = avg2(gauge_vars->gupxy, 0,1, 0,0, 0,1);
  double gupyy_ik = avg2(gauge_vars->gupyy, 0,1, 0,0, 0,1);
  double gupyz_ik = avg2(gauge_vars->gupyz, 0,1, 0,0, 0,1);
  ...
  // Note that gupij is \tilde{\gamma}^{ij}, so we need to multiply by \psi^{-4}.
  double gupxz_ij = avg2(gauge_vars->gupxz, 0,1, 0,1, 0,0);
  double gupyz_ij = avg2(gauge_vars->gupyz, 0,1, 0,1, 0,0);
  double gupzz_ij = avg2(gauge_vars->gupzz, 0,1, 0,1, 0,0);

  double lapse_Psi2_ij = avg2(lapse_psi2, 0,1, 0,1, 0,0);
...
//static inline double avg(double f[][][],int imin,int imax, int jmin,int jmax, int kmin,int kmax) {
