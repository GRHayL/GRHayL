...
void calculate_phitilde_and_A_i_rhs(const double *restrict dX,
                                   const double Lorenz_damping_factor,
                                   induction_gauge_rhs *restrict vars) {

  double dXm1=1.0/dX[0];
  double dYm1=1.0/dX[1];
  double dZm1=1.0/dX[2];
  ...
  vars->A_x_gauge_rhs = dXm1*(vars->alpha_Phi_minus_betaj_A_j_interp[1] - vars->alpha_Phi_minus_betaj_A_j_interp[0]);
  vars->A_y_gauge_rhs = dYm1*(vars->alpha_Phi_minus_betaj_A_j_interp[2] - vars->alpha_Phi_minus_betaj_A_j_interp[0]);
  vars->A_z_gauge_rhs = dZm1*(vars->alpha_Phi_minus_betaj_A_j_interp[3] - vars->alpha_Phi_minus_betaj_A_j_interp[0]);
  ...
  if(vars->shiftx_interp[2] < 0.0) {
    vars->phitilde_rhs+=0.5*dXm1*(+    vars->shiftx_interp[0]*vars->phitildex[0]
                                  -4.0*vars->shiftx_interp[1]*vars->phitildex[1]
                                  +3.0*vars->shiftx_interp[2]*vars->phitildex[2]);
  } else {
    vars->phitilde_rhs+=0.5*dXm1*(-    vars->shiftx_interp[4]*vars->phitildex[4]
  ...
  if(vars->shifty_interp[2] < 0.0) {
    vars->phitilde_rhs+=0.5*dYm1*(+    vars->shifty_interp[0]*vars->phitildey[0]
                                  -4.0*vars->shifty_interp[1]*vars->phitildey[1]
                                  +3.0*vars->shifty_interp[2]*vars->phitildey[2]);
  } else {
    vars->phitilde_rhs+=0.5*dYm1*(-    vars->shifty_interp[4]*vars->phitildey[4]
  ...
  if(vars->shiftz_interp[2] < 0.0) {
    vars->phitilde_rhs+=0.5*dZm1*(+    vars->shiftz_interp[0]*vars->phitildez[0]
                                  -4.0*vars->shiftz_interp[1]*vars->phitildez[1]
                                  +3.0*vars->shiftz_interp[2]*vars->phitildez[2]);
  } else {
    vars->phitilde_rhs+=0.5*dZm1*(-    vars->shiftz_interp[4]*vars->phitildez[4]
                                  +4.0*vars->shiftz_interp[3]*vars->phitildez[3]
                                  -3.0*vars->shiftz_interp[2]*vars->phitildez[2]);
  }

  // Next we add \partial_j (\alpha \sqrt{\gamma} A^j) to \partial_t psi6phi:
  vars->phitilde_rhs+=dXm1*(vars->alpha_sqrtg_Ax_interp[0] - vars->alpha_sqrtg_Ax_interp[1])
                    + dYm1*(vars->alpha_sqrtg_Ay_interp[0] - vars->alpha_sqrtg_Ay_interp[1])
                    + dZm1*(vars->alpha_sqrtg_Az_interp[0] - vars->alpha_sqrtg_Az_interp[1]);
