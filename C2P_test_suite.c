...
//    printf("Beginning %s test for routine %s\n",con2prim_test_names[which_routine]);
      ...
      for(int i=0;i<2/*npoints*/;i++) { // Density loop
...
double gdet = fabs(gxx * (gyy*gzz - gyz*gyz) + gxy * (gyz*gxz - gxy*gzz) + gxz * (gxy*gyz - gyy*gxz));
printf("gdet %.16e\n", gdet);
          } else {
            gxx = 1.0;
            gyy = 1.0;
...
double tmp2 = 1.0 - tmp_var/metric.lapseinv2;
          double u0 = 1.0/sqrt( SQR(metric.lapse) - tmp_var );

printf("tmp %.16e\nalp %.16e\nu0 %.16e\n", tmp_var, SQR(metric.lapse), u0);
...
//            fprintf(outfile, "Relative error for prim %s: (%e -> %e) %.16e\n", "Bz", prims_orig.Bz, prims.Bz, prims_error.Bz);
            ...
            fprintf(prims_out, "%d %.16e %.16e %.16e %.16e %.16e %.16e "
                               "%.16e %.16e %.16e %.16e %.16e %.16e "
                               "%.16e %.16e %.16e\n",
                               i, prims_orig.rho, prims.rho, prims_error.rho,
