 ...
 */
static void EigenCoord_set_x0x1x2_inbounds__i0i1i2_inbounds_single_pt(const paramstruct *restrict params, REAL *restrict xx[3],
...
#include "./set_Cparameters.h"
  ...
  {
    // xx_to_Cart for EigenCoordinate Cartesian (orig coord = Cartesian):
    REAL xx0 = xx[0][i0];
    REAL xx1 = xx[1][i1];
    REAL xx2 = xx[2][i2];
    /*
  ...
  // Cart_to_xx for EigenCoordinate Cartesian (orig coord = Cartesian);
  /*
   *  Original SymPy expressions:
   *  "[Cart_to_xx0_inbounds = Cartx,
   *    Cart_to_xx1_inbounds = Carty,
   *    Cart_to_xx2_inbounds = Cartz]"
   */
  {
    Cart_to_xx0_inbounds = Cartx;
    Cart_to_xx1_inbounds = Carty;
  ...
  {
    // xx_to_Cart for Coordinate Cartesian):
    REAL xx0 = xx[0][i0];
    REAL xx1 = xx[1][i1];
    REAL xx2 = xx[2][i2];
    /*
  ...
  {
    // xx_to_Cart_inbounds for Coordinate Cartesian):
    REAL xx0 = xx[0][i0_inbounds];
    REAL xx1 = xx[1][i1_inbounds];
    REAL xx2 = xx[2][i2_inbounds];
    /*
     *  Original SymPy expressions:
     *  "[xCart_from_xx_inbounds = xx0,
     *    yCart_from_xx_inbounds = xx1,
     *    zCart_from_xx_inbounds = xx2]"
     */
    xCart_from_xx_inbounds = xx0;
    yCart_from_xx_inbounds = xx1;
    zCart_from_xx_inbounds = xx2;
...
#define EPS_REL 1e-8
  ...
  if(fabs( (double)(xCart_from_xx - xCart_from_xx_inbounds) ) > EPS_REL * norm_factor ||
    ...
    fprintf(stderr,"Error in Cartesian coordinate system: Cartesian disagreement: ( %.15e %.15e %.15e ) != ( %.15e %.15e %.15e ) | xx: %e %e %e -> %e %e %e | %d %d %d\n",
 ...
 */
static void set_parity_for_inner_boundary_single_pt(const paramstruct *restrict params, const REAL xx0,const REAL xx1,const REAL xx2,
...
#include "./set_Cparameters.h"
  ...
  REAL REAL_parity_array[10];
  {
     ...
     *    REAL_parity_array[1] = 1,
     *    REAL_parity_array[2] = 1,
     *    REAL_parity_array[3] = 1,
     *    REAL_parity_array[4] = 1,
     *    REAL_parity_array[5] = 1,
     *    REAL_parity_array[6] = 1,
     *    REAL_parity_array[7] = 1,
     *    REAL_parity_array[8] = 1,
     *    REAL_parity_array[9] = 1]"
     */
    {
      REAL_parity_array[0] = 1;
      REAL_parity_array[1] = 1;
