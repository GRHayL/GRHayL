  ...
  for(int k=0; k<cctk_lsh[2]; k++)
    for(int j=0; j<cctk_lsh[1]; j++)
      for(int i=0; i<cctk_lsh[0]; i++) {
        ...
        const double vxL = vx[index];
        const double vyL = vy[index];
...
// GRHayL_IGH directly uses these HydroBase variables now
...
//        eps[index]   = PRIMS[PRESSURE]/PRIMS[RHOB]/(gamma_th-1.0);
        ...
        const double shiftxL = betax[index];
        const double shiftyL = betay[index];
        const double shiftzL = betaz[index];

        vel[index4D0] = (vxL + shiftxL)*lapseL_inv;
        vel[index4D1] = (vyL + shiftyL)*lapseL_inv;
        vel[index4D2] = (vzL + shiftzL)*lapseL_inv;
        ...
        if(one_minus_one_over_alpha_u0_squared > 1.0) {
          CCTK_VINFO("convert_from_GRHayL_IGH_to_HydroBase WARNING: Found superluminal velocity. This should have been caught by GRHayL_IGH.");
        }
